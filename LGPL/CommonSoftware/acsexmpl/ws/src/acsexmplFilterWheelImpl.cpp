...
void FilterWheel::readConfiguration(Descriptor* descr)
{
    ...
    CDB::DAL_var dal_p = getContainerServices()->getCDB();
    // Get tha DAo record for the component
    CDB::DAO_var dao_p = dal_p->get_DAO_Servant(m_fullName.c_str());

   
    // Read the sequence of long describing the position of each slot
    // in the wheel
    CDB::longSeq_var lngSeq = dao_p->get_long_seq("SlotStep");
    ...
    // For example in <e Name="Red" Delta=..../> the key is Red.
	CDB::stringSeq_var fltKeys = dao_p->get_string_seq("Filter");
...
void FilterWheel::updateFilter(ACE_CString name, int delta)
{
    ...
    CDB::WDAO_var wdao_p = wdal_p->get_WDAO_Servant(m_fullName.c_str());
...
void FilterWheel::updateWheel(int slot, int step)
{
    ...
    CDB::longSeq_var lngSeq =wdao_p->get_long_seq("SlotStep");
    ...
    wdao_p->set_long_seq("SlotStep", lngSeq.in());
}
 ...
 void FilterWheel::initialize()
{
    ...
    try 
	{
	CORBA::Any_var characteristic = get_characteristic_by_name("AvailableSlots");
	const char* val;
	if (!(characteristic.in()>>=val))
	    {
...
CORBA::Long FilterWheel::calibrateFilter(const char* name)
{
	...
	if (found) {
		...
		return m_wheelConfiguration[t].delta;
	} else {
