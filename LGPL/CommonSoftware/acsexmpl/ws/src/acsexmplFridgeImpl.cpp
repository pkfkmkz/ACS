...
void FridgeControl::cleanUp()
{
    ...
    FRIDGE::ROOnOffStates_var powerStat = powerStatus();
    if((m_controlLoop_p!=0) && (powerStat->get_sync(completion.out())!=FRIDGE::OFF))
	{
	off();
	}
...
FridgeControl::loadData()
{
    ...
    ACS::ROdouble_var currTemp = currTemperature();
    ACS::RWdouble_var refTemp = refTemperature();
    current   = currTemp->get_sync(completion.out());
    reference = refTemp->get_sync(completion.out());
