...
public class NCPublisher<T> extends OSPushSupplierPOA implements AcsEventPublisher<T>, ReconnectableParticipant {
     ...
     * is thrown during the publication of an event.
     * @param autoreconnect Whether autoreconneting to the channel should be done
     * @return A boolean that states whether the reconnection was successful
     */
     ...
     * @param reconnect Whether to reconnect before next steps
     * @param retry Whether to retry publishing the event
     * @param queue Whether to queue the event if not retrying
     * @param throwEx Whether to throw exception if message has not been published
     * @throws AcsJException
     *             if the event cannot be published for some reason or another and throwEx is true.
     */
    protected void publishCORBAEventOnError(StructuredEvent se, T customData, Throwable ex, boolean reconnect, boolean retry, boolean queue, boolean throwEx) throws AcsJException {
        boolean connected = true;
        boolean retried = false;
        boolean queued = false;

        if (reconnect && autoreconnect) {
            connected = reconnect();
        }

        if (connected && retry) {
            // Try to publish the event again
            publishCORBAEvent(se, customData, false);
            retried = true;
        } else if (queue) {
        ...
        if (!retried && throwEx) {
            String reason = "Failed to publish event on channel '" + channelName + "': " + ex.getClass().getName() + " was thrown.";
            if (queued) {
                reason += " Event was queued for later publishing.";
            }
            if (ex instanceof org.omg.CORBA.SystemException) {
                org.omg.CORBA.SystemException cex = (org.omg.CORBA.SystemException) ex;
                AcsJCORBAProblemEx jex = new AcsJCORBAProblemEx();
                jex.setMinor(cex.minor);
                jex.setInfo(reason);
                throw jex;
            }
            ...
            reason += " Cause: " + ex.getMessage();
			AcsJUnexpectedExceptionEx jex = new AcsJUnexpectedExceptionEx(reason);
			throw jex;
        }
    }
    ...
    protected void publishCORBAEvent(StructuredEvent se, T customData) throws AcsJException {
        publishCORBAEvent(se, customData, true);
    }
     ...
     * @param se A complete structured event
     * @param customData  The user-supplied event data, needed for eventProcessingHandler notification.
     * @param retry  Whether we should retry sending an event upon failure
     * @throws AcsJException
     *             if the event cannot be published for some reason or another.
     */
    protected void publishCORBAEvent(StructuredEvent se, T customData, boolean retry) throws AcsJException {
        try {
            ...
            publishCORBAEventOnError(se, customData, ex, false, false, true, true);
        } catch (org.omg.CORBA.OBJECT_NOT_EXIST ex) {
            // There's a problem with a Notify Service reference
            publishCORBAEventOnError(se, customData, ex, retry, retry, true, true);
        } catch (org.omg.CORBA.TIMEOUT ex) {
            // Publishing to the Notify Service took too long
            publishCORBAEventOnError(se, customData, ex, retry, retry, true, true);
        } catch (org.omg.CosEventComm.Disconnected ex) {
            // The Notify Service is disconnected
            publishCORBAEventOnError(se, customData, ex, retry, retry, true, true);
            ...
            // Other ex
            publishCORBAEventOnError(se, customData, thr, false, false, true, true);
        }
    }
