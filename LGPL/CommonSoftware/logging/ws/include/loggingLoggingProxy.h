 ...
 *
 * "@(#) $Id: loggingLoggingProxy.h,v 1.44 2012/02/29 12:50:09 tstaig Exp $"
 *
 * who       when        what
 * --------  ----------  ----------------------------------------------
 ...
 * @version "@(#) $Id: loggingLoggingProxy.h,v 1.44 2012/02/29 12:50:09 tstaig Exp $"
 */
class logging_EXPORT LoggingProxy : public ACE_Log_Msg_Callback
{
  ...
  public:
    ...
    /// Specifies the id of the thread. Pointer to the id must be stored in
    /// the thread-specific storage!
    static void ThreadId(const ACE_TCHAR *szName);

    /// Returns the id of the thread.
    static const ACE_TCHAR *ThreadId();
    ...
    template<class... Args>
    static void AddData(const ACE_TCHAR *szName, const ACE_TCHAR *szFormat, Args... argp);

    static LoggingTSSStorage& getTSS();
  ...
  private:
   ...
   // ACSLoggingLog::LogBinaryRecord * m_binaryRecord;
    ...
    /// Thread specific storage
    ///
    thread_local static LoggingTSSStorage tss;
    static int instances;
    static ACE_Recursive_Thread_Mutex classMutex;

    //Thread mutex to make makeDirectory() method access to file system safe
    static ACE_Recursive_Thread_Mutex createDirMutex;
