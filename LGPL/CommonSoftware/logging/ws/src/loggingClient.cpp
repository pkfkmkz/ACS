...
const ACE_TCHAR* ACSStructuredPushConsumer::m_LogEntryTypeName[] =
{
      ...
      channelName.compare(acscommon::LOGGING_CHANNEL_NAME)==0 )
  {
	...
	else if (reclist)
	{
        ...
        if (log)
        {
            if (toSyslog)
            {
                writeSyslogMsg(LoggingProxy::BinToXml(const_cast<ACSLoggingLog::LogBinaryRecord*>(log)).c_str());
            }
            else
            {
                if(toFile){
                    ACE_OS::fprintf(outputFile,"%s\n",LoggingProxy::BinToXml(const_cast<ACSLoggingLog::LogBinaryRecord*>(log)).c_str());
                    ACE_OS::fflush (outputFile);
                }else{
                    ACE_OS::printf("%s\n",LoggingProxy::BinToXml(const_cast<ACSLoggingLog::LogBinaryRecord*>(log)).c_str());
                    ACE_OS::fflush (stdout);
