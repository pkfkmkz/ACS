...
thread_local LoggingTSSStorage LoggingProxy::tss;
...
ACE_Recursive_Thread_Mutex LoggingProxy::createDirMutex;
...
LoggingProxy::log(ACE_Log_Record &log_record)
{
  ...
  void LoggingProxy::sendBinLogs(ACE_Log_Record &log_record, const ACE_TCHAR * timestamp, const ACE_TCHAR * entryType){
    ...
    if (flags & LM_RUNTIME_CONTEXT)
	{
	    ...
	    s_log->Host = CORBA::string_dup( (host()==0) ? log_msg->local_host() : host());
            ...
            const ACE_TCHAR * threadId = tss.threadId();
            if (threadId){
                s_log->ThreadId = threadId;
            }
        }
	}
    // context
  ...
  void LoggingProxy::sendXmlLogs(ACE_Log_Record &log_record,  const ACE_TCHAR * timestamp, const ACE_TCHAR * entryType){
	...
	if (log_msg)
	    ...
	    xml += " File=\"";
	    xml += tss.file();
	    ACE_OS::sprintf(line, "\" Line=\"%lu\"", tss.line());
	    xml += line;
	    }
	}
...
void
LoggingProxy::ThreadId(const ACE_TCHAR *szName)
{
    tss.threadId(szName);
}

const ACE_TCHAR *
LoggingProxy::ThreadId()
{
    return tss.threadId();
}
...
void LoggingProxy::AddData<>(const ACE_TCHAR *szName, const ACE_TCHAR *szValue) {
  ...
  snprintf (data, ADD_DATA_VALUE_MAX, "%s", szValue);
...
std::string LoggingProxy::BinToXml(ACSLoggingLog::LogBinaryRecord* record){
    ...
    if(record->Priority != record->type){
        ACE_OS::sprintf(line, " Priority=\"%d\"", record->Priority); //align to ACS priorty
        xml += line;
    }
    /*xml += " Audience=\"";
...
LoggingTSSStorage& LoggingProxy::getTSS() {
	return tss;
}
...
{
	ACE_Guard<ACE_Recursive_Thread_Mutex> guard(createDirMutex);
...
int LoggingProxy::logNonCentralizedLoggerCache(CacheLogger * logger,const ACE_CString & key, const ACE_TCHAR * timestamp)
{
	if (logger->open(key.c_str())==0)
	{
		if(!m_logBin)
		{
			...
			rapidxml::xml_node<>* xmlElement;
			ACE_CString str(size_t(32));
			int prio;
			for (LogDeque::iterator iter = m_cache.begin(); iter != m_cache.end(); iter++)
			{
				std::string xml(iter->c_str());
				...
				xmlElement = doc.first_node();
				if (xmlElement->first_attribute("Priority")) {
					int tprio = atoi(xmlElement->first_attribute("Priority")->value());
					if (tprio) prio = tprio;
				} else {
