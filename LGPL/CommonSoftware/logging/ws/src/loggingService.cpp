...
{
  // give the naming service a couple of seconds, everything is still fresh
  sleep(5);
...
LoggingService::create_EC ()
{
  ...
  if (!m_logBin) 
  {
    channelName = std::string(acscommon::LOGGING_CHANNEL_XML_NAME);
  }
  else
  {
    channelName = std::string(acscommon::LOGGING_CHANNEL_NAME);
  }

  channelAndDomainName = channelName
      + acscommon::NAMESERVICE_BINDING_NC_DOMAIN_SEPARATOR 
      + acscommon::ACS_NC_DOMAIN_LOGGING;

  name[0].id = CORBA::string_dup(channelAndDomainName.c_str());
  name[0].kind = acscommon::NC_KIND;

  // retry in the case the service is mangled e.g.: LoggingChannel exists but 
  // the resolution raises a NotFound exception
  int retries = 10;

  while (retries--) {
    try
    {
      // use the naming service to get the event channel reference
      ...
      retries = false;
      ACS_SHORT_LOG((LM_INFO, "Logging channel %s narrowed...", 
          channelAndDomainName.c_str()));
    }
    catch(CosNaming::NamingContext::NotFound& ex)
    {
      ex._tao_print_exception(__FILE__);

      ACS_SHORT_LOG((LM_ERROR, 
          "CosNaming::NamingContext::NotFound, NotFound.why = %d", 
          ex.why));
