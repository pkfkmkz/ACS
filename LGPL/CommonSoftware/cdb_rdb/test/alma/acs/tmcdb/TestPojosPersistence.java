...
//		hibernateLoggerFactory.getLogger("blabla").info("Greetings from your hibernate logger");
	...
	public void testSimpleSave() throws Exception {
		...
		try {
			...
			} catch(HibernateUtilException|PersistenceException e) {
	...
	public void testCriteriaAPI() throws Exception {
		...
		try {
			...
			CriteriaQuery<NetworkDevice> cq = cb.createQuery(NetworkDevice.class);
			Root<NetworkDevice> root = cq.from(NetworkDevice.class);
			cq.where(cb.equal(root.get("name"), "wall-e"));
			TypedQuery<NetworkDevice> q = hibernateUtil.getSession().createQuery(cq);
			assertEquals(2, q.getResultList().size());

			cb = hibernateUtil.getSession().getCriteriaBuilder();
			cq = cb.createQuery(NetworkDevice.class);
			root = cq.from(NetworkDevice.class);
			cq.where(cb.and(cb.equal(root.get("name"), "wall-e"), cb.equal(root.get("networkName"), "wall-e.eso.org")));
			q = hibernateUtil.getSession().createQuery(cq);
			assertEquals(1, q.getResultList().size());

			cb = hibernateUtil.getSession().getCriteriaBuilder();
			cq = cb.createQuery(NetworkDevice.class);
			root = cq.from(NetworkDevice.class);
			cq.where(cb.equal(root.get("configuration"), config));
			q = hibernateUtil.getSession().createQuery(cq);
			assertEquals(3, q.getResultList().size());

			cb = hibernateUtil.getSession().getCriteriaBuilder();
			CriteriaQuery<Configuration> cq2 = cb.createQuery(Configuration.class);
			Root<Configuration> root2 = cq2.from(Configuration.class);
			cq2.where(cb.and(cb.equal(root2.get("configurationName"), "rtobarConfig"), cb.lessThan(root2.get("creationTime").as(Timestamp.class), cb.currentTimestamp())));
			TypedQuery<Configuration> q2 = hibernateUtil.getSession().createQuery(cq2);
			assertEquals(1, q2.getResultList().size());

			try {
				cb = hibernateUtil.getSession().getCriteriaBuilder();
				cq2 = cb.createQuery(Configuration.class);
				root2 = cq2.from(Configuration.class);
				cq2.where(cb.equal(root2.get("configuratioName"), "rtobarConfig"));
				q2 = hibernateUtil.getSession().createQuery(cq2);
				q2.getResultList();
	...
	public void testHQL() throws Exception {
		...
		try {
			...
			TypedQuery<NetworkDevice> q = hibernateUtil.getSession().createQuery("from NetworkDevice as nd where nd.name = :name");
			q.setParameter("name", "wall-e");
			assertEquals(2, q.getResultList().size());

			q = hibernateUtil.getSession().createQuery("from NetworkDevice as nd where nd.name = :name and nd.networkName = :networkName");
			q.setParameter("name", "wall-e");
			q.setParameter("networkName", "wall-e.eso.org" );
			assertEquals(1, q.getResultList().size());

			q = hibernateUtil.getSession().createQuery("from NetworkDevice as nd where nd.configuration = :configuration");
			q.setParameter("configuration", config );
			assertEquals(3, q.getResultList().size());

			q = hibernateUtil.getSession().createQuery("from Configuration as conf where conf.configurationName = :configName and creationTime < :time");
			q.setParameter("configName", "rtobarConfig");
			q.setParameter("time", new Date());
			assertEquals(1, q.getResultList().size());

			try {
				// typo: should be configurationName
				q = hibernateUtil.getSession().createQuery("from Configuration as conf where conf.configuratioName = :configuratioName");
				q.setParameter("configuratioName", "rtobarConfig");
				q.getResultList();
				fail("Should fail, property 'configuratioName' doesn't exist for Configuration objects");
