...
class TestFindSchemaFiles(unittest.TestCase):
    ...
    def test_with_content(self, isfile_mock):
        ...
        ebs = generateXsdPythonBinding.xmlpybind.EntitybuilderSettings.CreateFromDocument(xml)
        self.assertEqual(([('../config/test_idl/TestSchemaOtherDir.xsd',
                            'TestSchemaOtherDir')], ['TestSchemaOtherDir']),
                         generateXsdPythonBinding.find_schema_files(ebs,'.'))
...
class TestGenerateBindings(unittest.TestCase):
    ...
    def test_command_correct(self, call_mock):
        ...
        self.assertEqual(0, generateXsdPythonBinding.generate_bindings('foo', [('../bar/bar.xsd', 'Bar')],['Bar'], workdir='test'))
        self.assertEqual('pyxbgen --module-prefix=foo --binding-root=../'+site_packages+' --archive-to-file=../'+site_packages+'/foo.wxs --archive-path=../test/bindings -u ../bar/bar.xsd -m Bar', call_mock.call_args[0][0])

    @mock.patch('generateXsdPythonBinding.call')
    def test_call_failed(self, call_mock):
        call_mock.return_value = -1
        self.assertEqual(-1, generateXsdPythonBinding.generate_bindings('foo', [('../bar/bar.xsd', 'Bar')], ['Bar'], workdir='test'))
