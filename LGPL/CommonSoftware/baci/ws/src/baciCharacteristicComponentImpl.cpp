...
CharacteristicComponentImpl::CharacteristicComponentImpl(
    ...
    component_mp(0)
{
    CORBA::Long actionThreadStackSize, monitoringThreadStackSize, monitoringThreadMaxStartDelay;
    actionThreadStackSize = 1024;
    monitoringThreadStackSize = 2048;
    monitoringThreadMaxStartDelay = 0;
    ...
    try {
        if (dao) monitoringThreadMaxStartDelay = dao->get_long("monitoringThreadMaxStartDelay");
    } catch (ACSErr::ACSbaseExImpl& ex) {
        ex.log(LM_WARNING);
    } catch (...) {
    }
    ...
    monitoringThreadMaxStartDelay *= 1000*10; // that we have delay in ACS time (100ns)

    // Create Component
    component_mp = new BACIComponent(getContainerServices()->getThreadManager(), name, getContainerServices()->getType(), this, actionThreadStackSize, monitoringThreadStackSize);
    if (component_mp==0) {
        return;
    }
    component_mp->setMTMaxStartDelay(monitoringThreadMaxStartDelay);
