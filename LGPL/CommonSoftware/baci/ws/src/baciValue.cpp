...
//TBDeleted      OUTPUT_INLINE_TYPE(pattern, BACIpattern)
      ...
      OUTPUT_PTR_SEQ_TYPE_WITH_BOUND(longLongSeq, BACIlongLongSeq)
      OUTPUT_PTR_SEQ_TYPE_WITH_BOUND(uLongLongSeq, BACIuLongLongSeq)
...
BACIValue::getAny(CORBA::Any &any) const
{
	...
	case type_uLongSeq:
	    ...
	    // Sequencs of longLong-s.
	case type_longLongSeq:
	    any <<= this->longLongSeqValue();
	    break;

	    // Sequencs of uLongLong-s.
	case type_uLongLongSeq:
	    any <<= this->uLongLongSeqValue();
	    break;
