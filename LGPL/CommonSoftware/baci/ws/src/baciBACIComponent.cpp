...
void monitorThreadWorker(void* param) {
    ...
    ACS::TimeInterval pollInterval, init, time, lastPollTime, wait;
    BACIValue value;
    bool timeTriggered;
    ...
    std::mt19937 mt(rd());
    std::uniform_int_distribution<ACS::TimeInterval> dist(0, component_p->getMTMaxStartDelay());
    ...
    wait = dist(mt);
    init = time = getTimeStamp();
    while (!myself_p->exitRequested() && (time - init) < wait) {
        myself_p->sleep();
        time = getTimeStamp();
    }

    while (myself_p->check()==true && component_p->isInDestructionState()==false) {
        if (myself_p->isSuspended()==false) {
            // sync. monitors
            time = getTimeStamp();

            //guard.acquire();
            for (int n=0; component_p->isInDestructionState()==false && n < component_p->getPropertyCount() && myself_p->exitRequested()==false; n++) {
                property_p = component_p->getPropertyAt(n);
                if (property_p==0 || property_p->isInDestructionState()==true) {
                    continue;
                }
                if ((property_p->getMonitorCount() > 0) && ((property_p->getPollInterval() > 0) || property_p->hasTriggerOnValueMonitor()==true || property_p->hasTriggerOnValuePercentMonitor()==true)) {
                    ...
                    bool valueOnChange = property_p->hasTriggerOnValueMonitor() || property_p->hasTriggerOnValuePercentMonitor();
                    bool useMinTrig = property_p->getMonMinTriggerTime() != 0 && (pollInterval == 0 || property_p->getMonMinTriggerTime() < pollInterval);
                    if (valueOnChange && useMinTrig) {
                    ...
                    //ICT-13495: We only require to check for timeTriggered, since pollInterval is changed to minTriggerTime for delta checks
                    if (timeTriggered) {
                        CompletionImpl co;
                        value.reset();
                        property_p->getValue(property_p, (BACIValue*)&value, co, descOut);
                        ...
                        if(co.isErrorFree()) {
                            ...
                            CanNotGetValueCompletion errComp(co, __FILE__, __LINE__, "baci::monitorThreadWorker");
...
BACIComponent::BACIComponent( ACS::ThreadManager *thrMgr,
    ...
    monitorThreadMaxStartDelay_m(0),
    ...
    monitorThreadStackSize_m(monitorThreadStackSize)
{
...
void BACIComponent::setMTMaxStartDelay(const ACS::TimeInterval& monitorThreadMaxStartDelay)
{
  ACS_TRACE("baci::BACIComponent::setMTMaxStartDelay");
  monitorThreadMaxStartDelay_m=(monitorThreadMaxStartDelay>=0) ? monitorThreadMaxStartDelay : 0;
}
