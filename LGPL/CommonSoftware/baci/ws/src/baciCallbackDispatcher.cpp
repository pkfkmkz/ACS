  ...
  if (CORBA::is_nil(cb_p)==true) {
    if (archiver != 0) {
      ...
      if( completion.previousError.length() == 0 ) {
        ACS_ARCHIVE_PRIORITY(this->getName(), archiver->getName(), value.getArchiveType(), value, completion.timeStamp, archiver->getPriority());
        return true;
      } else {
        baciErrTypeProperty::ArchiveMonitorProblemCompletion c(completion, __FILE__, __LINE__, "baci::BACIComponent::dispatchCallback");
        c.log();
        return true;
      }//if-else
    } else {
...
WORKING_CALLBACK(longLongSeq)
WORKING_CALLBACK(uLongLongSeq)
...
DONE_CALLBACK(longLongSeq)
DONE_CALLBACK(uLongLongSeq)
