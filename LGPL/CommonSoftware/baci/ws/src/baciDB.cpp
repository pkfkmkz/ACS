...
ACS::uLongSeq
DBConnector::readULongSeq(ACE_CString name)
{
  ACS::uLongSeq val;
  val.length(0);
  name = getFullAddress(name);
  ...
  ULongArray lngArray;
  if (fld.GetULongArray(lngArray)==true)
  {
      int n = 0;
      val.length(lngArray.size());
      for (ULongArray::iterator iter = lngArray.begin();
	   iter != lngArray.end();
	   iter++)
	  {
	  val[n++] = *iter;
...
ACS::longLongSeq
DBConnector::readLongLongSeq(ACE_CString name)
{
  ACS::longLongSeq val;
  val.length(0);
  name = getFullAddress(name);
  ...
  LongLongArray lngArray;
  if (fld.GetLongLongArray(lngArray)==true)
  {
      int n = 0;
      val.length(lngArray.size());
      for (LongLongArray::iterator iter = lngArray.begin();
	   iter != lngArray.end();
	   iter++)
	  {
	  val[n++] = *iter;
...
ACS::uLongLongSeq
DBConnector::readULongLongSeq(ACE_CString name)
{
  ACS::uLongLongSeq val;
  val.length(0);
  name = getFullAddress(name);
  Field fld;
  if (getDBTable()->GetField(name, "value", fld)==false) 
  ...
  ULongLongArray lngArray;
  if (fld.GetULongLongArray(lngArray)==true)
  {
      int n = 0;
      val.length(lngArray.size());
      for (ULongLongArray::iterator iter = lngArray.begin();
	   iter != lngArray.end();
	   iter++)
	  {
	  val[n++] = *iter;
...
void
DBConnector::writeULongSeq(ACE_CString name, const ACS::uLongSeq & val)
{
  ULongArray lngArray;
  for (CORBA::ULong i=0; i < val.length(); i++)
      {
...
void
DBConnector::writeLongLongSeq(ACE_CString name, const ACS::longLongSeq & val)
{
  LongLongArray lngArray;
  for (CORBA::ULong i=0; i < val.length(); i++)
      {
...
void
DBConnector::writeULongLongSeq(ACE_CString name, const ACS::uLongLongSeq & val)
{
  ULongLongArray lngArray;
  for (CORBA::ULong i=0; i < val.length(); i++)
      {
