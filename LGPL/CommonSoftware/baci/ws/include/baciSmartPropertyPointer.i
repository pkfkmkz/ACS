...
baci::SmartPropertyPointer<T>::SmartPropertyPointer(T* prop,CharacteristicComponentImpl* component_p):
    SmartServantPointer<T>(prop, std::ptr_fun(&baci::ServantDeleter<T>)),
    component_mp(component_p)
{
    init();
}
...
{
    if (this->get()==NULL || component_mp==NULL) {
	return;
    } else {
...
baci::SmartPropertyPointer<T>* baci::SmartPropertyPointer<T>::operator=(const T* prop) {
    this->reset(prop, std::ptr_fun(&baci::ServantDeleter<T>));
    init();
    return this;
}

template<class T>
baci::SmartPropertyPointer<T>* baci::SmartPropertyPointer<T>::operator=(T* prop) {
    this->reset(prop, std::ptr_fun(&baci::ServantDeleter<T>));
    init();
    return this;
}
