...
BaseHelper::init(CosNaming::NamingContext_ptr nc_p)
{
	...
	catch(CosNaming::NamingContext::NotFound& ex)
	{
		...
		ACS_SHORT_LOG((LM_ERROR, "BaseHelper::init() error occured for the '%s' channel!, unknown error on init", name[0].id.in()));
		ACSErrTypeCommon::CORBAProblemExImpl err = ACSErrTypeCommon::CORBAProblemExImpl(__FILE__, __LINE__, "BaseHelper::init");
...
try
    ...
    for (CORBA::ULong n = 0; n < channelNameList->length(); n++)
		...
		{
			ncFactoryNameTmp = tempDAO->get_string((std::string("NotificationServiceMapping/Channels_/") + channelNameList[n].in() + "/NotificationService").c_str());
			break;
		}
    }
catch(...)
...
if (!ncFactoryNameTmp && domainName)
{
	try
    {
		ncFactoryNameTmp = tempDAO->get_string((std::string("NotificationServiceMapping/Domains/") + domainName + "/NotificationService").c_str());
   }
	catch(...)
    {
    ACS_STATIC_SHORT_LOG((LM_DEBUG,
...
if (!ncFactoryNameTmp)
{
	try
    {
	ncFactoryNameTmp = tempDAO->get_string("NotificationServiceMapping/DefaultNotificationService");
    }
catch(...)
    {
    ACS_STATIC_SHORT_LOG((LM_DEBUG,
