...
public class TestAdminUser {
  ...
  public TestAdminUser() {
    try {
      ...
      admin.getSourceDefinitionHandler().createSource(new SourceDefinition("TEST", "Test source", Integer.valueOf(30000), Integer.valueOf(49821)));
      //admin.removeSource(Integer.valueOf(100));

      System.out.println("creating sources...");
      Collection sources = new ArrayList();
      for (int i=1; i<=100; i++)
      {
        sources.add(new SourceDefinition("TEST" + i, "Test source", Integer.valueOf(300000), Integer.valueOf(49821)));
      }
      ...
      {
        admin.getSourceDefinitionHandler().removeSource(Integer.valueOf(i));;
      }
      */
      /*
      ...
      for (int i=1; i<=10; i++) {
        ...
        for (int j=1; j<=100; j++) {
          alarms.add(new AlarmDefinition("ff1", "fm1", Integer.valueOf(1000 + ((i-1)*100) + j), "System name", "Identifier", "Problem description", Integer.valueOf(1), "cause", "action", "consequence", Boolean.FALSE, "http://www.cern.ch", "TEST", "936", "R", "006", null, null, Integer.valueOf(49821), "4384", Collections.singleton(new String("CERN.TEST CATEGORY ROOT.TEST CATEGORY " + i))));
        }
      ...
      */
      ...
      for (int i = 1; i <= 10; i++) {
        for (int j = 1; j <= 100; j++) {
          AlarmDefinition alarm = new AlarmDefinition("ff1", "fm1", Integer.valueOf(1000 + ((i - 1) * 100) + j));
          alarms.add(alarm);

          //admin.getAlarmDefinitionHandler().removeAlarm(new Triplet("ff1", "fm1", Integer.valueOf(1000 + ((i-1)*100) + j)));
        }
