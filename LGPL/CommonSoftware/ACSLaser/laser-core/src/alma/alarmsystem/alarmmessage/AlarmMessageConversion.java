...
public class AlarmMessageConversion {
	...
	throws ValidationException, MarshalException {
		...
		if (coreAlarm.getNodeParentIds()!=null) {
			...
			acsAlarm.removeAllNodeParentIds();
			while (iter.hasNext()) {
		...
		if (coreAlarm.getNodeChildrenIds()!=null) {
			...
			acsAlarm.removeAllNodeChildrenIds();
			while (iter.hasNext()) {
		...
		if (coreAlarm.getMultiplicityChildrenIds()!=null) {
			...
			acsAlarm.removeAllMultiplicityChildrenIds();
			while (iter.hasNext()) {
		...
		if (coreAlarm.getMultiplicityParentIds()!=null) {
			...
			acsAlarm.removeAllMultiplicityParentIds();
			while (iter.hasNext()) {
	...
	throws ValidationException, MarshalException, ParseException  {
		...
		Integer priority = Integer.valueOf(alarm.getPriority());
		...
		Boolean instant = Boolean.valueOf(alarm.getInstant());
		...
		Triplet triplet = new Triplet(
				...
				Integer.valueOf(alarm.getTriplet().getFaultCode()));
		...
		if (alarm.getLocation()!=null) {
			if (alarm.getLocation().getBuilding()!=null){
				building = new Building (
					...
					Integer.valueOf(alarm.getLocation().getBuilding().getZone()),
		...
		if (alarm.getResponsiblePerson()!=null) {
				...
				Integer.valueOf(alarm.getResponsiblePerson().getResponsibleId()),
		...
		source.setConnectionTimeout(Integer.valueOf(alarm.getSource().getConnectionTimeout()));
		...
		ResponsiblePerson responsiblePersonSrc = new ResponsiblePerson (
				Integer.valueOf(alarm.getSource().getResponsiblePerson().getResponsibleId()),
		...
		SourceStatus sourceStatus = new SourceStatus(
				Boolean.valueOf(alarm.getSource().getSourceStatus().getConnected()),
				Boolean.valueOf(alarm.getSource().getSourceStatus().getEnabled()),
				lastContact);
		...
		StatusImpl status = new StatusImpl(
				Boolean.valueOf(alarm.getStatus().getActive()),
				Boolean.valueOf(alarm.getStatus().getMasked()),
				Boolean.valueOf(alarm.getStatus().getReduced()),
				Boolean.valueOf(alarm.getStatus().getActivatedByBackup()),
			    Boolean.valueOf(alarm.getStatus().getTerminatedByBackup()),
		...
		for (int pos=0; pos<alarm.getCategoriesCount(); pos++) {
			...
			CategoryImpl catImpl = new CategoryImpl(
					Integer.valueOf(cat.getCategoryId()),
					cat.getName(),
					cat.getDescription(),
					cat.getPath(),
					cat.getIsLeaf());
			catImpl.setParentId(Integer.valueOf(cat.getParentId()));
		...
		alarmImpl.setMultiplicityThreshold(Integer.valueOf(multiplityThreshold));
		return alarmImpl;
	}
}
