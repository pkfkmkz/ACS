...
public class AlarmSystemInterfaceProxy implements AlarmSystemInterface, SynchroBufferListener, ExceptionListener {
  /** logging category
   */
  private static Logger cat = LogManager.getLogger(AlarmSystemInterfaceProxy.class.getName());
  ...
  private void publish(Collection states, boolean backup) throws Exception {
    ...
    if (backup) {
      publisher.publish(topic.toString(), alarm_message, (int)configuration.getBackupDeliveryMode(), (int)configuration.getBackupPriority(), configuration.getBackupTimeToLive());
    } else {
      publisher.publish(topic.toString(), alarm_message, (int)configuration.getChangesDeliveryMode(), (int)configuration.getChangesPriority(), configuration.getChangesTimeToLive());
    }
