...
public class FaultCode implements NameOverrideFeature, Serializable {
	...
	public FaultCode(int value, boolean instant,
		...
		e.put("priority", new ElementValue(String.valueOf(priority)));
		if (cause != null)
			e.put("cause", new ElementValue(cause));
		if (action != null)
			e.put("action", new ElementValue(action));
		if (consequence != null)
			e.put("consequence", new ElementValue(consequence));
		e.put("problem-description", new ElementValue(problemDescription));
	}
