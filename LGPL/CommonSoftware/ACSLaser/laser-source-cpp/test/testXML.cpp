...
class AcsAlarmTestCase : public CPPUNIT_NS::TestFixture
{
  ...
  protected:
	...
	private:
		...
		void verifyFaultStateXML(std::string xmlToVerify);
		void verifyPropertiesXML(std::string xmlToVerify);
		void verifyTimestampXML(std::string xmlToVerify);
		DOMDocument *parseDOM(std::string xmlToParse);

		DOMLSParser  *parser;
		acsalarm::acsDOMErrorHandler* errHandler;
 ...
 */
void AcsAlarmTestCase::verifyTimestampXML(std::string xmlToVerify)
{
	try
	{
	...
	{
		ACS_LOG(LM_ERROR, "verifyTimestampXML", (LM_ERROR,
...
void AcsAlarmTestCase::testProps()
{
	...
	// test propertiesToXML method
	acsalarm::Properties properties4;
	...
	verifyPropertiesXML(cernMsg.propertiesToXML(properties4));
}
 ...
 *  </user-properties>
 */
void AcsAlarmTestCase::verifyPropertiesXML(std::string xmlToVerify)
{
	try
	{
		DOMDocument* doc = parseDOM(xmlToVerify);
	...
	{
		ACS_LOG(LM_ERROR, "verifyPropertiesXML", (LM_ERROR,
			"***** XMLException message: ***** \n\n%s \n *****\n", acsalarm::StrX(toCatch.getMessage()).localForm()))
	}
}
...
void AcsAlarmTestCase::setUp()
{
	...
	errHandler = new acsalarm::acsDOMErrorHandler();
...
void AcsAlarmTestCase::testFaultState()
{
	...
	std::shared_ptr<acsalarm::FaultState> fltstate = ACSAlarmSystemInterfaceFactory::createFaultState(family, member, CODE_VALUE);
	...
	std::shared_ptr<acsalarm::Timestamp> tstampAutoPtr(tstampPtr);
	...
	std::shared_ptr<acsalarm::Properties> propsAutoPtr(propsPtr);
 ...
 *  </fault-state>
 */
void AcsAlarmTestCase::verifyFaultStateXML(std::string xmlData)
{
	try
	{
	...
	{
		ACS_LOG(LM_ERROR, "verifyFaultStateXML", (LM_ERROR,
			"***** XMLException message: ***** \n\n%s \n *****\n", acsalarm::StrX(toCatch.getMessage()).localForm()))
	}
}
