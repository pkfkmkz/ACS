...
class AcsAlarmTestCase : public CPPUNIT_NS::TestFixture
{
  ...
  protected:
	...
	private:
		...
		void verifyASIMessageXML(std::string, bool);
		...
		void commonTestASIMessage(std::shared_ptr<std::vector<acsalarm::FaultState> > fltstates, bool fullyPopulated);

		DOMDocument *parseDOM(std::string);
...
void AcsAlarmTestCase::commonTestASIMessage(std::shared_ptr<std::vector<acsalarm::FaultState> > statesAutoPtr, bool fullyPopulated)
{
	...
	std::shared_ptr<acsalarm::Timestamp> timestampAutoPtr(timestampPtr);
	...
	// test toXML method
	laserSource::CERNASIMessage cernAsiMsg(asiMessage);
	verifyASIMessageXML(cernAsiMsg.toXML(), fullyPopulated);
}
...
void AcsAlarmTestCase::testASIMessageFaultStateNotPopulated()
{
	...
	std::shared_ptr<std::vector<acsalarm::FaultState> > statesAutoPtr(statesPtr);
 ...
 * </ASI-message>
*******************************************/
void AcsAlarmTestCase::verifyASIMessageXML(std::string xmlData, bool propertiesAndTimestampPopulated)
{
	try
	{
