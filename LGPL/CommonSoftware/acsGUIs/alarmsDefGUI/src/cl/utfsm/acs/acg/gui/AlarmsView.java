...
public class AlarmsView extends ViewPart implements IMyViewPart {
			...
			public void handleEvent(Event event){
				...
				switch(type){
					...
					case FAULT_CODE_DATA:{
						tooltip = _alarmManager.getFaultCode(it.getParentItem().getParentItem().getText(), Integer.valueOf(it.getText())).getProblemDescription();
						break;
					}
			...
			public void handleEvent(Event event) {
				...
				try {
					if(type == NodeType.FAULT_FAMILY)
						...
						_alarmManager.deleteFaultCode(_alarmManager.getFaultFamily(ff),_alarmManager.getFaultCode(ff,Integer.valueOf(alarm).intValue()));
		...
		_addFaultCode = new Listener() {
			public void handleEvent(Event event) {
				...
				if (returnCode == InputDialog.OK) {
					...
					newFaultCode.setValue(Integer.valueOf(dialog.getValue()).intValue());
					try {
		...
		Listener updateFaultCode = new Listener() {
			public void handleEvent(Event event) {
				...
				try{
					...
					if(fct.getValue() != tfc) {
						sortFaultFamilyList();
						selectElementFromTree(tff, null, Long.toString(fct.getValue()));
					}
				}catch(IllegalOperationException e){
	...
	public void sortFaultCodeList(FaultFamily family, TreeItem jTree) {
		...
		for(FaultCode fc: fcList)
			tmp.add(Long.valueOf(fc.getValue()));
		Collections.sort(tmp);
		for(Long ifc: tmp)
			for(FaultCode fc: fcList)
		...
		for (FaultCode fc: fcList) {
			TreeItem kTree = new TreeItem(jTree, SWT.NONE);
			kTree.setText(Long.toString(fc.getValue()));
			kTree.setData(NodeType.FAULT_CODE_DATA);
		}
		if(fcList.isEmpty()) {
			iTree.setForeground(new Color(iTree.getDisplay(), 255, 0, 0));
