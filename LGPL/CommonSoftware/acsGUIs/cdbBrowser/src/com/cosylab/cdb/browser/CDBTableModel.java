...
class CDBTableModel extends AbstractTableModel
{
	...
	if(resetValue.get(Integer.valueOf(row)) == null){
	    
	    //Browser.getInstance().display("storing value: " + getValueAt(row,col),true);
	    resetValue.put(Integer.valueOf(row), (String)data[row][col]);
	}
    }

    public void resetValues(){
	for(int i = 0; i < getRowCount(); i++){
 	    if(resetValue.get(Integer.valueOf(i)) != null){
 		setValueAt((String)resetValue.get(Integer.valueOf(i)),i,1);
	    }
 	}
	resetValue.clear();
    }
