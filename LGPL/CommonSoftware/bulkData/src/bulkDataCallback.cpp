...
int BulkDataCallback::handle_stop (void)
{
	...
	if ( locLoop == 0 )
			{
			ACS_SHORT_LOG((LM_ERROR,"BulkDataCallback::handle_stop timeout ( (%ld sec + %ld usec) * %u ) expired for flow: %s of receiver: %s (state: %d) - not received first frame",
	...
	if((state_m == CB_SEND_PARAM) || (state_m == CB_SEND_DATA))
	    ...
	    if ( locLoop == 0 )
		{
		ACS_SHORT_LOG((LM_ERROR,"BulkDataCallback::handle_stop timeout ( (%ld sec + %ld usec) * %u ) expired for flow: %s of receiver: %s (state: %d) - not all data received",
				waitPeriod_m.sec(), waitPeriod_m.usec(), loop_m, flowname_m.c_str(), recvName_m.c_str(), state_m));
