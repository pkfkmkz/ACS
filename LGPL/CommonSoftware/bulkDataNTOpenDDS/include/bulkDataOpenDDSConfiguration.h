...
protected:
	...
	/**
	 * sets DDSTransportLogVerbosity depend on the debug level
	 */
	static void setDDSTransportLogVerbosity();
  ...
  static unsigned int DDSTransportLogVerbosity;

	static const char* const DEFAULT_QoS_FILE;

        static const char* const DEFAULT_INI_FILE;

	static std::string urlProfileQoS;

        static std::string urlINIConfig;
	...
	void fillUrlProfileQoS(const char*suffix=NULL);
        ...
        void fillUrlINIConfig(const char* suffix=NULL);
	/// search for QoS library file with suffix in path
	bool findProfileQoS(const char* path, const char*suffix=NULL);

  /// Parser for OpenDDS QoS configuration
  static OpenDDS::DCPS::QOS_XML_Handler profileList;
