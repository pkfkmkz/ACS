...
*
* "@(#) $Id: bulkDataNTConfigurationParser.h,v 1.15 2013/02/06 13:59:09 bjeram Exp $"
*
* who       when      what
* --------  --------  ----------------------------------------------
...
#include "bulkDataOpenDDSSenderStream.h"
#include "bulkDataOpenDDSReceiverStream.h"

#include <dds/DCPS/QOS_XML_Handler/XML_Intf.h>
...
namespace AcsBulkdata
{
	...
	private:
		...
		std::string                m_baseLibrary; /// used for the Qos library name of generated XML
		xercesc::DOMLSSerializer   *m_writer;
		xercesc::DOMLSOutput       *m_output;
		...
		void parseConfig(std::string config, const struct ParsingInfo &parsingInfo);
		...
		std::string getQosProfile(std::string profileName, std::string baseProfile, xercesc::DOMNode *node);
    ...
    // OpenDDS
    void parseOpenDDSQosProfile(DDSConfiguration * flowconfig, string profileName);
	...
	public:
		 ...
		 */
		BulkDataConfigurationParser(std::string baseLibraryName);
		 ...
		 */
		void parseSenderConfig(std::string configXML);
		/**
		 * Given an XML document, parses it an retrieves the list of receiver streams
		 * that can be found. Each stream, and its corresponding flows, are properly
		 * configured depending on the QoS settings coming from the XML document
		 */
		void parseReceiverConfig(std::string configXML);
		 ...
		 */
		std::set<std::string> getAllSenderStreamNames();

		/**
		 * Returns a set of strings containing all Receiver Stream names found in
		 * the parsed XML document. If none was found, an empty set is returned
		 */
		std::set<std::string> getAllReceiverStreamNames();
		 ...
		 */
		std::set<std::string> getAllSenderFlowNames(std::string streamName);
		 ...
		 * empty set is returned
		 */
		std::set<std::string> getAllReceiverFlowNames(std::string streamName);
		 ...
		 */
		SenderStreamConfiguration * getSenderStreamConfiguration(std::string streamName);
		 ...
		 */
		SenderFlowConfiguration * getSenderFlowConfiguration(std::string streamName, std::string flowName);
		 ...
		 */
		ReceiverStreamConfiguration * getReceiverStreamConfiguration(std::string streamName);
		 ...
		 */
		ReceiverFlowConfiguration * getReceiverFlowConfiguration(std::string streamName, std::string flowName);
