...
#include "bulkDataOpenDDSReceiverImpl.h"
...
void BulkDataNTReceiverImpl<TCallback>::openReceiver()
{
	...
	try {
		...
		std::set<std::string> streamNames = parser_m->getAllReceiverStreamNames();
		...
		for(it = streamNames.begin(); it != streamNames.end(); it++) {
			...
			ACS_SHORT_LOG((LM_INFO,"BulkDataNTReceiverImpl<>::openReceiver Opening receiver stream '%s' with configuration from CDB", (*it).c_str()));
			AcsBulkdata::BulkDataNTReceiverStream<TCallback>* stream = createReceiverStream((*it).c_str(), (*it).c_str());
		...
		err.log(LM_DEBUG);
		throw err.getAVOpenReceiverErrorEx();
	} catch(ACS_BD_Errors::FlowCreateProblemExImpl &ex) {
		ACSBulkDataError::AVOpenReceiverErrorExImpl err = ACSBulkDataError::AVOpenReceiverErrorExImpl(ex,__FILE__,__LINE__,__PRETTY_FUNCTION__);
		err.log(LM_DEBUG);
		throw err.getAVOpenReceiverErrorEx();
	} catch(...) {
...
void BulkDataNTReceiverImpl<TCallback>::openReceiverStreamCfg (const char * stream_name,  const char * stream_cfg)
{
	...
	std::map<std::string, AcsBulkdata::BulkDataConfigurationParser::ReceiverCfg>::iterator it;
	...
	try {
		...
		for(std::set<std::string>::iterator it = streamNames.begin(); it != streamNames.end(); it++)
			if( ACE_OS::strcmp(stream_name, (*it).c_str()) == 0 ) {
				isConfigured = true;
				break;
			}
...
AcsBulkdata::BulkDataNTReceiverStream<TCallback>* BulkDataNTReceiverImpl<TCallback>::createReceiverStream(const char *stream_name, const char *stream_cfg) {
	...
	if( cfg == 0 )
		throw ACS_BD_Errors::StreamCreateProblemExImpl(__FILE__, __LINE__, __PRETTY_FUNCTION__);
	...
	std::set<std::string> flowNames = parser_m->getAllReceiverFlowNames(stream_cfg);
	std::set<std::string>::iterator it;
	for(it = flowNames.begin(); it != flowNames.end(); it++) {
		std::string flowName = *it;
		AcsBulkdata::ReceiverFlowConfiguration *fCfg = parser_m->getReceiverFlowConfiguration(stream_cfg, flowName);
		if( cfg == 0 )
			throw ACS_BD_Errors::FlowCreateProblemExImpl(__FILE__, __LINE__, __PRETTY_FUNCTION__);

		stream->createFlow(flowName.c_str(), *fCfg);
	}
...
AcsBulkdata::BulkDataNTReceiverStream<TCallback>* BulkDataNTReceiverImpl<TCallback>::createDefaultReceiverStream(const char *stream_cfg) {
	...
	if( stream_cfg != 0 ) {
		if( parser_m == 0 ) {
			ACS_BD_Errors::StreamCreateProblemExImpl ex = ACS_BD_Errors::StreamCreateProblemExImpl(__FILE__, __LINE__, __PRETTY_FUNCTION__);
...
AcsBulkdata::BulkDataNTReceiverStream<TCallback>* BulkDataNTReceiverImpl<TCallback>::getReceiverStream(const char *streamName)
{
	...
	ACS_BD_Errors::StreamNotExistExImpl ex(__FILE__, __LINE__, __PRETTY_FUNCTION__);
