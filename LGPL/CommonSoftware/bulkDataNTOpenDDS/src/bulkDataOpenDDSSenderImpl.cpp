...
#include "bulkDataOpenDDSSenderImpl.h"
...
void BulkDataNTSenderImpl::openSenders() {
	...
	try {
		...
		std::set<std::string> streamNames = parser_m->getAllSenderStreamNames();
		...
		for(it = streamNames.begin(); it != streamNames.end(); it++) {
			...
			ACS_SHORT_LOG((LM_INFO,"BulkDataNTSenderImpl::openSenders Opening sender stream '%s' with configuration from CDB", (*it).c_str()));
			AcsBulkdata::BulkDataNTSenderStream* stream = createSenderStream((*it).c_str());
...
AcsBulkdata::BulkDataNTSenderStream* BulkDataNTSenderImpl::createSenderStream(const char *stream_name) {
	...
	std::set<std::string> flowNames = parser_m->getAllSenderFlowNames(stream_name);
	std::set<std::string>::iterator it;
	for(it = flowNames.begin(); it != flowNames.end(); it++) {
		const char * flowName = (*it).c_str();
...
void BulkDataNTSenderImpl::disconnect()
{
	...
	try
	{
		...
		for( it = senderStreams_m.begin(); it != senderStreams_m.end(); ) {
