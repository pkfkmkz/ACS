...
*/
#include "bulkDataOpenDDSStream.h"
...
BulkDataNTStream::~BulkDataNTStream()
{
  ...
  } catch(DDSParticipantDeleteContainedEntitiesProblemExImpl &e) {
    e.log();
  }
...
void BulkDataNTStream::createDDSFactory()
{
  ...
  std::vector<std::string> params{"-DCPSConfigFile",
                                  configuration_m.urlINIConfig.c_str()};
  std::vector<ACE_TCHAR*> c_params{};
  for(auto& str : params)
    c_params.push_back(&str.front());

  int c_params_size = c_params.size();

  // Initialize DomainParticipantFactory
  factory_m = TheParticipantFactoryWithArgs(c_params_size,c_params.data());
  ...
  if(factory_m!=0)
    {
     ...
     // Clean up Transport instances and configurations initialized
     // by TheParticipantFactoryWithArgs, so it finishes threads related with
     // transport instances
     if ( factoryRefCount_m == 0)
     {
       OpenDDS::DCPS::TransportConfig_rch cfg = TheTransportRegistry->global_config();
       for (auto it=cfg->instances_.begin(); it!=cfg->instances_.end(); ++it)
       {
         TheTransportRegistry->remove_inst(*it);
       }
...
DDS::DomainParticipant* BulkDataNTStream::createDDSParticipant()
{
	...
	// I (Kris) propose that we incorporate the $ACS_INSTANCE into the domainID_m to provide consistent isolation between developers.
	char *acsInstance = std::getenv("ACS_INSTANCE");
	if (acsInstance)
	{
		domainID_m = (int)strtol(acsInstance, NULL, 10);
		ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "Found ACS_INSTANCE %s, using it as DDS domainID_m : %d",
		                                           acsInstance, domainID_m));
	}
	else
	{
		ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "No ACS_INSTANCE was found, DDS domainID_m = %d", domainID_m)); 
	}
	...
	domainParticipant =factory_m->create_participant(domainID_m, participant_qos, NULL, OpenDDS::DCPS::DEFAULT_STATUS_MASK );
	if (domainParticipant==NULL)
	{
		DDSParticipantCreateProblemExImpl ex(__FILE__, __LINE__, __PRETTY_FUNCTION__);
		ex.setDomainID(domainID_m);
		throw ex;
	}
...
void BulkDataNTStream::destroyDDSParticipant(DDS::DomainParticipant* domainParticipant)
{
	...
	ret = domainParticipant->delete_contained_entities();
	if (ret != DDS::RETCODE_OK)
	{
		DDSParticipantDeleteContainedEntitiesProblemExImpl ex(__FILE__, __LINE__, __PRETTY_FUNCTION__);
		throw ex;
	}
...
//
//  std::cout << "String profile <" << cfg.stringProfileQoS.c_str() << ">\n";
//
...
//  profLen=cfg.profileList.length(); //how many profiles do we have already ?
//
