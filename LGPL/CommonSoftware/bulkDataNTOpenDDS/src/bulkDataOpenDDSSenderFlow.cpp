...
#include "bulkDataOpenDDSSenderFlow.h"
#include <iostream>
#include <ACE.h>
...
BulkDataNTSenderFlow::BulkDataNTSenderFlow(BulkDataNTSenderStream *senderStream,
    ...
    ddsPublisher_m(0), ddsTopic_m(0), writerReaderListener_m(0), ddsDataWriter_m(0), sts_m(0), frame_m(0)


{
  AUTO_TRACE(__PRETTY_FUNCTION__);
  ACS_SHORT_LOG((LM_INFO, "-->BulkDataNTSenderFlow<--"));
  ...
  frame_m = new ACSBulkData::BulkDataNTFrame;
  handle_m = ddsDataWriter_m->register_instance(frame_m.in());

  setACKsTimeout(senderFlowCfg_m.getACKsTimeout());
  setThrottling(senderFlowCfg_m.getThrottling());

  //XXX 
  errorStatusConsumer_p = nullptr;
...
BulkDataNTSenderFlow::~BulkDataNTSenderFlow()
{
	...
	ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_INFO, "Sender Flow: %s @ stream: %s has been destroyed.", flowName_m.c_str(), streamName.c_str()));
        ...
        if (errorStatusConsumer_p != nullptr){
...
unsigned int BulkDataNTSenderFlow::getNumberOfReceivers()
{
  ...
  DDS::StatusCondition_var condition = ddsDataWriter_m->get_statuscondition();
  condition->set_enabled_statuses(DDS::PUBLICATION_MATCHED_STATUS);

  DDS::PublicationMatchedStatus pubmatched;
  if (ddsDataWriter_m->get_publication_matched_status(pubmatched)
      != DDS::RETCODE_OK)
  {
	  ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_INFO, "Error calling get_publication_matched_status()"))
      return 0;
  }

  return pubmatched.current_count;

}//getNumberOfReceivers

void BulkDataNTSenderFlow::setACKsTimeout(double ACKsTimeout)
{
	CORBA::Long ackTOSec = static_cast<CORBA::Long>(ACKsTimeout);
	CORBA::Long ackTONanosec = 1000000 * static_cast<CORBA::Long>(ACKsTimeout - ackTOSec);
...
void BulkDataNTSenderFlow::startSend(ACE_Message_Block *param, bool resetReceiver)
{
	startSend((unsigned char*)(param->rd_ptr()), param->length(), resetReceiver);
}

void BulkDataNTSenderFlow::startSend(const unsigned char *param, size_t len, bool resetReceiver)
{
	// Clean statistictics
	//getStatistics(false);
	//getStatistics(true);
...
//	frame_m->data.length(len);
	...
	if (param!=0 && len!=0)
		frame_m->data = ACSBulkData::DataSeq(len, const_cast<CORBA::Octet*>(param), false);

	ret = ddsDataWriter_m->write(*frame_m, DDS::HANDLE_NIL);
	if (param!=0 && len!=0)
		frame_m->data.release();
	...
	if (waitForACKs || restFrameCount==0)
	{
		...
		{
			// the message can cause performance penalty for small data sizes
      DDS::OfferedDeadlineMissedStatus offDeadline;
      ddsDataWriter_m->get_offered_deadline_missed_status(offDeadline);
			ACS_SHORT_LOG((LM_DEBUG, "deadline offered by the data writer has been missed #: %d times for flow %s (type: %s)", offDeadline.total_count, flowName_m.c_str(), dataType2String[dataType])); //OpenDDS
		}
...
void BulkDataNTSenderFlow::getStatistics(bool log)
{
  ...
  if (log)
  {
    DDS::LivelinessLostStatus livenessLost;
    DDS::OfferedDeadlineMissedStatus offeredDeadlineMissed;
    DDS::OfferedIncompatibleQosStatus offeredIncompatibleQos;
  
    ddsDataWriter_m->get_liveliness_lost_status(livenessLost);
    ddsDataWriter_m->get_offered_deadline_missed_status(offeredDeadlineMissed);
    ddsDataWriter_m->get_offered_incompatible_qos_status(offeredIncompatibleQos);
  
    std::stringstream msg;
    msg << "DataWriter status for flow: " << flowName_m.c_str() << std::endl
        << "Offered deadline missed: " << offeredDeadlineMissed.total_count << " last change: " << offeredDeadlineMissed.total_count_change << std::endl
        << "Offered incompatible Qos: " << offeredIncompatibleQos.total_count << " last change: " << offeredIncompatibleQos.total_count_change << std::endl
        << "Liveness lost: " << livenessLost.total_count << " last change: " << livenessLost.total_count_change << std::endl;
  
    LOG_TO_DEVELOPER(LM_DEBUG,msg.str().c_str());
  }

//	DDS::DataWriterProtocolStatus dwps;  //RTI
//	DDS::DataWriterCacheStatus dwcs;     //RTI

	// This must be read even without logging as it performs a kind of "clean"

//	ddsDataWriter_m->get_datawriter_protocol_status(dwps);   //RTI
//	if (log) {
...
//
//		if(flowMethod == 0) {
//			ddsDataWriter_m->get_datawriter_protocol_status(this->delayedStatistics.startSendDwps);   //RTI
//			ddsDataWriter_m->get_datawriter_cache_status(this->delayedStatistics.startSendDwcs);      //RTI
//		}
