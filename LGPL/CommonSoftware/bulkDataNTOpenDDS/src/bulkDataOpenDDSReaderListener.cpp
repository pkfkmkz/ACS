...
#include "bulkDataOpenDDSReaderListener.h"
...
void BulkDataNTReaderListener::on_data_available(DDS::DataReader* reader) {
    ...
    frameDataReader_mp = ACSBulkData::BulkDataNTFrameDataReader::_narrow(reader);
    ...
    retCode = frameDataReader_mp->take(message, si, DDS::LENGTH_UNLIMITED, DDS::NOT_READ_SAMPLE_STATE, DDS::ANY_VIEW_STATE, DDS::ANY_INSTANCE_STATE);
            ...
            if (si[i].valid_data == true) {
                ...
                case ACSBulkData::BD_PARAM: {
                    ...
                    if (currentState_m==StartState || currentState_m==StopState || currentState_m==IgnoreDataState) {
                        ...
                        memcpy(tmpArray.get(), message[i].data.get_buffer(), message[i].data.length());
                        if (processingQueue) {
