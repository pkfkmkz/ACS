...
#include "bulkDataOpenDDSCallback.h"
#include <iostream>
#include <iosfwd>
...
public:
    ...
    int cbStart(unsigned char* userParam_p, unsigned  int size) {
        ...
        if( errorFlag ) {
            ...
            err_sts.flow =  getFlowName();
            nc::SimpleSupplier* errorStatusSupplier_p = getReceiverFlow()->getErrorStatusSupplier();
    ...
    void enableErrorFlag() {
        errorFlag = true;
    }

    bool isErrorFlagEnabled() {
        return errorFlag;
    }
    static long cbDealy;
...
void print_usage(char *argv[]) {
    cout << "Usage: " << argv[0] << " [-s streamName] -f flow1Name[,flow2Name,flow3Name...] [-d cbReceive delay(sleep) in usec] [-u[unicast port] unicast mode] [-m multicast address] [-n suppers printing in cbReceive] [-w output filename prefix] [-e force send error flag] [-t active for (t) seconds instead of waiting for key to be pressed]" << endl;
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    SimpleClient *client;
    {
        ...
        // Parse the args
        ACE_Get_Opt get_opts (argc, argv, "s:f:d:m:u::nw:et:g");
        ...
        while(( c = get_opts()) != -1 ) {
            switch(c) {
                ...
                {
                    setErrorFlag = true;
                    std::cout << "Error flag set..." << std::endl;
                    break;
                }
                case 't':
                {
                    wait_seconds = atoi(get_opts.opt_arg());
                    std::cout << "Receiver will be running for "<< wait_seconds << " seconds before shutting down." << std::endl;
                    break;
                }
                case 'g':
                {
        ...
        vector<AcsBulkdata::BulkDataNTReceiverFlow*> flows;
        ...
        */
        ...
        for(it = flowNames.begin(); it != flowNames.end(); it++) {
            ...
            */
            ...
            if (setErrorFlag) cb -> enableErrorFlag();
            cb->setStoreData(outputFilenamePrefix != 0);
            AcsBulkdata::BulkDataNTReceiverFlow *flow = receiverStream.createFlow((*it), flowCfg, cb);
