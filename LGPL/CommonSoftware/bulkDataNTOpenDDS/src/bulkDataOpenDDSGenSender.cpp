...
int main(int argc, char *argv[])
{
    ...
    // Parse the args
    ACE_Get_Opt get_opts (argc, argv, "o:f:s:b:p:l:t:a:ncr:L:g");
                ...
                for(unsigned int n=1; n<=nbSeq; n++) {
                    ...
                    for(unsigned int i=0; i<numOfCreatedFlows; i++) {
                        ...
                        if(cdpProtocolCompatible) {
                            ...
                            // TODO: BulkData C++ implementation assumes that the size of unsigned int is 4 bytes and little endian. The size and endian may vary depending on the platform. Use fixed-size type and serialize 4-bytes integer in a proper way to keep portability.
                    ...
                    for(unsigned int j=1; j<=loop; j++) {
                        ...
                        for(unsigned int i=0; i<numOfCreatedFlows; i++) {
                            ...
                            ACS_SHORT_LOG((LM_INFO, "Outer Loop [%d/%d] Inner loop [%d/%d] Transfer rate for flow '%s': %f MBytes/sec", n,nbSeq,j,loop,tmpFlowNames[i].c_str(), throuhgput));
                    ...
                    // and stopSend
                    for(unsigned int i=0; i<numOfCreatedFlows; i++) {
                        ACS_SHORT_LOG((LM_INFO, "Outer loop [%d/%d]: Average transfer rate for flow '%s': %f MBytes/sec", n,nbSeq,tmpFlowNames[i].c_str(), throughputSums[i]/loop));
                    ...
                    ACS_SHORT_LOG((LM_INFO, "Outer loop [%d/%d]: Average transfer rate for all the flow(s): %f MBytes/sec", n,nbSeq,sumThrouhgput/(loop*numOfCreatedFlows)));
                } // for(unsigned int n=0; n<nbSeq; n++)
                ACS_SHORT_LOG((LM_INFO, "Average transfer rate for all the flow(s): %f MBytes/sec", sumTotalThroughput/(nbSeq*loop*numOfCreatedFlows)));
