...
#include "bulkDataOpenDDSConfigurationParser.h"
...
BulkDataConfigurationParser::BulkDataConfigurationParser(std::string baseLibraryName) :
   m_baseLibrary(baseLibraryName),
   m_writer(0),
   m_parser(0),
...
void BulkDataConfigurationParser::parseSenderConfig(std::string config) {
	cleanSenderConfigs();
	parseConfig(config, SENDER_PARSING_INFO);
}

void BulkDataConfigurationParser::parseReceiverConfig(std::string config) {
	cleanReceiverConfigs();
...
void BulkDataConfigurationParser::parseConfig(std::string config, const struct ParsingInfo &parsingInfo)
{

	// Parse the XML string and get the DOM Document
	m_parser->reset();
	try {
		MemBufInputSource is((const XMLByte *)config.c_str(), config.length(), "id");
		m_parser->parse(is);
	} catch(const XMLException& toCatch) {
	...
	for(unsigned int i=0; i!=streamNodesList->getLength(); i++) {
		...
		for(unsigned int j=0; j!= streamChildrenNodesList->getLength(); j++) {
			...
			if( ACE_OS::strcmp(childNodeName.c_str(), parsingInfo.reqStreamQoSNodeName) == 0 ) {
				...
				std::string baseQoSProfile = parsingInfo.defaultStreamProfileName;
				try {
					m_baseQoSlibrary = getStringFromAttribute(streamChildNode,"baseQosLibrary", DEFAULT_QOS_LIBRARY_NAME);
					baseQoSProfile = getStringFromAttribute(streamChildNode,"baseQosProfile", parsingInfo.defaultStreamProfileName);
				} catch(CDBProblemExImpl &ex) {
					cleanConfig(&receiverCfg, &senderCfg, parsingInfo.type);
					throw ex;
				}
				...
				string profileQoS = getQosProfile(streamName.get(), baseQoSProfile, streamChildNode);
				if( parsingInfo.type == SENDER ) {
					senderCfg.streamCfg->libraryQos       = (m_baseLibrary != "" ? m_baseLibrary : DYNAMIC_LIBRARY_NAME);
					senderCfg.streamCfg->profileQos       = streamName.get();
					senderCfg.streamCfg->stringProfileQoS = profileQoS;
          // Parse OpenDDS QoS Configuration
          //senderCfg.streamCfg->parseOpenDDSQoS();
				}
				else {
					receiverCfg.streamCfg->libraryQos       = (m_baseLibrary != "" ? m_baseLibrary : DYNAMIC_LIBRARY_NAME);
					receiverCfg.streamCfg->profileQos       = streamName.get();
					receiverCfg.streamCfg->stringProfileQoS = profileQoS;
          // Parse OpenDDS QoS Configuration
          //receiverCfg.streamCfg->parseOpenDDSQoS();
				}
			...
			// Process the Flow nodes
				...
				for(unsigned int k=0; k!= flowNodesList->getLength(); k++) {
					...
					if( ACE_OS::strcmp(childFlowNodeName.c_str(), parsingInfo.reqFlowQoSNodeName) != 0 ) {
						...
						stringstream s;
						s << "Node name is different from '" << parsingInfo.reqFlowQoSNodeName << "': " << childFlowNodeName;
					...
					std::string baseQoSProfile = parsingInfo.defaultFlowProfileName;
					try{
						m_baseQoSlibrary = getStringFromAttribute(childFlowNode,"baseQosLibrary", DEFAULT_QOS_LIBRARY_NAME);
						baseQoSProfile = getStringFromAttribute(childFlowNode,"baseQosProfile", parsingInfo.defaultFlowProfileName);
					} catch(CDBProblemExImpl &ex) {
						cleanConfig(&receiverCfg, &senderCfg, parsingInfo.type);
						throw ex;
					}
					...
					string profileQoS = getQosProfile(profileName, baseQoSProfile, childFlowNode);
					...
					if( parsingInfo.type == SENDER ) {
						...
						flowConfig->libraryQos = (m_baseLibrary != "" ? m_baseLibrary : DYNAMIC_LIBRARY_NAME);
						flowConfig->profileQos = profileName;
						flowConfig->stringProfileQoS = profileQoS;

            // OpenDDS qos profiles
            parseOpenDDSQosProfile(flowConfig,profileName);
...
void BulkDataConfigurationParser::parseOpenDDSQosProfile(DDSConfiguration * flowconfig, string profileName)
{

  if (flowconfig == 0)
    return;

  //XML_MemBuf xml_membuf;
  OpenDDS::DCPS::QOS_XML_String_Handler xml_membuf;

  std::stringstream ss;

  // Add search path for schema validation
  xml_membuf.add_search_path(ACE_TEXT("MODROOT"), ACE_TEXT("/config/CDB/schemas/"));
  //xml_membuf.add_search_path(ACE_TEXT("DDS_ROOT"), ACE_TEXT("/docs/schema/"));
  xml_membuf.add_search_path(ACE_TEXT("ACSROOT"), ACE_TEXT("/config/CDB/schemas/"));
  xml_membuf.add_search_path(ACE_TEXT("INTROOT"), ACE_TEXT("/config/CDB/schemas/"));
 
  // Add tags "dds" for OpenDDS schema validation 
  ss << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
     << "<dds xmlns=\"http://www.omg.org/dds\" " 
     << "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" "
     << "xsi:schemaLocation=\"http://www.omg.org/dds dds_qos.xsd\""
     << ">\n"
     << flowconfig->stringProfileQoS
     << "\n</dds>";

  std::string str = ss.str().c_str();

  // initialize and parse XML
  DDS::ReturnCode_t const retcode = xml_membuf.init(ACE_TEXT_CHAR_TO_TCHAR(str.c_str()));
  if (retcode != DDS::RETCODE_OK)
  {
			ACS_SHORT_LOG((LM_INFO, "Cannot parse OpenDDS QoS Xml string."));
  }

  dds::qosProfile qos = xml_membuf.getProfile(ACE_TEXT_CHAR_TO_TCHAR(profileName.c_str()));

  flowconfig->profileList.addQoSProfile(qos);
...
double BulkDataConfigurationParser::getDoubleFromAttribute(DOMNode *node, const char * attribute, double defaultVal) {
	...
	if( sft == 0 )
		...
		std::unique_ptr<XSValue> val(XSValue::getActualValue(sft, XSValue::dt_double, status));
...
unsigned short BulkDataConfigurationParser::getUnsignedShortFromAttribute(DOMNode *node, const char * attribute, unsigned short defaultVal) {
	...
	if( sft == 0 )
		...
		XSValue::Status status;
		std::unique_ptr<XSValue> val(XSValue::getActualValue(sft, XSValue::dt_unsignedShort, status));
...
bool BulkDataConfigurationParser::getBooleanFromAttribute(DOMNode *node, const char * attribute, bool defaultVal) {
	...
	if( sft == 0 )
		...
		XSValue::Status status;
		std::unique_ptr<XSValue> val(XSValue::getActualValue(sft, XSValue::dt_boolean, status));
...
string BulkDataConfigurationParser::getQosProfile(std::string profileName, std::string baseProfile, DOMNode *node) {
	...
	for(unsigned int i=0; i!= children->getLength(); i++) {
		...
		if( child->getNodeType() == DOMNode::ELEMENT_NODE ) {
			...
			m_output->setByteStream(formatTarget);
			m_writer->write(child, m_output);
...
std::set<std::string> BulkDataConfigurationParser::getAllSenderStreamNames() {
...
std::set<std::string> BulkDataConfigurationParser::getAllReceiverStreamNames() {
	std::set<std::string> names;
...
std::set<std::string> BulkDataConfigurationParser::getAllSenderFlowNames(std::string streamName) {
	std::set<std::string> names;
	SenderCfgMap::iterator it;
	for(it = senderConfigMap_m.begin(); it != senderConfigMap_m.end(); it++) {
...
std::set<std::string> BulkDataConfigurationParser::getAllReceiverFlowNames(std::string streamName) {
	std::set<std::string> names;
	ReceiverCfgMap::iterator it;
	for(it = receiverConfigMap_m.begin(); it != receiverConfigMap_m.end(); it++) {
		if (it->first == streamName) {
...
SenderStreamConfiguration * BulkDataConfigurationParser::getSenderStreamConfiguration(std::string streamName) {
	SenderCfgMap::iterator it;
	for(it = senderConfigMap_m.begin(); it != senderConfigMap_m.end(); it++)
		if (it->first == streamName)
...
SenderFlowConfiguration * BulkDataConfigurationParser::getSenderFlowConfiguration(std::string streamName, std::string flowName) {
	SenderCfgMap::iterator it;
	for(it = senderConfigMap_m.begin(); it != senderConfigMap_m.end(); it++) {
		if (it->first == streamName) {
			std::map<std::string, SenderFlowConfiguration *> flows = it->second.flowsCfgMap;
...
ReceiverStreamConfiguration * BulkDataConfigurationParser::getReceiverStreamConfiguration(std::string streamName) {
	ReceiverCfgMap::iterator it;
	for(it = receiverConfigMap_m.begin(); it != receiverConfigMap_m.end(); it++)
		if (it->first == streamName) {
...
ReceiverFlowConfiguration * BulkDataConfigurationParser::getReceiverFlowConfiguration(std::string streamName, std::string flowName) {
	ReceiverCfgMap::iterator it;
	for(it = receiverConfigMap_m.begin(); it != receiverConfigMap_m.end(); it++) {
		if (it->first == streamName) {
			std::map<std::string, ReceiverFlowConfiguration *> flows = it->second.flowsCfgMap;
