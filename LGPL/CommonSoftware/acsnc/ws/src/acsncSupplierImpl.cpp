...
Supplier::publishEvent(const CosNotification::StructuredEvent &event)
{
    ...
    try
    {
        while((tmp = eventBuff.front()) != NULL){
            //Adding stats for ICT-12884
            t0_m = duration_cast< milliseconds >(system_clock::now().time_since_epoch());
         ...
         */
        t1_m = duration_cast< milliseconds >(system_clock::now().time_since_epoch()) - t0_m;
        ACS_SHORT_LOG((LM_DEBUG,"CORBA::TIMEOUT caught on channel %s , time taken: %s milliseconds", channelName_mp, std::to_string(t1_m.count()).c_str() ));
         ...
         */
        t1_m = duration_cast< milliseconds >(system_clock::now().time_since_epoch()) - t0_m;
        ACS_SHORT_LOG((LM_DEBUG,"CORBA::OBJECT_NOT_EXIST caught on channel: %s. Time taken: %s milliseconds ", channelName_mp, std::to_string(t1_m.count()).c_str() ));
    ...
    catch(CORBA::TRANSIENT &ex)
    {
        ...
        */
        ACS_SHORT_LOG((LM_DEBUG,"CORBA::TRANSIENT exception caught on channel: %s", channelName_mp));
    ...
    {
        ACS_SHORT_LOG((LM_DEBUG,"CosEventComm::Disconnected caught on channel: %s. Propagating exception.", channelName_mp));

        ACSErrTypeCommon::CORBAProblemExImpl cex(__FILE__, __LINE__, "nc::SimpleSupplier::publishEvent");
        cex.setInfo(ex._info().c_str());

        acsncErrType::PublishEventFailureExImpl pex(cex, __FILE__, __LINE__, "nc::Supplier::publishEvent");
        pex.setChannelName(channelName_mp);
        pex.log(LM_DEBUG);
    ...
    {
        ACS_SHORT_LOG((LM_DEBUG,"CORBA::SystemException caught on channel: %s.", channelName_mp));
    ...
    {
        ACS_SHORT_LOG((LM_DEBUG,"std::exception caught on channel: %s. Propagating exception", channelName_mp));

        ACSErrTypeCommon::StdExceptionExImpl stdex(__FILE__, __LINE__, "nc::SimpleSupplier::publishEvent");
        stdex.setWhat(ex.what());

        acsncErrType::PublishEventFailureExImpl pex(stdex, __FILE__, __LINE__, "nc::Supplier::publishEvent");
        pex.setChannelName(channelName_mp);
        pex.log(LM_DEBUG);
        throw pex;
    }
    ...
    {
        ACS_SHORT_LOG((LM_DEBUG,"Unexpected exception caught on channel: %s. Propagating exception", channelName_mp));
...
void Supplier::retrySendEvent(const CosNotification::StructuredEvent &event){
    ACS_SHORT_LOG((LM_DEBUG,"retrySendEvent invoked to re-initialize notification channel connection"));
    ...
    while(maxAttemps--)
    {
        try 
        {
            ...
            reinit();
            ACS_SHORT_LOG((LM_DEBUG, "Supplier reconnected to Notification Channel sucessfully"));
        } 
        catch(...) 
        {
            // Couldn't reconnect to the channel. The event will be appended
            ACS_SHORT_LOG((LM_ERROR,"Supplier couldn't reconnect to Notification Channel."));
    ...
    try 
    { 
        // ICT-3208: removed 12 seconds sleep time
        // Wait some time to allow consumers to reconnect
        // sleep(SLEEP_TIME_BEFORE_SENDING_BUFFERED_EVENTS);
        //
        ACS_SHORT_LOG((LM_DEBUG, "Sending buffered events before sending current event."));        
        ...
        ACS_SHORT_LOG((LM_DEBUG, "Buffered events sent successfully."));
        ...
        ACS_SHORT_LOG((LM_DEBUG,"Exception caught while publishing buffered event. will try to publish it in subsequent publication"));
