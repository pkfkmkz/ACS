...
Consumer::reinit()
{
    ...
    ACE_Write_Guard<ACE_RW_Thread_Mutex> guard(reinitThMutex_m);
    ...
    try {
        ...
        std::vector<CosNotifyFilter::Filter_var> filters;
        if(reference_m.in()!=0) {
            ...
            try {
                ...
                ACS_SHORT_LOG((LM_INFO,"Consumer::reinit failed to remove subscriptions for the '%s' channel!", channelName_mp));
            }
            ...
            try {
                if(CORBA::is_nil(proxySupplier.in()) == false) {
                    CosNotifyFilter::FilterIDSeq_var fids = proxySupplier->get_all_filters();
                    for (unsigned int i = 0; i < fids.in().length(); i++)
                        filters.push_back(proxySupplier->get_filter(fids.in()[i]));
                    proxySupplier->remove_all_filters();
                    proxySupplier->disconnect_structured_push_supplier();
                }
            } catch(...) {
                ACS_SHORT_LOG((LM_ERROR,"Consumer::reinit failed to disconnect supplier for the '%s' channel!", channelName_mp));
            }
            ...
            try {
                ...
                ACS_SHORT_LOG((LM_ERROR,"Consumer::reinit failed to destroy consumer admin for the '%s' channel!", channelName_mp));
            }
            ...
            try {
                ...
                ACS_SHORT_LOG((LM_ERROR,"Consumer::reinit failed to destroy the reference for the '%s' channel!", channelName_mp));
            }
        }
        ...
        for (std::vector<CosNotifyFilter::Filter_var>::size_type i = 0; i != filters.size(); i++)
            proxySupplier_m->add_filter(filters[i].in());
        for (std::vector<std::string>::size_type i = 0; i != subscriptions_m.size(); i++)
            addSubscription(subscriptions_m[i].c_str());
        consumerReady();
...
Consumer::shouldReconnect()
{
    ...
    if(::CORBA::is_nil(proxySupplier_m) || !autoreconnect_m)
    {
        return false;
    }
    ...
    if(getChannelTimestamp(channelTimestamp))
    {
...
Consumer::ncChecker(void* arg)
{
    ...
    try {
        ...
        Logging::AcsLogService_ptr remote_logger = Logging::AcsLogService::_narrow(obj);
        logger->setCentralizedLogger(remote_logger);
    } catch (...) {
        ACS_SHORT_LOG((LM_WARNING, "Consumer::ncChecker(): Failed to retrieve Centralized logger. Will only use local logger."));
    }
...
Consumer::checkNotifyChannel()
{
    ...
    while(!stopNCCheckerThread)
    {
        ...
        if(!eventsReceived)
        {
            ...
            //Checking state of internal elements. ICT-9678, ignoring the NS reconnection code entirely
            if (::CORBA::is_nil(notifyChannel_m)) {
                ACS_SHORT_LOG((LM_WARNING, "EventChannel %s reference is NULL.", channelName_mp));
                reinitNC = true;
            }
            if (::CORBA::is_nil(consumerAdmin_m)) {
                ACS_SHORT_LOG((LM_WARNING, "ConsumerAdmin %d on EventChannel %s is NULL.", adminid, channelName_mp));
                reinitNC = true;
            }
            if(::CORBA::is_nil(proxySupplier_m)) {
                ACS_SHORT_LOG((LM_WARNING, "ProxyPushSupplier %d with ConsumerAdmin %d on EventChannel %s is NULL.", proxySupplierID, adminid, channelName_mp));
                reinitNC = true;
            }
            ...
            try {
                if (!::CORBA::is_nil(proxySupplier_m)) {
                    msg = std::string("ProxyPushSupplier ") + std::to_string(proxySupplierID) + std::string(" threw an exception retrieving 'MyAdmin' attribute.");
                    CosNotifyChannelAdmin::ConsumerAdmin_var ca = proxySupplier_m->MyAdmin();
                }
                if (!::CORBA::is_nil(consumerAdmin_m)) {
                    msg = std::string("ConsumerAdmin ") + std::to_string(adminid) + std::string(" threw an exception calling 'get_proxy_supplier' method.");
                    CosNotifyChannelAdmin::ProxySupplier_var ps = consumerAdmin_m->get_proxy_supplier(proxySupplierID);
                    msg = std::string("ConsumerAdmin ") + std::to_string(adminid) + std::string(" threw an exception retrieving 'MyChannel' attribute.");
                    CosNotifyChannelAdmin::EventChannel_var ec = consumerAdmin_m->MyChannel();
                }
                if (!::CORBA::is_nil(notifyChannel_m)) {
                    msg = std::string("EventChannel ") + std::string(channelName_mp) + std::string(" threw an exception calling 'get_consumeradmin' method.");
                    CosNotifyChannelAdmin::ConsumerAdmin_var ca = notifyChannel_m->get_consumeradmin(adminid);
                }
            } catch(...) {
                ACS_SHORT_LOG((LM_WARNING, "%s", msg.c_str()));
                reinitNC = true;
            }
            if (reinitNC) {
                if(!reinitFailed) {
 ...
 */ //Reverted due to ICT-7771 and new comments in ICT-599
	ACS_TRACE("Consumer::addSubscription");
     ACE_Read_Guard<ACE_RW_Thread_Mutex> guard(reinitThMutex_m);
    ...
    if (std::find(subscriptions_m.begin(), subscriptions_m.end(), std::string(type_name)) == subscriptions_m.end())
        subscriptions_m.push_back(std::string(type_name));
}//addSubscription
//-----------------------------------------------------------------------------
 ...
 */ //Reverted due to ICT-7771 and new comments in ICT-599
    ...
    std::vector<std::string>::iterator el = std::find(subscriptions_m.begin(), subscriptions_m.end(), std::string(type_name));
    if (el != subscriptions_m.end())
        subscriptions_m.erase(el);
}//removeSubscription
//-----------------------------------------------------------------------------
