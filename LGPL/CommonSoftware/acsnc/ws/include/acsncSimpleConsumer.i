...
void SimpleConsumer<T>::push_structured_event(
    ...
    ACE_Read_Guard<ACE_RW_Thread_Mutex> guard(reinitThMutex_m, false);
    if (!guard.locked()) {
        ACS_SHORT_LOG((LM_WARNING, "There's a reinitialization on-going. Ignoring event."));
        return;
    }
    ...
    if (customIDLStruct == NULL) {
        CORBA::Any tmpEvent;
        T customTmp;
        tmpEvent <<= customTmp;
        acsncErrType::WrongEventReceivedExImpl er(__FILE__, __LINE__, "nc::SimpleConsumer<T>::push_structured_event");
        ...
        er.log();
        ACS_SHORT_LOG((LM_WARNING, "Any type could not be mapped to IDL Struct. Ignoring event to prevent a segmentation fault."));
        return;
    }
