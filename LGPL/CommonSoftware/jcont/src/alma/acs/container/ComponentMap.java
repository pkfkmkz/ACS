...
class ComponentMap
{
	...
	synchronized boolean reserveComponent(int compHandle) {
		boolean ret = false;
		Integer handleObj = Integer.valueOf(compHandle);
	...
	synchronized void sort(int[] sortedHandles) {
		...
		try {
			for (int i = 0; i < sortedHandles.length; i++) {
				...
				if (compAdapter != null) {
					Integer keyObj = Integer.valueOf(sortedHandles[i]);
	...
	{
		return m_map.remove(Integer.valueOf(compHandle));
	}
	...
	{
		return m_map.get(Integer.valueOf(compHandle));
	}
