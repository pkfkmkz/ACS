...
//m_logger.finest(compName + " component impl created, with classloader " + compImpl.getClass().getClassLoader().getClass().getName());
            ...
            //changes for ICT-13587
            try {
                String componentsCdbPath = "MACI/Components";
                com.cosylab.CDB.DAO myDao = null;
                try {
                    myDao = sharedCdbRef.get_DAO_Servant(componentsCdbPath + "/" + compName);
                } catch (CDBRecordDoesNotExistEx e) {
                    myDao = sharedCdbRef.get_DAO_Servant(componentsCdbPath + "/*");
                    String ctype = myDao.get_string("Type");
                    int i = 1;
                    while (ctype.compareTo(type) != 0) {
                        myDao = sharedCdbRef.get_DAO_Servant(componentsCdbPath + "/*" + i);
                        ctype = myDao.get_string("Type");
                        i++;
                    }
                }
                String keepAlive = myDao.get_string("KeepAliveTime");
                m_logger.info("Component '" + compName + "' has KeepAliveTime '" + keepAlive + "'.");
            } catch (CDBFieldDoesNotExistEx e) {
                m_logger.info("Component '" + compName + "' doesn't have any KeepAliveTime configuration associated with it.");
            } catch (CDBRecordDoesNotExistEx e) {
                m_logger.info("Component '" + compName + "' doesn't have any KeepAliveTime configuration associated with it.");
            }
        }
...
//            System.out.println(msg);
	...
	public LogLevels get_logLevels(String logger_name) throws LoggerDoesNotExistEx {
		tryToWaitForContainerStart();
		if (!logConfig.isKnownLogger(logger_name))
