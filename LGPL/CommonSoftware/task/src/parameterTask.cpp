...
static const char *rcsId="@(#) $Id: parameterTask.cpp,v 1.27 2010/04/27 12:24:57 htischer Exp $"; 
...
typedef std::map<string, vector<string>>::value_type nameValType;
...
void parameterTask::run(const ACS::StringSequence & parameters, const char* filePrefix)
{
	try
	{
		...
		Parameters::ParameterSet pset(&xmlData);
...
string parameterTask::buildParameterSetXML(const string & xmlFileNamePrefix) 
{
	...
	if (impl != NULL)
	{
		try
		{
			...
			// set our internal ptr to point to the new document
			...
			std::map<string, vector<string>>::iterator position;
			...
			DOMLSSerializer *domWriter = impl->createLSSerializer();
			DOMLSOutput* domOutput = impl->createLSOutput();
			if (domWriter->getDomConfig()->canSetParameter(XMLUni::fgDOMWRTFormatPrettyPrint, true)) {
...
void parameterTask::buildParameterMap(const ACS::StringSequence & params, const string & baseFileName) 
{
	...
	for(CORBA::ULong i = 0; i < params.length(); i++) {
		...
		if(NULL != params[i]) {
			...
			ACS_DEBUG_PARAM("parameterTask::buildParameterMap", "The equals sign position is: %lu", nameEndPosition)
