...
namespace Parameters {
	...
	{    
	  public:
	     ...
	     * Constructor.
	     */
	    StringArrayParamDef(const std::string & paramName, const std::string  & helpText, const std::string & promptText, 
	       bool requiredBoolVal, std::shared_ptr<int> maxLen, std::shared_ptr<std::vector<std::string>> defaultVals);
	     ...
	     * @return the default value as an shared_ptr to a vector of string.
	     * if defined for this pdef, then the shared_ptr will 
	     * point to something meaningful, else null.
	     */
	    std::shared_ptr<std::vector<std::string>> getDefaultValues();
	  ...
	  private:
	    std::vector<std::string> defaultValues_m;
	    int maxLen_m;
