...
void ParameterSet::setParam(string paramName, IntParam value)
{
	...
	shared_ptr<IntParam> previousValue;
	if(mapEntry != intParamMap.end()) 
	{
...
{
   map<string, DoubleParam >::iterator mapEntry = doubleParamMap.find(paramName);
   shared_ptr<DoubleParam> previousValue;
   if(mapEntry != doubleParamMap.end()) 
   {
		// there was an existing entry, so save it in case we need to "roll back" due to error
...
{
   map<string, StringParam >::iterator mapEntry = stringParamMap.find(paramName);
   shared_ptr<StringParam> previousValue;
   if(mapEntry != stringParamMap.end()) 
   {
		// there was an existing entry, so save it in case we need to "roll back" due to error
...
{
   map<string, IntArrayParam >::iterator mapEntry = intArrayParamMap.find(paramName);
   shared_ptr<IntArrayParam> previousValue;
   if(mapEntry != intArrayParamMap.end()) 
   {
		// there was an existing entry, so save it in case we need to "roll back" due to error
...
{
   map<string, DoubleArrayParam >::iterator mapEntry = doubleArrayParamMap.find(paramName);
   shared_ptr<DoubleArrayParam> previousValue;
   if(mapEntry != doubleArrayParamMap.end()) 
   {
		// there was an existing entry, so save it in case we need to "roll back" due to error
...
{
	map<string, StringArrayParam >::iterator mapEntry = stringArrayParamMap.find(paramName);
	shared_ptr<StringArrayParam> previousValue;
	if(mapEntry != stringArrayParamMap.end()) 
	{
		// there was an existing entry, so save it in case we need to "roll back" due to error
...
int ParameterSet::parseDOM(const char* xmlFile, InMemoryXmlData * xmlData) 
{
	...
	parser->getDomConfig()->setParameter(XMLUni::fgXercesDOMHasPSVIInfo, true);
	...
	try {
		if(NULL != xmlFile) {
			...
			std::string data = xmlData->getContent();
			mbis = new  MemBufInputSource((const XMLByte *) data.c_str(), data.length(), xmlData->getId().c_str(), false);
			const Wrapper4InputSource wrapper4InputSource(mbis);
...
void ParameterSet::handleIntArrayParam(DOMElement *paramElem) 
{
	...
	if(NULL != nameTextNode) {
		...
		paramName = string(tmpString);
		XMLString::release(&tmpString);
		ACS_DEBUG_PARAM("ParameterSet::handleIntArrayParam", "param name: %s", paramName.c_str())
	}
	else {
...
void ParameterSet::handleDoubleParam(DOMElement *paramElem) 
{
	...
	if(NULL != nameTextNode) {
		...
		paramName = string(tmpString);
		XMLString::release(&tmpString);
		ACS_DEBUG_PARAM("ParameterSet::handleDoubleParam", "param name: %s", paramName.c_str())
	}
	else {
...
void ParameterSet::processParamNodes(DOMNodeList *paramNodes) 
{
	for (unsigned int i = 0; i < paramNodes->getLength(); i++) {
		...
		if(paramItem->getNodeType() == DOMNode::ELEMENT_NODE) {
			...
			const DOMTypeInfo * typeInfo = paramElem->getSchemaTypeInfo();
			const XMLCh* typeName = typeInfo->getTypeName();
...
void ParameterSet::validateBoolParams() 
{
	...
	shared_ptr< vector<BoolParamDef> > boolParamDefs = ParameterSetDef->getBoolParamDefs();
...
void ParameterSet::validateIntParams() 
{
	...
	shared_ptr< vector<IntParamDef> > paramDefs = ParameterSetDef->getIntParamDefs();
...
void ParameterSet::validateDoubleParams() 
{
	...
	shared_ptr< vector<DoubleParamDef> > paramDefs = ParameterSetDef->getDoubleParamDefs();
	for(unsigned int i = 0; i < paramDefs->size(); ++i)
	{
...
void ParameterSet::validateStringParams() 
{
	...
	shared_ptr< vector<StringParamDef> > stringParamDefs = ParameterSetDef->getStringParamDefs();
...
{
	// for each int parameter defined in psetdef
	shared_ptr< vector<IntArrayParamDef> > paramDefs = ParameterSetDef->getIntArrayParamDefs();
	for(unsigned int i = 0; i < paramDefs->size(); ++i)
	{
...
{
	// for each string parameter defined in psetdef
	shared_ptr< vector<StringArrayParamDef> > paramDefs = ParameterSetDef->getStringArrayParamDefs();
	for(unsigned int i = 0; i < paramDefs->size(); ++i)
	{
...
{
	// for each double parameter defined in psetdef
	shared_ptr< vector<DoubleArrayParamDef> > paramDefs = ParameterSetDef->getDoubleArrayParamDefs();
	for(unsigned int i = 0; i < paramDefs->size(); ++i)
	{
...
void ParameterSet::validateStringParam(StringParamDef pDef) 
{
	...
	// optional parameter
		...
		try {
			...
			shared_ptr<string> defaultVal = pDef.getDefault();
	...
	shared_ptr<vector<string> > validVals = pDef.getValidValues();
...
void ParameterSet::validateIntParam(IntParamDef pDef) 
{
	...
	// optional parameter
		...
		try {
			...
			// exception caught means the value wasn't defined in the pset; 
			// default it if there is a default in psetdef
			shared_ptr<int> defaultVal = pDef.getDefault();
			shared_ptr<string> defaultStringVal = pDef.getStringDefault();
			if(NULL != defaultVal.get()) {
	...
	shared_ptr<int> max = pDef.getMax();
	if(NULL != max.get())
	{
	...
	shared_ptr<int> min = pDef.getMin();
	if(NULL != min.get())
	{
	...
	// verify against valid values, if defined in psetdef
	shared_ptr<vector<int> > validVals = pDef.getValidValues();
	if(NULL != validVals.get() && pDef.getHasValidValues()) {
		IntParam param = getIntParam(pDef.getName());
		int paramVal = param.getValue();
		bool found = false;
...
void ParameterSet::validateDoubleParam(DoubleParamDef pDef) 
{
	...
	// optional parameter
		...
		try {
			...
			// exception caught means the value wasn't defined in the pset; 
			// default it if there is a default in psetdef
			shared_ptr<double> defaultVal = pDef.getDefault();
			shared_ptr<string> defaultStringVal = pDef.getStringDefault();
			if(NULL != defaultVal.get()) {
				// if parameter not specified in pset, use default from psetdef 
	...
	// verify that value in pset is below max and above min, if defined in psetdef
	shared_ptr<double> max = pDef.getMax();
	if(NULL != max.get())
	{
	...
	shared_ptr<double> min = pDef.getMin();
	if(NULL != min.get())
	{
	...
	// verify against valid values, if defined in psetdef
	shared_ptr<vector<double> > validVals = pDef.getValidValues();
	if(NULL != validVals.get() && pDef.getHasValidValues()) {
		DoubleParam param = getDoubleParam(pDef.getName());
		double paramVal = param.getValue();
		bool found = false;
		for(unsigned int j = 0; j < validVals->size(); ++j) {
			ACS_LOG(LM_DEBUG, "ParameterSet::validateDoubleParam", 
				(LM_DEBUG, "valid value[%u] is: %f and val is: %f", j, validVals->begin()[j], paramVal))
			if(false == found) {
...
void ParameterSet::validateIntArrayParam(IntArrayParamDef pDef) 
{
	...
	// optional parameter
		...
		try {
			...
			// exception caught means the value wasn't defined in the pset; 
			// default it if there is a default in psetdef
			shared_ptr< vector<int> > defaultVals = pDef.getDefaultValues();
	...
	shared_ptr<int> maxLen = pDef.getMaxLen();
	if(NULL != maxLen.get())
	{
...
void ParameterSet::validateStringArrayParam(StringArrayParamDef pDef) 
{
	...
	// optional parameter
		...
		try {
			...
			// exception caught means the value wasn't defined in the pset; 
			// default it if there is a default in psetdef
			shared_ptr< vector<string> > defaultVals = pDef.getDefaultValues();
			if(NULL != defaultVals.get() && 0 != defaultVals->size()) {
...
void ParameterSet::validateDoubleArrayParam(DoubleArrayParamDef pDef) 
{
	...
	// optional parameter
		...
		try {
			...
			// exception caught means the value wasn't defined in the pset; 
			// default it if there is a default in psetdef
			shared_ptr< vector<double> > defaultVals = pDef.getDefaultValues();
 ...
 */
void ParameterSet::setSchemaLocation(DOMLSParser * parser)
{
	...
	if(NULL != xsdDir) {
		...
		parser->getDomConfig()->setParameter(XMLUni::fgXercesSchemaExternalSchemaLocation, (void*)schemaLocationStrX.unicodeForm());
	}
