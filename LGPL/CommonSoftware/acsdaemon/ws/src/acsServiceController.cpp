        ...
        if (ACSServiceController * c = dynamic_cast<ACSServiceController*>(this)) {
        	if (c->desc->getACSService() == NOTIFICATION_SERVICE &&
        		...
        		ACS_SHORT_LOG((LM_INFO, "Getting reference of %s using address: %s",
        				...
        				addr_str.str().c_str()));
        		int argc = 1;
        		const char *argv[] = {"some_daemon"};
        		CORBA::ORB_var orb = CORBA::ORB_init (argc,const_cast<char**>(argv),"TAO");
...
bool ServiceController::start(acsdaemon::DaemonCallback_ptr callback) {
    m_mutex->acquire();
    setState(getActualState());
    active = true;
...
void ServiceController::stop(acsdaemon::DaemonCallback_ptr callback) {
    m_mutex->acquire();
    setState(getActualState());
    active = false;
...
void ACSDaemonContext::processRequest(ACSServiceRequestTarget target, ACSServiceRequestType type, ACSServiceRequestDescription *desc, acsdaemon::DaemonCallback_ptr callback) {
    switch (target) {
    case IMP:
