...
//            } catch (CORBA::TIMEOUT& timeout) {
                ...
                ACS_SHORT_LOG((LM_WARNING, "Failed to make a 'working' callback call for local request! (%s)", ex._info().c_str()));
                ex._tao_print_exception("in ACSServiceRequest::execute");
            }
        }
