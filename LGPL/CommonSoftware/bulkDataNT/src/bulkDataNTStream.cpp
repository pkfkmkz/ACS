...
BulkDataNTStream::~BulkDataNTStream()
{
  ...
  try {
    ...
    ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_WARNING, "Unexpected CORBA::Exception: %s", ex._info().c_str()));
    ex._tao_print_exception("in BulkDataNTStream::~BulkDataNTStream");
  } catch(std::exception& ex) {
    ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_WARNING, "Unexpected std::exception: %s", ex.what()));
  } catch(...) {
    ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_WARNING, "Unexpected unkown exception"));
  }
