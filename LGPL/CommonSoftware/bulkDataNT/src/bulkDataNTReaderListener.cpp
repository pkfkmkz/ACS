...
: BulkDataNTDDSLoggable("BulkDataNT:"+string(name)),
    currentState_m(StartState), topicName_m(name), dataLength_m(0), frameCounter_m(0), totalFrames_m(0),
    callback_mp (cb), enableCB_m(true), skipdata_m(skipReceivingDataAfterFailure), tm(getNamedLogger("BDNTThrMgrLogger")), processQueue(tm.create<ProcessQueue>(name)) {
...
/*    DDS::DataReaderProtocolStatus drps;
    ...
    ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "Protocol Status (TOTAL): sample rcv: %lld (%lld) HB: %lld (%lld) ACKs: %lld (%lld) NACKs: %lld (%lld) Rejected: %lld",
        drps.received_sample_count, drps.received_sample_bytes, drps.received_heartbeat_count, drps.received_heartbeat_bytes, drps.sent_ack_count, drps.sent_ack_bytes,
        drps.sent_nack_count, drps.sent_nack_bytes, drps.rejected_sample_count));
*/
...
void BulkDataNTReaderListener::on_data_available(DDS::DataReader* reader) {
    ...
    retCode = frameDataReader_mp->take(message, si, DDS_LENGTH_UNLIMITED, DDS_NOT_READ_SAMPLE_STATE, DDS_ANY_VIEW_STATE, DDS_ANY_INSTANCE_STATE);
    for (int i = 0; i < si.length(); i++) {
        if (retCode == DDS::RETCODE_OK) {
            if (si[i].valid_data == true) {
                if (DDSConfiguration::debugLevel>3) {
                    // the message can cause performance penalty for small data sizes
                    ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "Got BulkDataNTFrame (on_data_available): typeOfdata: %d, restDataLength: %d, size %d for: %s",
                        message[i].typeOfdata, message[i].restDataLength, message[i].data.length(), topicName_m.c_str()));
                }//if
                switch(message[i].typeOfdata) {
                case ACSBulkData::BD_PARAM: {
                    if (DDSConfiguration::debugLevel>0) {
                        ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "startSend has been received on: %s with paramter size: %d", topicName_m.c_str(), message[i].data.length()));
                    }
                    if (currentState_m==StartState || currentState_m==StopState || currentState_m==IgnoreDataState) {
                        ...
                        cbReceiveTotalSec_m = 0.0; cbReceiveNumCalls_m =0;
                        std::shared_ptr<unsigned char> tmpArray(new unsigned char[message[i].data.length()], std::default_delete<unsigned char[]>());
                        memcpy(tmpArray.get(), message[i].data.get_contiguous_buffer(), message[i].data.length());
                        if (processingQueue) {
                            if (enableCB_m) processQueue->cbStart(tmpArray, message[i].data.length());
                        } else {
                            if (enableCB_m) BDNT_LISTENER_USER_ERR(callback_mp->cbStart(tmpArray.get(), message[i].data.length()));
                        }
                        conseqErrorCount_m=0;
                    } else { //error
                        ...
                        wfo.setTotalFrameCount(totalFrames_m); wfo.setFrameLength(message[i].data.length());
                        if (processingQueue)
                ...
                case ACSBulkData::BD_DATA: {
                    ...
                    if (currentState_m==DataRcvState ) {
                        if (dataLength_m==0) { // we get the first data frame
                                ...
                                totalFrames_m = message[i].restDataLength+1;
                                frameCounter_m = 0;
                            }//if

                            dataLength_m += message[i].data.length();
                            frameCounter_m ++;

                            if ( message[i].restDataLength>0) {
                                if (nextFrame_m!=0 && nextFrame_m!=message[i].restDataLength) { // do we miss a frame ?
                                    FrameLostCompletion lde(__FILE__, __LINE__, __FUNCTION__);
                                    lde.setNextDataFrame(nextFrame_m);
                                    lde.setFrameCount(frameCounter_m);
                                    lde.setRestFrames(message[i].restDataLength);
                                    lde.setFrameLength(message[i].data.length());
                                ...
                                nextFrame_m = message[i].restDataLength-1;
                                ...
                                if (DDSConfiguration::debugLevel>1) {
                                    ...
                                    ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "All data from sendData has been received: %ld (Bytes) on: %s in %f sec. Receiver Data Rate: %f MBytes/sec",
                                        dataLength_m, topicName_m.c_str(), deltaTime, ((dataLength_m/(1024.0*1024.0))/deltaTime)));
                            ...
                            memcpy(tmpArray.get(), message[i].data.get_contiguous_buffer(), message[i].data.length());
                            if (processingQueue) {
                                if (enableCB_m) processQueue->cbReceive(tmpArray, message[i].data.length());
                            } else {
                                if (enableCB_m) BDNT_LISTENER_USER_ERR(callback_mp->cbReceive(tmpArray.get(), message[i].data.length()));
                            }
                            conseqErrorCount_m=0;
                ...
                case ACSBulkData::BD_STOP: {
                    if (currentState_m==DataRcvState ) {
                        ...
                        if (frameCounter_m==0) {
                            ...
                            if (frameCounter_m != totalFrames_m) {
                                ...
                                lde.setNextDataFrame(nextFrame_m);
                                lde.setFrameCount(frameCounter_m);
                                lde.setRestFrames(message[i].restDataLength); // should be ??
                                lde.setFrameLength(message[i].data.length()); // should be 0
                                lde.setStreamFlowName(topicName_m.c_str());
                                if (processingQueue)
                                    processQueue->onDataLost(frameCounter_m, totalFrames_m, lde);
                                else
                        ...
                        if (currentState_m==StopState) {
                                ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_WARNING, "On %s stop (BD_STOP) arrived in stop state - will be ignored!", topicName_m.c_str()));
                        } else { //StartState || IgnoreDataState
                                ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_WARNING, "On %s stop (BD_STOP) arrived in start state: no parameter data (startSend) has arrived!", topicName_m.c_str()));
                            }//if-else
                    }
                    ...
                    if (cbReceiveNumCalls_m>0) { // if we call cbReceive at least once we calculate the average
                        ...
                        if (DDSConfiguration::debugLevel>1) {
                            ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "Average processing time for: %s for %d call(s) of cbReceive(): %fs. What corresponds to throughput of: %fMB/sec",
                            ...
                            ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "Protocol Status for: %s [sample rcv: %lld (%lld) HB: %lld (%lld) ACKs: %lld (%lld) NACKs: %lld (%lld) Rejected: %lld]",
                                topicName_m.c_str(), drps.received_sample_count_change, drps.received_sample_bytes_change, drps.received_heartbeat_count_change, drps.received_heartbeat_bytes_change,
                                drps.sent_ack_count_change, drps.sent_ack_bytes_change, drps.sent_nack_count_change, drps.sent_nack_bytes_change, drps.rejected_sample_count_change));
                            ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_DEBUG, "Cache Status: sample count (peak): %lld (%lld)", drcs.sample_count, drcs.sample_count_peak));
                        }
                ...
                default:
                    ...
                    udt.setDataType(message[i].typeOfdata);
    ...
    frameDataReader_mp->return_loan(message, si);
}//on_data_available
    ...
    if (lcs.alive_count_change>0) {
        for(int i=0; i<lcs.alive_count_change; i++) {
            ...
            if (lcs.alive_count>1) {
                ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_WARNING, "Too many connected senders to flow: %s of the stream: %s: %d Max allowed 1", callback_mp->getFlowName(), callback_mp->getStreamName(), lcs.alive_count));
            }
            if (processingQueue)
        ...
        for(int i=lcs.alive_count_change; i<0; i++) {
            ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_INFO, "A sender has disconnected to flow: %s of the stream: %s. Total alive connection(s): %d", callback_mp->getFlowName(), callback_mp->getStreamName(), lcs.alive_count));
            if (processingQueue)
                processQueue->onSenderConnect(lcs.alive_count);
            else
...
void BulkDataNTReaderListener::on_sample_rejected( DDS::DataReader*, const DDS::SampleRejectedStatus& srs) {
    ...
    if (DDSConfiguration::debugLevel > 0 || rg.checkAndIncrement()) {
        ...
        ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_WARNING, "Sample Rejected, but NOT lost (need to be resent): reason %d, change: %d, total: %d!", srs.last_reason, srs.total_count_change, srs.total_count));
    }
}//on_sample_rejected
...
void BulkDataNTReaderListener::increasConseqErrorCount() {
    ...
    if (conseqErrorCount_m>=maxConseqErrorCount_m) {
        ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__, (LM_ALERT, "Too many consequent errors: %d/%d on %s", conseqErrorCount_m, maxConseqErrorCount_m, topicName_m.c_str()));
            //TBD: disconnect
    }
...
void BulkDataNTReaderListener::enableCallingCB() {
    enableCB_m=true;
    ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__,
    (LM_INFO, "Calling user's CB for flow: %s of the stream: %s has been ENABLED.", callback_mp->getFlowName(), callback_mp->getStreamName()));
}//enableCallingCB

void BulkDataNTReaderListener::disableCallingCB() {
    enableCB_m=false;
    ACS_LOG(LM_RUNTIME_CONTEXT, __FUNCTION__,
    (LM_INFO, "Calling user's CB for flow: %s of the stream: %s has been DISABLED.", callback_mp->getFlowName(), callback_mp->getStreamName()));
}//disableCallingCB
