...
namespace acsalarm
{
	...
	class ASIMessage
	{
		public:
			ASIMessage();
			ASIMessage(std::shared_ptr<std::vector<acsalarm::FaultState> > & faults);
			virtual ~ASIMessage();

			std::vector<acsalarm::FaultState> & getFaultStates() { return *faultStates; }
			void setFaultStates(std::shared_ptr<std::vector<acsalarm::FaultState> > &faults) { faultStates = faults; }
			...
			void setSourceTimestamp(std::shared_ptr<acsalarm::Timestamp> &ts) { sourceTimestamp = ts; }

		private:
			std::shared_ptr<std::vector<acsalarm::FaultState> > faultStates;
			bool backup;
			std::string version;
		...
		protected:
			std::shared_ptr<acsalarm::Timestamp> sourceTimestamp;
	};
}
#endif
