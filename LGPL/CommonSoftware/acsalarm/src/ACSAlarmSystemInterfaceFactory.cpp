 ...
 */
acsalarm::AlarmSystemInterface* ACSAlarmSystemInterfaceFactory::createSource()
{
 ...
 */
acsalarm::AlarmSystemInterface* ACSAlarmSystemInterfaceFactory::createSource(string sourceName)
{
 ...
 */
AlarmSystemType ACSAlarmSystemInterfaceFactory::usingACSAlarmSystem()
{ 
	ACE_Guard<ACE_Recursive_Thread_Mutex> guard(main_mutex);
...
shared_ptr<acsalarm::FaultState>ACSAlarmSystemInterfaceFactory::createFaultState(string family, string member, int code) 
{
	ACS_TRACE("ACSAlarmSystemInterfaceFactory::createFaultState(string, string, int)");
	ACE_Guard<ACE_Recursive_Thread_Mutex> guard(main_mutex);
	shared_ptr<acsalarm::FaultState> retVal;
	if (m_AlarmSystemType  == NOT_YET_INITIALIZED) {
...
shared_ptr<acsalarm::FaultState>ACSAlarmSystemInterfaceFactory::createFaultState()
{
...
void ACSAlarmSystemInterfaceFactory::createAndSendAlarm(string & faultFamily, string & faultMember, int faultCode, bool active, Properties & faultProperties, string sourceName) 
{
	...
	shared_ptr<acsalarm::FaultState> fltstate = ACSAlarmSystemInterfaceFactory::createFaultState(faultFamily, faultMember, faultCode);
	...
	shared_ptr<Timestamp> tstampAutoPtr(new Timestamp());
	...
	shared_ptr<Properties> propsAutoPtr(propsPtr);
...
void ACSAlarmSystemInterfaceFactory::cleanUpSourceSingleton()
{
	...
	if(m_sourceSingleton_p!=NULL)
	{
		// force the deletion of the allocated memory for the shared source shared_ptr
