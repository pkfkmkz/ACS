...
{
	shared_ptr<acsalarm::AlarmSystemInterface> temp(ACSAlarmSystemInterfaceFactory::createSource());
				...
				bool active)
{
	...
	if (alarmProps.getSize()>0) {
		shared_ptr<Properties> props(new Properties(alarmProps));
