...
void smartptrtest :: defaultConstructorTest (void)
{
    ...
    CPPUNIT_ASSERT_EQUAL ((MockComponent *)foo.get(), MockComponent::_nil());
}

void smartptrtest :: parmConstructorTest (void)
{
    ACE_CString cname("Foo");
    maci::SmartPtr<MockComponent> foo(maci::make_shared_component(mcs, true, MockComponent::_duplicate(mcs->getComponent<MockComponent>(cname.c_str()))));
    CPPUNIT_ASSERT_EQUAL (foo.get(), mcs->getComponent<MockComponent>(cname.c_str()));
}
...
void smartptrtest :: copyConstructorTest (void)
{
    ...
    CPPUNIT_ASSERT_EQUAL (bar.get(), foo.get());
}
...
void smartptrtest :: releaseTest(void)
{
    ...
    maci::SmartPtr<MockComponent> foo(maci::make_shared_component(mcs, true, MockComponent::_duplicate(mcs->getComponent<MockComponent>(cname.c_str()))));
    foo.reset();
    CPPUNIT_ASSERT_EQUAL (foo.get(), (MockComponent *)0);
}
...
void smartptrtest :: isNilSetTest(void)
{
    ...
    cout.flush();
    CPPUNIT_ASSERT (foo.get() != (MockComponent *)0);
}
...
void smartptrtest :: assnopr_cpyarginvalidTest(void)
{
    ACE_CString cname("Foo");
    maci::SmartPtr<MockComponent> foo;
    maci::SmartPtr<MockComponent> bar(maci::make_shared_component((maci::MockContainerServices *) 0, false, MockComponent::_duplicate(mcs->getComponent<MockComponent>(cname.c_str()))));
    foo = bar;
}
...
void smartptrtest :: assnopr_constinvalidTest (void)
{
    ACE_CString cname("Foo");
    maci::SmartPtr<MockComponent> foo;
    const maci::SmartPtr<MockComponent> bar(maci::make_shared_component((maci::MockContainerServices *) 0, false, MockComponent::_duplicate(mcs->getComponent<MockComponent>(cname.c_str()))));
    foo = bar;
}
