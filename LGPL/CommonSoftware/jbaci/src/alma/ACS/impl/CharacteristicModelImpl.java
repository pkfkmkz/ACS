...
public class CharacteristicModelImpl implements CharacteristicModelOperations {
		...
		throws NoSuchCharacteristic
	{
		try
		{
			return Long.parseLong(dao.get_string(prefix+name));
		}
		catch (Throwable th)
		{
			throw new NoSuchCharacteristic(name, modelName);
	...
	public long getuLongLong(String name) throws NoSuchCharacteristic {
		try {
			return Long.parseUnsignedLong(dao.get_string(prefix+name));
		} catch (Throwable th) {
			throw new NoSuchCharacteristic(name, modelName);
		}
	}
	...
	public int getuLong(String name) throws NoSuchCharacteristic {
		return getLong(name);
	}
	...
	public int[] getuLongSeq(String name) throws NoSuchCharacteristic {
		try {
			...
			for (int i = 0; i < strAr.length; i++) {
				try {
					retVal[i] = Integer.parseUnsignedInt(strAr[i]);
	...
	public long[] getLongLongSeq(String name) throws NoSuchCharacteristic {
		try {
			String[] strAr = dao.get_string_seq(prefix+name);
			long[] retVal = new long[strAr.length];
			for (int i = 0; i < strAr.length; i++) {
				try {
					retVal[i] = Long.parseLong(strAr[i]);
				} catch (NumberFormatException ex) {
					BigInteger tmp = new BigInteger(strAr[i]);
					if (tmp.signum() > 0)
						retVal[i] = Long.MAX_VALUE;
					else
						retVal[i] = Long.MIN_VALUE;
				}
			}
			return retVal;
		} catch (Throwable th) {
	...
	public long[] getuLongLongSeq(String name) throws NoSuchCharacteristic {
		try {
			String[] strAr = dao.get_string_seq(prefix+name);
			long[] retVal = new long[strAr.length];
			for (int i = 0; i < strAr.length; i++) {
				try {
					retVal[i] = Long.parseUnsignedLong(strAr[i]);
				} catch (NumberFormatException ex) {
					BigInteger tmp = new BigInteger(strAr[i]);
					if (tmp.signum() > 0)
						retVal[i] = Long.MAX_VALUE;
