...
import glob, os, string, re, xml.parsers.expat, sys, time
...
class Subsystem(object):
    ...
    def __init__(self, name, basedir,min,max,exaplesAndTestsRange,exclude=["CVS"],include=None):
        ...
        if len(self.duplicatedErrors)==0:
            ...
            print("The subsystem contains the following duplicated error definitions:", self.duplicatedErrors)
        ...
        if len(self.outOfRangeErrors)==0:
            ...
            print("The subsystem contains the following errors with an invalid number:", self.outOfRangeErrors)
        ...
        if len(self.testAndExampleErrors)==0:
            ...
            print("The following errors are used for test and examples:", self.testAndExampleErrors)
        # Store the time
        self.endTime=time.ctime()
        self.endFloatTime=time.time()
        # Print the time
        print(self.endTime+ ", check terminated")
        print("{:.3f}".format(self.endFloatTime-self.startFloatTime), "seconds requested to check this system")
    ...
    def checkSubsystemErrors(self):
        ...
        for t in range(1,len(self.errors)):
            ...
            if actNum == prevNum and self.duplicatedErrors.count(actNum)==0 and actNum<self.testErrorRange["Min"] and actNum>self.testErrorRange["Max"]:
        ...
        for t in self.errors:
            if t.getNumber()>=self.testErrorRange["Min"] and t.getNumber()<=self.testErrorRange["Max"]:
    ...
    def generateHTMLPage(self,fileName):
        ...
        text=text+'<P><FONT size="-1">Subsystem scanned in ' + "{:.3f}".format(self.endFloatTime-self.startFloatTime) + ' seconds.</FONT></P>'
    ...
    def generateErrorDetailsHTMLPage(self,error,fileName):
        ...
        page=page+'<P><FONT size="-1">Subsystem scanned in ' + "{:.3f}".format(self.endFloatTime-self.startFloatTime) + ' seconds.</FONT></P>'
