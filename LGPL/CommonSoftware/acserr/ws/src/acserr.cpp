...
static const char *rcsId="@(#) $Id: acserr.cpp,v 1.92 2010/05/31 09:36:51 bjeram Exp $";
...
*******************************************************************************/
void ACSErr::handle_exception(std::string function, bool rethrow=false) {
    ...
    try {
        ...
        ACS_LOG(LM_RUNTIME_CONTEXT, function.c_str(), (LM_WARNING, "Unexpected CORBA::Exception: %s", ex._info().c_str()));
        std::cout << function << " " << "Unexpected CORBA::Exception: " << ex._info() << std::endl;
        ex._tao_print_exception((function.c_str()));
    } catch(const std::exception& ex) {
        ACS_LOG(LM_RUNTIME_CONTEXT, function.c_str(), (LM_WARNING, "Unexpected std::exception: %s", ex.what()));
    } catch(...) {
        ACS_LOG(LM_RUNTIME_CONTEXT, function.c_str(), (LM_WARNING, "Unexpected unkown exception"));
    }
    if (rethrow) std::rethrow_exception(eptr);
}
...
void ErrorTraceHelper::prepareMsg() const {
    if (m_errorTracePtr == nullptr || m_depth == 0) {
        return;
    }
    m_errorMsg = CORBA::string_dup(prepareMsg(m_errorTracePtr, m_depth).c_str());
}

std::string ErrorTraceHelper::prepareMsg(ACSErr::ErrorTrace* et, unsigned int depth) const {
    std::ostringstream ostr;
    ostr << "ACSErr::ErrorTrace" << " (type=" << et->errorType << ", code=" << et->errorCode << "): " << et->shortDescription.in();
    for (unsigned int j=0; j < et->data.length(); j++) {
        ostr << (j == 0 ? " [" : ", ");
        ostr << et->data[j].name.in() << "=\"" << et->data[j].value.in() << "\"";
    }
    if (et->data.length() > 0) {
        ostr << "]";
    }
    ostr << " at " << et->routine.in() << "(" << et->file.in() << ":" << et->lineNum << ")";
    if (depth > 1 && et->previousError.length() > 0) {
        ostr << std::endl << prepareMsg(&et->previousError[0], depth-1);
    } else {
        ostr << std::ends;
    }
    return ostr.str();
