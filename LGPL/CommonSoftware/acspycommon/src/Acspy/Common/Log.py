...
def _is_internal_frame(frame):
    """Signal whether the frame is a CPython, logging module internal or ACS Log module internal."""
    if logging._is_internal_frame(frame):
        return True
...
def flush():
    ...
    global NEXTEVENT
    global INTERVAL
    global CENTRALHANDLER
    global EXITFLUSHTHREAD

    if not EXITFLUSHTHREAD:
...
def startPeriodicFlush(interval=DEFAULT_FLUSH_PERIOD):
    ...
    if FLUSHTHREAD is None and not threading._SHUTTING_DOWN:
        EXITFLUSHTHREAD = False
        INTERVAL = interval
...
def stopPeriodicFlush():
    ...
    EXITFLUSHTHREAD = True
    try:
        SCHEDULER.cancel(NEXTEVENT)
    except:
        pass
    if FLUSHTHREAD is not None:
        FLUSHTHREAD.join()
        FLUSHTHREAD = None
    EXITFLUSHTHREAD = None
#------------------------------------------------------------------------
...
class Logger(logging.Logger):
    ...
    def findCaller(self, stack_info=False, stacklevel=1):
        ...
        if stack_info:
            with io.StringIO() as sio:
                ...
                if sinfo[-1] == '\n':
                    sinfo = sinfo[:-1]
        filename = os.path.normcase(sys.argv[0] if sys.argv[0] in co.co_filename else co.co_filename)
        return filename, f.f_lineno, co.co_name, sinfo
    ...
    def makeRecord(self, name, level, fn, lno, msg, args, exc_info, func=None, extra=None, sinfo=None):
        ...
        """
        return makeACSLogRecord(name, level, fn, lno, msg, args, exc_info, func, extra, sinfo)
    #------------------------------------------------------------------------
        ...
        try:
            ...
            if self.stats.getDisableStatistics() == False:
                ...
                if timeElapsedSinceLastStatistics >= self.stats.getStatisticsCalculationPeriod():
                    ...
                    self.acsLog(LEVELS[ACSLog.ACS_LOG_DEBUG], statsMsg, extra = { 'data' : statsData})
