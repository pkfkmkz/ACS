...
if 'ACS_LOG_FILE' in environ:
    LOG_FILE_NAME = environ['ACS_LOG_FILE']
else:
    if 'ACS_TMP' in environ:
        LOG_FILE_NAME = path.join(environ['ACS_TMP'], 'acs_local_log')
    else:
        if os.path.isdir(path.join(environ['ACSDATA'], 'tmp/' + gethostname()))==False:
            os.mkdir(path.join(environ['ACSDATA'], 'tmp/' + gethostname()))
...
def makeACSLogRecord(name, level, fn, lno, msg, args, exc_info, func=None, extra=None, sinfo=None):
    """
    Build the ACSLogRecord for this information.
    """
    rv = ACSLogRecord(name, level, fn, lno, msg, args, exc_info, func, sinfo)
    if extra:
        for key in extra:
...
class ACSHandler(logging.handlers.BufferingHandler):
    ...
    def sendLog(self, record):
        ...
        #Skip remote logging if the CORBALogger is not available at the time
        if self.getCORBALogger() is None:
            return
