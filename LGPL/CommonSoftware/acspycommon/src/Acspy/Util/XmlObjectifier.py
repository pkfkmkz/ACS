    ...
    if xmlObject.hasChildNodes():
		...
		# Handle name spaces
		    ...
		    # Keep new name spaces for later use
                    ...
                    elementName = elementName.replace(nameSpaceKey,nameSpaceMapping[nameSpaceKey])
        ...
        for item in items:
            if len(xmlObject.__dict__[item]) == 1:
                xmlObject.__dict__[item] = xmlObject.__dict__[item][0]
...
class XmlObject(minidom.Document):
    ...
    def __init__(self, xmlString = None, fileName = None, skipChars = '',
        ...
        for attr in dir(dom):
            if attr not in ['__init__', '__weakref__', 'documentElement', 'firstChild', 'lastChild', 'localName', '__class__', '__dict__', '__dir__']:
...
class XmlElement(minidom.Element):
    ...
    def __init__(self, element, mapNameSpaces, nameSpaceMapping, skipChars):
        ...
        for attr in dir(element):
            if attr not in ['__init__', 'getAttribute', '__weakref__', '_localName', 'attributes', 'firstChild', 'lastChild', 'localName', 'ownerDocument', '__class__', '__dict__', '__dir__']:
