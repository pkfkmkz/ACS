...
class TypelessProperty(Property):
    ...
    def __init__(self, name, charCompRef):
        ...
        try:
            ...
            for attr in list(xml_obj.attributes.keys()):
    ...
    def _get_resolution(self):
        ...
        '''
        try:
            return int(self.getCDBDict()['resolution'])
        except:
            #warn them about CDB access
            self.getLogger().logWarning("Some problem occurred when attempting to retrieve data from the ACS CDB")
            print_exc()
...
class GenericProperty(TypelessProperty):
    ...
    def _get_default_timer_trigger(self):
        ...
        '''
        try:
            return int(float(self.getCDBDict()['default_timer_trig']) * 10000000.0)
        except:
            #warn them about CDB access
            self.getLogger().logInfo("Some problem occurred when attempting to retrieve default timer trigger data from the ACS CDB")
            print_exc()
            #return an acceptable default value instead... 1.0 seconds (Default time)
            return 10000000
    #--------------------------------------------------------------------------
    def _get_min_timer_trigger(self):
        ...
        '''
        try:
            return int(float(self.getCDBDict()['min_timer_trig']) * 10000000.0)
        except:
            #warn them about CDB access
    ...
    def _get_graph_min(self):
        ...
        try:
            ...
            print_exc()
            #return an acceptable default value instead.
            return self.coerceToPropertyType("Min")
    #--------------------------------------------------------------------------
    def _get_graph_max(self):
        ...
        try:
            ...
            print_exc()
            #return an acceptable default value instead.
            return self.coerceToPropertyType("Max")
    #--------------------------------------------------------------------------
    ...
    def get_async(self, cb, desc):
        ...
        cb.done(retVal, compl, CBDescOut(0, desc.id_tag))
        return 
    #--------------------------------------------------------------------------
    ...
    def create_monitor(self, cb, desc):
        ...
        return self.create_postponed_monitor(0, cb, desc)
    #--------------------------------------------------------------------------
    def monitorRunner(self, cb, desc):
        ...
        cb.working(value, compl, CBDescOut(0, desc.id_tag))
        return
    #--------------------------------------------------------------------------
    def create_postponed_monitor(self, start_time, cb, desc):
        ...
        timeoutID = GenericProperty.getScheduler().scheduleTimeout(self.monitorRunner, start_time, frequency, (cb, desc))

        monitorObject = self.getMonitorObject(GenericProperty.getScheduler(), timeoutID)
    ...
    def set_sync(self, newVal):
        ...
        try:
            ...
            if compl == None:
                #succeeded! now just create a "no-error" completion
                compl = Completion(int(getTimeStamp().value),  #unsigned long long timeStamp;
                                   0,  #ACSErr::CompletionType type;
                                   0,  #ACSErr::CompletionCode code;
                                   ())  #ErrorLinkedList  previousError;
        except Exception as e:
    ...
    def set_async(self, newVal, cb, desc):
        ...
        cb.done(compl, CBDescOut(0, desc.id_tag))
        return 
    #--------------------------------------------------------------------------
