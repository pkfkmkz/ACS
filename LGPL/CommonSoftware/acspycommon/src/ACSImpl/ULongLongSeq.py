...
'''
This module provides an implementation of the PuLongLongSeq IDL interface:
'''
...
from ACSImpl.Monitors            import MonitoruLongLong
#--GLOBALS---------------------------------------------------------------------

#------------------------------------------------------------------------------
#--P property------------------------------------------------------------------
#------------------------------------------------------------------------------
class PuLongLongSeq(GenericProperty):
    '''
    Properties can be derived from PuLongLongSeq only if their IDL derives from
    ACS::PuLongLongSeq.
    '''
    #--------------------------------------------------------------------------
    ...
    def getMonitorObject(self, scheduler, timeoutID):
        ...
        '''
        return MonitoruLongLong(scheduler, timeoutID)
    #--------------------------------------------------------------------------
...
#--RO property-----------------------------------------------------------------
#------------------------------------------------------------------------------
class ROuLongLongSeq(ACS__POA.ROuLongLongSeq, PuLongLongSeq):
    '''
    Properties can be derived from ROuLongLongSeq only if their IDL derives from
    ACS::ROuLongLongSeq.
    '''
    #--------------------------------------------------------------------------
    def __init__(self, name, charCompRef, devIORef=None):
        ...
        Raises: Nothing.
        '''
        PuLongLongSeq.__init__(self, name, charCompRef, devIORef)
        return
    #--------------------------------------------------------------------------
    #--Overriden methods because BACI is inconsistent--------------------------
    #--------------------------------------------------------------------------
...
#--RW property----------------------------------------------------------------
#-----------------------------------------------------------------------------
class RWuLongLongSeq(ACS__POA.RWuLongLongSeq, ROuLongLongSeq):
    '''
    Properties can be derived from ROuLongLongSeq only if their IDL derives from
    ACS::ROuLongLongSeq.
    '''
    ...
    def __init__(self, name, charCompRef, devIORef=None):
        ...
        Raises: Nothing.
        '''
        ROuLongLongSeq.__init__(self, name, charCompRef, devIORef)
        return
    #--------------------------------------------------------------------------
    #--Overriden methods because BACI is inconsistent--------------------------
    #--------------------------------------------------------------------------
    ...
    def increment(self, cb, desc):
        ...
        '''
        compl = self.set_sync([x+1 for x in self.get_sync()[0]])
