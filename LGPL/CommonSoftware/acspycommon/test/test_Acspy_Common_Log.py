...
#
# "@(#) $Id: test_Acspy_Common_Log.py,v 1.1.1.1 2012/03/07 17:40:45 acaproni Exp $"
#
# who       when      what
# --------  --------  ----------------------------------------------
...
#------------------------------------------------------------------------------
__revision__ = "$Id: test_Acspy_Common_Log.py,v 1.1.1.1 2012/03/07 17:40:45 acaproni Exp $"
#--REGULAR IMPORTS-------------------------------------------------------------
import unittest
import mock
...
class LogLevelsCheck(unittest.TestCase):
    ...
    def testLoggingTraceLevel(self):
        ...
        self.assertEqual(Log.logging.NOTSET+1, Log.logging.TRACE)
        self.assertEqual("TRACE", Log.logging.getLevelName(Log.logging.TRACE))

    def testLoggingNoticeLevel(self):
        """LogLevel NOTICE level is correctly defined in logging module"""
        self.assertEqual(Log.logging.INFO+1, Log.logging.NOTICE)
        self.assertEqual("NOTICE", Log.logging.getLevelName(Log.logging.NOTICE))

    def testloggingAlertLevel(self):
        """LogLevel ALERT level is correctly defined in logging module"""
        self.assertEqual(Log.logging.CRITICAL+1, Log.logging.ALERT)
        self.assertEqual("ALERT", Log.logging.getLevelName(Log.logging.ALERT))

    def testLoggingEmergencyLevel(self):
        """LogLevel EMERGENCY level is correctly defined in logging module"""
        self.assertEqual(Log.logging.ALERT+1, Log.logging.EMERGENCY)
        self.assertEqual("EMERGENCY", Log.logging.getLevelName(Log.logging.EMERGENCY))

    def testLoggingOffLevel(self):
        """LogLevel OFF level is correctly defined in logging module"""
        self.assertEqual(Log.logging.EMERGENCY+1, Log.logging.OFF)
        self.assertEqual("OFF", Log.logging.getLevelName(Log.logging.OFF))

    def testLogLevelMapping(self):
        """LogLevel numeric log levels map to correct logging values"""
        self.assertEqual(Log.LEVELS[0], Log.logging.NOTSET)
        self.assertEqual(Log.LEVELS[1], Log.logging.TRACE)
        self.assertEqual(Log.LEVELS[2], Log.logging.DELOUSE)
        self.assertEqual(Log.LEVELS[3], Log.logging.DEBUG)
        self.assertEqual(Log.LEVELS[4], Log.logging.INFO)
        self.assertEqual(Log.LEVELS[5], Log.logging.NOTICE)
        self.assertEqual(Log.LEVELS[6], Log.logging.WARNING)
        self.assertEqual(Log.LEVELS[7], Log.logging.ERROR)
        self.assertEqual(Log.LEVELS[8], Log.logging.ERROR)
        self.assertEqual(Log.LEVELS[9], Log.logging.CRITICAL)
        self.assertEqual(Log.LEVELS[10], Log.logging.ALERT)
        self.assertEqual(Log.LEVELS[11], Log.logging.EMERGENCY)
        self.assertEqual(Log.LEVELS[99], Log.logging.OFF)

    def testLogLevelReverseMapping(self):
        """LogLevel Logging values map to correct numeric log levels"""
        self.assertEqual(0, Log.RLEVELS[Log.logging.NOTSET])
        self.assertEqual(1, Log.RLEVELS[Log.logging.TRACE])
        self.assertEqual(2, Log.RLEVELS[Log.logging.DELOUSE])
        self.assertEqual(3, Log.RLEVELS[Log.logging.DEBUG])
        self.assertEqual(4, Log.RLEVELS[Log.logging.INFO])
        self.assertEqual(5, Log.RLEVELS[Log.logging.NOTICE])
        self.assertEqual(6, Log.RLEVELS[Log.logging.WARNING])
        self.assertEqual(8, Log.RLEVELS[Log.logging.ERROR])
        self.assertEqual(9, Log.RLEVELS[Log.logging.CRITICAL])
        self.assertEqual(10, Log.RLEVELS[Log.logging.ALERT])
        self.assertEqual(11, Log.RLEVELS[Log.logging.EMERGENCY])
        self.assertEqual(99, Log.RLEVELS[Log.logging.OFF])

    def testACSLogLevelMapping(self):
        """LogLevel ACSLog log levels map to correct logging values"""
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_TRACE], Log.logging.TRACE)
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_DEBUG], Log.logging.DEBUG)
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_INFO], Log.logging.INFO)
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_NOTICE], Log.logging.NOTICE)
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_WARNING], Log.logging.WARNING)
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_ERROR], Log.logging.ERROR)
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_CRITICAL], Log.logging.CRITICAL)
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_ALERT], Log.logging.ALERT)
        self.assertEqual(Log.LEVELS[Log.ACSLog.ACS_LOG_EMERGENCY], Log.logging.EMERGENCY)
...
class LoggerClassCheck(unittest.TestCase):
    ...
    def testLoggerInit(self):
        ...
        self.assertEqual("mylogger", self.mylogger.name)
        self.assertNotEqual(None, self.mylogger.stdouthandler)
        self.assertNotEqual(None, self.mylogger.acshandler)
        ...
        self.assertEqual([self.mylogger.stdouthandler, self.mylogger.acshandler], self.mylogger.handlers)
        self.assertEqual(None, self.mylogger.parent)


    def testSetLevels(self):
        """Logger class set log levels to user-defined levels"""
        self.mylogger.setLevels(maci.Container.LogLevels(False,9, 9))
    ...
    def testSetLevelsDefault(self):
        ...
        self.mylogger.setLevels(maci.Container.LogLevels(False,9, 9))
        self.mylogger.setLevels(maci.Container.LogLevels(True,0, 0))
        self.assertEqual(True, self.mylogger.usingDefault)
        self.assertEqual(Log.DEFAULTLOCALHANDLER, self.mylogger.stdouthandler)
        self.assertEqual(Log.DEFAULTCENTRALHANDLER, self.mylogger.acshandler)
    ...
    def testSetLevelsValidInputs(self):
        ...
        for k in Acspy.Common.Log.LEVELS:
            self.mylogger.setLevels(maci.Container.LogLevels(False, k, k))
            self.assertEqual(False, self.mylogger.usingDefault)
    ...
    def testMakeRecordEmptyExtra(self):
        ...
        rv = self.mylogger.makeRecord('foo', Log.LEVELS[ACSLog.ACS_LOG_INFO], '/path/to/file.py',
                                      596, 'Main - Text', (), None, 'logInfo', {})
        self.assertEqual(expectedkeys.sort(), list(rv.__dict__.keys()).sort())
    ...
    def testMakeRecordExtras(self):
        ...
        self.assertEqual(True, 'AddName' in list(rv.__dict__.keys()))
        self.assertEqual(True, 'AddValue' in list(rv.__dict__.keys()))
...
class LoggerFunctionCheck(unittest.TestCase):
    ...
    def verifyOutput(self, handler, level, msg):
        ...
        self.assertEqual("handle", logcall[0])
        outrecord = logcall[1]
        self.assertEqual(level, outrecord[0].levelno)
        self.assertEqual(msg, outrecord[0].msg)
        #self.assertEqual(tfn, os.path.splitext(outrecord[0].filename)[0])
    ...
    def testLogErrorTrace(self):
        ...
        self.assertEqual("handle", logcall[0])
        self.assertEqual(ACSLog.ACS_LOG_ERROR, logcall[1][0].priority)
        self.assertEqual(et, logcall[1][0].errortrace)
    ...
    def testLogTypeSafe(self):
        ...
        self.assertEqual("handle", logcall[0])
        self.assertEqual(ACSLog.ACS_LOG_ERROR, logcall[1][0].priority)
        self.assertEqual(msg, logcall[1][0].msg)
        self.assertEqual(True, isinstance(logcall[1][0].rtCont, ACSLog.RTContext))
        self.assertEqual(ctxt, logcall[1][0].rtCont)
        self.assertEqual(True, isinstance(logcall[1][0].srcInfo, ACSLog.SourceInfo))
        self.assertEqual(src, logcall[1][0].srcInfo)
        self.assertEqual("", logcall[1][0].audience)
        self.assertEqual("", logcall[1][0].array)
        self.assertEqual("", logcall[1][0].antenna)
    ...
    def testLogTypeSafeNoContextorSource(self):
        ...
        self.assertEqual(ACSLog.ACS_LOG_ERROR, logcall[1][0].priority)
        self.assertEqual(msg, logcall[1][0].msg)
        self.assertEqual(True, logcall[1][0].rtCont is None)
        self.assertEqual(True, logcall[1][0].srcInfo is None)
        self.assertEqual("", logcall[1][0].audience)
        self.assertEqual("", logcall[1][0].array)
        self.assertEqual("", logcall[1][0].antenna)
...
#        self.mylogger.setLevels(maci.Container.LogLevels(False,0, 0))
        keys = sorted(list(Acspy.Common.Log.LEVELS.keys()), key=lambda key: Acspy.Common.Log.LEVELS[key])
        for l in keys:
            if l == 0 or l == 99:
...
class LoggerHandlerConfigCheck(unittest.TestCase):
    ...
    def testSetDefaultLevels(self):
        ...
        for k in Log.LEVELS:
            Log.setDefaultLevels(maci.Container.LogLevels(False, k, k))
    ...
    def testSetDefaultLevelsInvalidInputs(self):
        ...
        self.assertRaises(KeyError, Log.setDefaultLevels,
                          maci.Container.LogLevels(False, 25, 25))
        self.assertEqual(10,Log.DEFAULTLOCALHANDLER.level)
        self.assertEqual(10,Log.DEFAULTCENTRALHANDLER.level)
...
class OneLoggerCheck(unittest.TestCase):
    ...
    def testUnnamedLogger(self):
        ...
        self.assertEqual('None', self.mylogger.name)
        self.assertEqual(Log.DEFAULTLOCALHANDLER, self.mylogger.stdouthandler)
        self.assertEqual(Log.DEFAULTCENTRALHANDLER, self.mylogger.acshandler)
        
    def testUnnamedLoggerNamesList(self):
        """OneLoggerCheck names list has unnamed logger"""
        self.assertEqual([self.mylogger.name], Log.getLoggerNames())

    def testUnnamedLoggerNamesList(self):
        """OneLoggerCheck names list has unnamed logger when filtered by unnamed logger"""
        self.assertEqual([self.mylogger.name], Log.getLoggerNames(self.mylogger.name))
...
class SeveralLoggerCheck(unittest.TestCase):
    ...
    def tearDown(self):
        Log.setDefaultLevels(maci.Container.LogLevels(False,3, 3))
        self.plogger.setLevels(maci.Container.LogLevels(True,0, 0))
        self.clogger.setLevels(maci.Container.LogLevels(True,0, 0))
    
    def testLoggerNamesRequested(self):
        """SeveralLoggerCheck getLoggerNames returns all known loggers"""
        self.assertEqual(list(Log.logging.Logger.manager.loggerDict.keys()), Log.getLoggerNames())
    ...
    def testLoggerNamesRequestedFiltered(self):
        ...
        self.assertEqual(True, self.cname in Log.getLoggerNames(self.pname[:-1]))
        self.assertEqual(True, self.pname in Log.getLoggerNames(self.pname[:-1]))
        self.assertEqual([self.cname], Log.getLoggerNames(self.cname))
    ...
    def testLoggerSingleDefault(self):
        ...
        self.plogger.setLevels(maci.Container.LogLevels(False,4, 5))
        self.assertEqual(beforeloc, Log.DEFAULTLOCALHANDLER.level)
        self.assertEqual(beforecent, Log.DEFAULTCENTRALHANDLER.level)
        self.assertEqual(False, self.plogger.usingDefault)
        self.assertEqual(True, self.clogger.usingDefault)
        self.assertNotEqual(self.plogger.stdouthandler, self.clogger.stdouthandler)
        self.assertNotEqual(self.plogger.acshandler, self.clogger.acshandler)
        self.assertNotEqual(self.plogger.stdouthandler.level, self.clogger.stdouthandler.level)
        self.assertNotEqual(self.plogger.acshandler.level, self.clogger.acshandler.level)

    def testLoggerDefault(self):
        """SeveralLoggerCheck Levels changes on all loggers using default level"""
        Log.setDefaultLevels(maci.Container.LogLevels(False,4, 5))
        self.assertEqual(True, self.plogger.usingDefault)
        self.assertEqual(True, self.clogger.usingDefault)
        self.assertEqual(Log.DEFAULTLOCALHANDLER.level, self.plogger.stdouthandler.level)
        self.assertEqual(Log.DEFAULTCENTRALHANDLER.level, self.plogger.acshandler.level)
        self.assertEqual(Log.DEFAULTLOCALHANDLER.level, self.clogger.stdouthandler.level)
        self.assertEqual(Log.DEFAULTCENTRALHANDLER.level, self.plogger.acshandler.level)
    ...
    def testNestedOutput(self):
        ...
        nlogger.removeHandler(nlogger.stdouthandler)
        nlogger.stdouthandler = mock.Mock()
        nlogger.addHandler(nlogger.stdouthandler)
...
class DispatchPacketCheck(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.logger.removeHandler(self.logger.stdouthandler)
        self.logger.stdouthandler = mock.Mock()
        self.logger.addHandler(self.logger.stdouthandler)
        Log.CENTRALHANDLER.buffer = []
    ...
    def testBuffering(self):
        ...
        self.assertEqual(1, len(Log.CENTRALHANDLER.buffer))
        Log.CENTRALHANDLER.buffer = []
    ...
    def testDispatch(self):
        ...
        self.assertEqual(0, len(Log.CENTRALHANDLER.buffer))
        self.logger.logEmergency("Dispatch Sent")
        self.assertEqual(0, len(Log.CENTRALHANDLER.buffer))
        self.assertEqual('logEmergency',Log.CENTRALHANDLER.logSvc.method_calls[-1][0])
        try:
            cleanup
    ...
    def testBufferingandDispatch(self):
        ...
        self.assertEqual(0, len(Log.DEFAULTCENTRALHANDLER.buffer))
        self.assertEqual(1, len(Log.CENTRALHANDLER.buffer))
        self.logger.logAlert("Dispatch Sent")
        self.assertEqual(0, len(Log.CENTRALHANDLER.buffer))
        self.assertEqual('logNotice',Log.CENTRALHANDLER.logSvc.method_calls[-2][0])
        self.assertEqual('logAlert',Log.CENTRALHANDLER.logSvc.method_calls[-1][0])
        try:
            cleanup
            Log.CENTRALHANDLER.logSvc = None
        except:
...
class PeriodicFlushCheck(unittest.TestCase):
    ...
    def testCycleStartStop(self):
        ...
        self.assertEqual(True, Log.FLUSHTHREAD.is_alive())
        self.assertEqual(False, Log.NEXTEVENT is None)
        fs = Log.FLUSHTHREAD
        Log.stopPeriodicFlush()
        self.assertEqual(False, fs.is_alive())
        self.assertEqual(True, Log.FLUSHTHREAD is None)
