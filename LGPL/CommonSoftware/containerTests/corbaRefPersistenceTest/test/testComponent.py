...
for cmd in ['pyClient', 'cppClient', 'javaClient']:
    clients[cmd] = pexpect.spawn(cmd,searchwindowsize=2000)
    clients[cmd].expect("Ready")
    print("%s: %s" % (cmd, six.ensure_str(clients[cmd].after)))
print("")
...
for cmd in clients:
    ...
    print("%s: %s" % (cmd, six.ensure_str(clients[cmd].after.strip())))
print("")
...
for cmd in clients:
    ...
    print(six.ensure_str(clients[cmd].after))
    print("%s: %s" % (cmd, six.ensure_str(clients[cmd].after).strip()))
print("")
