...
{
    Logging::Logger::LoggerSmartPtr l = cs->getLogger();
    ...
    ACS_DEBUG_PARAM("TestLogLevelsComp::getLevels()","logger name:  %s ", l->getName().c_str());	
    //level[3] = static_cast< CORBA::Long >(LogLevelDefinition::fromACEPriority(ACE_Log_Priority(l->getRemoteLevel(l->getName()))));
    //level[4] = static_cast< CORBA::Long >(LogLevelDefinition::fromACEPriority(ACE_Log_Priority(l->getLocalLevel(l->getName()))));
    Logging::LoggingConfigurable::LogLevels logLevels = ContainerImpl::getContainer()  -> get_logLevels(l->getName().c_str()) ;
    level[3] = logLevels.minLogLevel;
    level[4] = logLevels.minLogLevelLocal;
