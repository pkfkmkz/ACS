...
static const char *rcsId="@(#) $Id: acssampImpl.cpp,v 1.34 2008/10/07 06:41:54 cparedes Exp $";
...
{  
	...
	if (!CORBA::is_nil(obj.in()))
		...
		if (ACE_OS::strcmp(ret_struct->properties[ind].name,fullName.c_str()) == 0 )
		    ...
		    if ( myPropIntf.find("RWdouble") != ACE_CString::npos) 
			{
		    ...
		    else if (myPropIntf.find("ROdouble") != ACE_CString::npos)
			{
		    ...
		    else if (myPropIntf.find("RWlong") != ACE_CString::npos)
			{
		    ...
		    else if (myPropIntf.find("ROlong") != ACE_CString::npos)
			{
		    ...
		    else if (myPropIntf.find("ROfloat") != ACE_CString::npos)
			{
...
ACSSampImpl::addComponenttoList(CORBA::Object_ptr component_ref) 
{
    ...
    ACS_DEBUG_PARAM("acssamp::ACSSampImpl::addComponenttoList","current number of SampObj: %lu",component_list.size());
}
...
ACSSampImpl::removeComponentfromList(CORBA::Object_ptr component_ref) 
{
    ...
    ACS_DEBUG_PARAM("acssamp::ACSSampImpl::removeComponentfromList","remaining number of SampObj: %lu",component_list.size());
}
