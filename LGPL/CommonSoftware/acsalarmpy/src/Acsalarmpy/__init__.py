...
from . import AcsAlarmSystem_xsd
from . import FaultState
...
class AlarmSystemInterfaceFactory(object):
    ...
    def getLogger(cls):
        if cls.logger is None:
            cls.logger = Log.getLogger("AlarmSystemInterfaceFactory")
        return cls.logger
    @classmethod
    def init(cls, man=None):
        ...
        Returns: None
        """
        cls.getLogger().logTrace("AlarmSystemInterfaceFactory::init() entering.")
        cls.manager = man
        ...
        cls.getLogger().logDebug("Using %s alarm system" % cls.systemtype)
        cls.getLogger().logTrace("AlarmSystemInterfaceFactory::init() exiting.")
    ...
    def done(cls):
        ...
        Returns:  None
        """
        cls.getLogger().logTrace("AlarmSystemInterfaceFactory::done() entering.")
        cls.manager = None
        cls.initialized = False
        cls.systemtype = None
        cls.registry = {}
        cls.getLogger().logTrace("AlarmSystemInterfaceFactory::done() exiting.")
    ...
    def createSource(cls, sourceName = None):
        ...
        """
        cls.getLogger().logTrace("AlarmSystemInterfaceFactory::createSource() entering.")
        if not cls.initialized:
            raise ErrFactory.ACSASFactoryNotInitedExImpl()
        cls.getLogger().logTrace("AlarmSystemInterfaceFactory::createSource() exiting.")
    ...
    def createFaultState(cls, family = None, member = None, code = None):
        ...
        Raises:  exception if factory has not been initialized
        """
        cls.getLogger().logTrace("AlarmSystemInterfaceFactory::createFaultState() entering.")
        if not cls.initialized:
            raise ErrFactory.ACSASFactoryNotInitedExImpl()
        cls.getLogger().logTrace("AlarmSystemInterfaceFactory::createFaultState() exiting.")
