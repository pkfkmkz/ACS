...
static const char *rcsId="@(#) $Id: acsutilFindFile.cpp,v 1.6 2012/01/20 22:07:43 tstaig Exp $"; 
                           ...
                           char      *dirFlag)
{
    ...
    struct stat stbuf;
    const char *pchar;
    char *tok,*intRoot,*intList,*acsRoot,*acsdeps;/*,*acsData;*/
    ...
    *mode = 0; *size = 0; *dirFlag = 0;
    ...
    acsdeps = getenv ("ACSDEPS");
    if (acsdeps != (char *) NULL) {
        sprintf(myFile,"%s/%s",acsdeps,fileName);
        if (stat(myFile,&stbuf) == 0) { goto exit_success;}
    }
