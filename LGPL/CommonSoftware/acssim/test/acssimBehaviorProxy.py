...
if __name__=="__main__":
    ...
    print("dir(TEST_RPS_1):", [p for p in dir(bp) if not p.startswith("__")])
    print()

    addComponent("TEST_RPS_1", MockComponent("TEST_RPS_1"))
    try:
        print("bp.getMethod('on'):", dict(OrderedDict(sorted(bp.getMethod('on').items()))))
        print("Good!")
    except:
    ...
    try:
        print("bp.getMethod('nonexistentMethod'):", end='')
        bp.getMethod('nonexistentMethod')
        print("Bad!")
    except CORBA.NO_RESOURCES as temp_ex:
