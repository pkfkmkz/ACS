...
if __name__=="__main__":
    ...
    print("dyn.getMethod('startRamping'):", dict(OrderedDict(sorted(dyn.getMethod('startRamping').items()))))
    #from PowerSuppy
    print("dyn.getMethod('_get_readback'):", dict(OrderedDict(sorted(dyn.getMethod('_get_readback').items()))))
    print()

    print("--HELLOWORLD1--")
    dyn = Dynamic("HELLOWORLD1", "IDL:alma/acsexmplHelloWorld/HelloWorld:1.0")
    print("dyn.getMethod('displayMessage'):", dict(OrderedDict(sorted(dyn.getMethod('displayMessage').items()))))
    try:
        print("dyn.getMethod('nonexistentMethod'):", end='')
        dyn.getMethod('nonexistentMethod')
    except CORBA.NO_RESOURCES as exception:
