...
def _executeDict(in_dict, args, local_ns):
    ...
    return _executeList(in_dict['Value'], args, local_ns, in_dict['Type'] if 'Type' in in_dict else None)
#------------------------------------------------------------------------------
def _executeList(code_list, args, local_ns, code_type=None):
    '''
    ...
    if code_type != "TypeCodes" and isinstance(code_list, list):
        code_list = listToCodeObj(code_list, {})
    ...
    if isfunction(code_list):
        ...
        exec(code_list, globals(), _locals)
        exec("joe = stringFunction(parameters)", globals(), _locals)
        return _locals['joe']
    elif code_type == "TypeCodes":
        return code_list[-1]
    else:
...
#             logger().logWarning("Failed to execute the '" + line + "' statement!")
    ...
    if type(final_val)==str and final_val.count("raise ") == 1:
        ...
        exec(final_val, globals(), _locals)
