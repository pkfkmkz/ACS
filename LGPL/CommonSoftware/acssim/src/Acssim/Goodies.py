...
def getComponent(comp_name):
    ...
    '''
    #sanity check
    if (comp_name in _COMP_REFS) == False:
        return None
...
def getSimProxy(comp_name, comp_type=None):
    ...
    '''
    #sanity check
    if comp_name not in _COMP_PROXIES:
...
def getComponentXMLObj(comp_name):
    ...
    try:
        ...
        t_xml = getCDBAccess().getField("alma/simulated/" + comp_name)
...
def getCompLocalNS(comp_name):
    ...
    '''
    #sanity check
    if (comp_name in COMPONENTS_NS)==0:
...
def supplyEventByInstance(component_name, channel_name, event_instance):
    ...
    '''
    #sanity check
    if (channel_name in _SUPPLIERS_DICT)==False:
...
def __cleanUp():
    ...
    for sup in list(_SUPPLIERS_DICT.values()):
        sup.disconnect()
