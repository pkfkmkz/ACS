...
def logger():
    global LOGGER
    if LOGGER is None:
        LOGGER = getLogger("Acssim.Servants.Genertor")
    return LOGGER
...
def getRandomValue(typeCode, compRef):
    ...
    except CORBA.NO_IMPLEMENT as e:
        raise e
    except:
...
def getRandomEnum(typeCode):
    ...
    tGlobals = {}
    tLocals  = {}
    exec("from random import choice", tGlobals, tLocals)
    exec("import " + modName, tGlobals, tLocals)
...
def getRandomTuple(typeCode, compRef, valType):
    ...
    realValType = typeCode.kind()
    realTypeCode  = typeCode.content_type()
    ...
    if realValType == CORBA.tk_sequence:
        ...
        if realTypeCode.kind()==CORBA.tk_octet or realTypeCode.kind()==CORBA.tk_char:
            ...
            if isinstance(retVal[0], str):
                return six.ensure_binary(''.join(retVal))
            retVal.insert(0, '')
            return six.ensure_binary(reduce((lambda x, y : str(x) + chr(y)), retVal))
        else:
            return tuple(retVal)
