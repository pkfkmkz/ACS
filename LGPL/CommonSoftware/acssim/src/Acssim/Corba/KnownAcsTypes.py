...
def logger():
    global LOGGER
    if LOGGER is None:
        LOGGER = getLogger("Acssim.Servants.KnownAcsTypes")
    return LOGGER
...
def getKnownBaciType(ifr_id):
    ...
    if ifr_id == "IDL:alma/ACSErr/Completion:1.0":
        return ACSErr.Completion(int(getTimeStamp().value),  #ULL;
...
def tryCallbackParams(params, compRef):
    ...
    compl = ACSErr.Completion(int(getTimeStamp().value),  #ULL
                              0,  #ACSErr::CompletionType type;
                              0,  #ACSErr::CompletionCode code;
                              ())
                              
    cbId = 0    #default value
    for param in params:
        if isinstance(param, ACS.CBDescIn):
            cbId = param.id_tag
            break
        
    cbDescOut = ACS.CBDescOut(0, cbId)
