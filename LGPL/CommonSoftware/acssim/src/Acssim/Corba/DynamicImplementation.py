...
def _mergeClasses(complete_dict, new_class):
    ...
    temp_keys = list(new_class.__dict__.keys())
...
def _createMethodImplementation(obj_ref, meth_name):
    ...
    try:
        ...
        #add the method.
        obj_ref.__dict__[meth_name] = MethodType(genericFunction, obj_ref)
    return
...
#------------------------------------------------------------------------------
class ADynImpl(with_metaclass(DynamicImplementationMeta, object)):
    pass
...
class DynamicImplementation(ADynImpl):
    ...
    def __init__(self, ir_id=None):
        ...
        if self.__real_class is None:
            importlib.reload(self.__real_mod)
            self.__real_class = self.__real_mod.__dict__.get(classname) #get class
