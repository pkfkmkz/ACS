...
def getSuperIDs(ir_id):
    ...
    interface = getIFR().lookup_id(ir_id)
...
def getDefinition(ir_label):
    ...
    Raises: ???
    '''
    interf = getIFR().lookup_id(ir_label)
    return interf
#-----------------------------------------------------------------------------
def listToCodeObj(code_list, locals_dict):
    ...
    compiler("from __future__ import print_function", "sim_file")
    code_obj = compiler(code_string, "sim_file") #, "exec")
