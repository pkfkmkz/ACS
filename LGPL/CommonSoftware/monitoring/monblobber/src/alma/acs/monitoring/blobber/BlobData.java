...
public class BlobData extends ComponentData
{
	...
	void calculateStatistics() {
		if (getDataSize() > 0) {
			...
			if (sampleData instanceof Integer) {
				statistics.min = Integer.valueOf((int) Math.round(stat.getMin()));
				statistics.max = Integer.valueOf((int) Math.round(stat.getMax()));
				statistics.mean = Double.valueOf(stat.getMean()); // or Float, to indicate lower precision?
				statistics.stdDev = Double.valueOf(stat.getStandardDeviation()); // or Float, to indicate lower precision?
			}
			else if (sampleData instanceof Long) {
				statistics.min = Long.valueOf(Math.round(stat.getMin()));
				statistics.max = Long.valueOf(Math.round(stat.getMax()));
				statistics.mean = Double.valueOf(stat.getMean()); 
				statistics.stdDev = Double.valueOf(stat.getStandardDeviation()); 
			}
			else if (sampleData instanceof Float) {
				statistics.min = Float.valueOf(Double.valueOf(stat.getMin()).floatValue());
				statistics.max = Float.valueOf(Double.valueOf(stat.getMax()).floatValue());
				statistics.mean = Float.valueOf(Double.valueOf(stat.getMean()).floatValue());
				statistics.stdDev = Float.valueOf(Double.valueOf(stat.getStandardDeviation()).floatValue());
			}
			else if (sampleData instanceof Double) {
				statistics.min = Double.valueOf(stat.getMin());
				statistics.max = Double.valueOf(stat.getMax());
				statistics.mean = Double.valueOf(stat.getMean());
				statistics.stdDev = Double.valueOf(stat.getStandardDeviation());
			}
		}
