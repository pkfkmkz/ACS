...
static const char *rcsId="@(#) $Id: MonitorComponent.cpp,v 1.6 2012/10/10 09:48:54 bjeram Exp $";
...
bool MonitorComponent::addProperty(const char *propName,  const char *pType,  ACS::Property* propRef, ACS::TimeInterval &monitoringInterval)
{
	...
	{
		if (propType.find("RO")!=ACE_CString::npos) {
			mp = new ROMONITORPOINTNSSEQ(ACS, longLong, longLong)(
					propName,
					monitoringInterval,
					propRef,
					TMCDB::longLongSeqValueType,
					monitorDataBlock_m.monitorBlobs[seqIndex_m]
			);
			ALARMTIENSSEQ(ACS, longLong, longLong) *alarmServant = new ALARMTIENSSEQ(ACS, longLong, longLong)(dynamic_cast<ROMONITORPOINTNSSEQ(ACS, longLong, longLong) *>(mp), false);
			dynamic_cast<ROMONITORPOINTNSSEQ(ACS, longLong, longLong) *>(mp)->setAlarmServant(alarmServant);
		} else
			mp = new MONITORPOINTNSSEQ(ACS, longLong, longLong)(
					propName,
					monitoringInterval,
					propRef,
					TMCDB::longLongSeqValueType,
		...
		CBTIENSSEQ(ACS, longLong, longLong) *monitorServant = new CBTIENSSEQ(ACS, longLong, longLong)(dynamic_cast<MONITORPOINTNSSEQ(ACS, longLong, longLong) *>(mp), false);
		dynamic_cast<MONITORPOINTNSSEQ(ACS, longLong, longLong) *>(mp)->setMonitorServant(monitorServant);
		mp->activate(containerServices_m);
		monitorPoints_m[seqIndex_m] = mp;
		seqIndex_m++;
		return true;;
	...
	{
		if (propType.find("RO")!=ACE_CString::npos) {
			mp = new ROMONITORPOINTNSSEQ(ACS, uLongLong, uLongLong)(
					propName,
					monitoringInterval,
					propRef,
					TMCDB::uLongLongSeqValueType,
					monitorDataBlock_m.monitorBlobs[seqIndex_m]
			);
			ALARMTIENSSEQ(ACS, uLongLong, uLongLong) *alarmServant = new ALARMTIENSSEQ(ACS, uLongLong, uLongLong)(dynamic_cast<ROMONITORPOINTNSSEQ(ACS, uLongLong, uLongLong) *>(mp), false);
			dynamic_cast<ROMONITORPOINTNSSEQ(ACS, uLongLong, uLongLong) *>(mp)->setAlarmServant(alarmServant);
		} else
			mp = new MONITORPOINTNSSEQ(ACS, uLongLong, uLongLong)(
					propName,
					monitoringInterval,
					propRef,
					TMCDB::uLongLongSeqValueType,
		...
		CBTIENSSEQ(ACS, uLongLong, uLongLong) *monitorServant = new CBTIENSSEQ(ACS, uLongLong, uLongLong)(dynamic_cast<MONITORPOINTNSSEQ(ACS, uLongLong, uLongLong) *>(mp), false);
		dynamic_cast<MONITORPOINTNSSEQ(ACS, uLongLong, uLongLong) *>(mp)->setMonitorServant(monitorServant);
		mp->activate(containerServices_m);
		monitorPoints_m[seqIndex_m] = mp;
		seqIndex_m++;
		return true;;
	...
	if (propType.find("stringSeq:")!=ACE_CString::npos)
	{
		...
		if (propType.find("RO")!=ACE_CString::npos) {
			mp = new ROMONITORPOINTSTRSEQ(
					propName,
					monitoringInterval,
					propRef,
					TMCDB::stringSeqValueType,
					monitorDataBlock_m.monitorBlobs[seqIndex_m]);
			ALARMTIESTRSEQ *alarmServant = new ALARMTIESTRSEQ(dynamic_cast<ROMONITORPOINTSTRSEQ *>(mp), false);
			dynamic_cast<ROMONITORPOINTSTRSEQ *>(mp)->setAlarmServant(alarmServant);
		} else
			mp = new MONITORPOINTSTRSEQ(
					propName,
					monitoringInterval,
					propRef,
					TMCDB::stringSeqValueType,
					monitorDataBlock_m.monitorBlobs[seqIndex_m]);
		CBTIESTRSEQ *monitorServant = new CBTIESTRSEQ(dynamic_cast<MONITORPOINTSTRSEQ *>(mp), false);
		dynamic_cast<MONITORPOINTSTRSEQ *>(mp)->setMonitorServant(monitorServant);
		mp->activate(containerServices_m);
		monitorPoints_m[seqIndex_m] = mp;
		seqIndex_m++;
		return true;;
