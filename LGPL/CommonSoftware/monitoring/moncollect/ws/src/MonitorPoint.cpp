...
static const char *rcsId="@(#) $Id: MonitorPoint.cpp,v 1.5 2012/10/10 09:48:54 bjeram Exp $";
	...
	monitorBlob_m(mb)
{
	...
	std::mt19937 mt(rd());
	std::uniform_int_distribution<ACS::TimeInterval> dist(-30.0/100e-9, 30.0/100e-9);
	wait_m = 1800.0/100e-9 + dist(mt);
}
...
void EnumMonitorPoint::working(CORBA::ULong value, const ACSErr::Completion& comp, const ACS::CBDescOut& cbdescout)
{
    ...
    if ( curSeqPos_m < seqLen_m )
    {
        ...
        if (bufferFull)
        {
            static RepeatGuard repeatGuard(wait_m, 0);
            if(repeatGuard.check()) {
                CORBA::String_var propName = property_m->name();
                ACS_SHORT_LOG((LM_WARNING, "Data lost from buffer on property '%s'", propName.in()));
            }
            curSeqInit_m++;
        }
    }
