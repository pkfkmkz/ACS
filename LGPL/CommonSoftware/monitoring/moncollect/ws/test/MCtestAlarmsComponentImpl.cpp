	...
	m_EnumTestROProp_p(0)
{
	...
	for(unsigned int i=0;i<m_longLongSeqROVal.length();i++)
		m_longLongSeqROVal[i]=1;
	m_time12 = 134608945243381570;
	m_longLongSeqRODevIO = new MCtestDevIOSeqNoIncremental<ACS::longLongSeq>(m_longLongSeqROVal, m_time12);
	...
	for(unsigned int i=0;i<m_uLongLongSeqROVal.length();i++)
		m_uLongLongSeqROVal[i]=1;
	m_time13 = 134608945243381570;
	m_uLongLongSeqRODevIO = new MCtestDevIOSeqNoIncremental<ACS::uLongLongSeq>(m_uLongLongSeqROVal, m_time13);
...
ACS::ROlongLongSeq_ptr MCtestAlarmsComponentImpl::longLongSeqROProp()
{
	if(m_longLongSeqROProp_p == 0)
	{
...
ACS::ROuLongLongSeq_ptr MCtestAlarmsComponentImpl::uLongLongSeqROProp()
{
	if(m_uLongLongSeqROProp_p == 0)
	{
...
void MCtestAlarmsComponentImpl::reset()
{
	...
	for(unsigned int i=0;i<m_uLongSeqROVal.length();i++)
			m_longLongSeqROVal[i]=1;
	m_longLongSeqROProp_p->getDevIO()->write(m_longLongSeqROVal, m_time12);
	for(unsigned int i=0;i<m_uLongSeqROVal.length();i++)
			m_uLongLongSeqROVal[i]=1;
	m_uLongLongSeqROProp_p->getDevIO()->write(m_uLongLongSeqROVal, m_time13);
	for(unsigned int i=0;i<m_booleanSeqROVal.length();i++)
			m_booleanSeqROVal[i]=true;
	m_booleanSeqROProp_p->getDevIO()->write(m_booleanSeqROVal, m_time14);
	m_EnumTestROVal = 1;
	m_EnumTestROProp_p->getDevIO()->write(m_EnumTestROVal, m_time15);
}
...
void MCtestAlarmsComponentImpl::increase()
{
	...
	m_uLongSeqROProp_p->getDevIO()->write(m_uLongSeqROVal, m_time11);
	for(unsigned int i=0;i<m_longLongSeqROVal.length();i++)
		m_longLongSeqROVal[i]++;
	m_longLongSeqROProp_p->getDevIO()->write(m_longLongSeqROVal, m_time12);
	for(unsigned int i=0;i<m_uLongLongSeqROVal.length();i++)
		m_uLongLongSeqROVal[i]++;
	m_uLongLongSeqROProp_p->getDevIO()->write(m_uLongLongSeqROVal, m_time13);
	for(unsigned int i=0;i<m_booleanSeqROVal.length();i++)
		m_booleanSeqROVal[i] = true;
	m_booleanSeqROProp_p->getDevIO()->write(m_booleanSeqROVal, m_time14);
