...
public abstract class AbstractReverseEngineeringStrategy extends DelegatingStrategy {
	...
	public AbstractReverseEngineeringStrategy(RevengStrategy delegate) { super(delegate); };
		...
		for (int i = 0; i < inheritanceTranslators.length; i++) {
			...
			if( cType != null ) {
				if( cType == CascadeType.NONE) {
					cascade = "none"; // if not set, it produces an EJB3 "all" cascading
					break;
				}
				else if( cType == CascadeType.AGGREGATION ) {
					cascade = "save-update, persist, lock";
					break;
				}
					...
					cascade = "none"; // fallback
					break;
				}
			}
		}
		return RevengUtils.createAssociationInfo(cascade, null, false, false);
	}
