...
public class ClientProxy extends CORBAReferenceSerializator implements Client, Serializable
{
	...
	public static long pingTimeoutSec = (long) DEFAULT_PING_TIMEOUT_DEFAULT_SEC*1000;
	...
	static {
		...
		if (secValue != null)
		{
			try 
			{
				pingTimeoutSec = (long) Double.parseDouble(secValue)*1000;
			} catch (Throwable th) {
				// noop
			}

			// at least one second
			pingTimeoutSec = Math.max(1000, pingTimeoutSec);
		}
	}
	...
	public boolean ping() throws RemoteException
	{
		...
		try
		{
			// NOTE: JacORB 3.4 returns the new instance
	...
	/**
	 * @see com.cosylab.acs.maci.Client#pingCB()
	 */
	public void pingCB(si.ijs.maci.PingCallback cb) throws RemoteException
	{
		// invalid reference check
		if (client == null)
			return;
	...
	public si.ijs.maci.Client wrapForRoundtripTimeout(si.ijs.maci.Client corbaRef, long timeout) throws AcsJCORBAProblemEx {
		...
		try
		{
			...
			rrtPolicyAny.insert_ulonglong(UTCUtility.durationJavaMillisToOmg(timeout));
			...
			ex2.setInfo("Failed to set the object-level client-side corba roundtrip timeout to " + (timeout/1000.0));
			throw ex2;
		}
	}
