...
public class PingTimerTask extends TimerTask 
{
	...
	class PingCallbackImpl extends si.ijs.maci.PingCallbackPOA {
	...
	public void run()
	{
		try
		{
			...
			clientInfo.getClient().pingCB(cb._this(CORBAReferenceSerializator.getOrb()));

			// malfunctioning client check
			if (cb.waitForPing(ClientProxy.pingTimeoutSec) == false)
			{
