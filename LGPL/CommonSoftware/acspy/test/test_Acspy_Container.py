...
class TestInitialization(unittest.TestCase):
    ...
    @mock.patch.object(Acspy.Container.Container, 'configCORBA')
    ...
    @mock.patch.object(Acspy.Container.Container, 'configCORBA')
    @mock.patch.object(Acspy.Container, 'BaseClient')
    ...
    @mock.patch.object(Acspy.Container.Container, 'configCORBA')
    @mock.patch.object(Acspy.Container, 'BaseClient')
    @mock.patch.object(Acspy.Container, 'CDBaccess')
    ...
    @mock.patch.object(Acspy.Container, 'BaseClient')
    @mock.patch.object(Acspy.Container, 'CDBaccess')
    @mock.patch.object(Acspy.Container, 'findFile')
...
class TestGetMyCorbaRef(unittest.TestCase):
    ...
    @mock.patch.object(Acspy.Container.ACSCorba, 'getPOARoot')
    @mock.patch.object(Acspy.Container, 'CDBaccess')
...
class TestActivateComponent(unittest.TestCase):
    ...
    @mock.patch.object(Acspy.Container, 'BaseClient')
    @mock.patch.object(Acspy.Container.Container, 'configCORBA')
    @mock.patch.object(Acspy.Container.Container, 'getMyCorbaRef')
    @mock.patch.object(Acspy.Container.Container, 'getCDBInfo')
    ...
    def test_bad_class_fault(self):
        mockmodule = types.ModuleType('MockClass')
        sys.modules['MockModule'] = mockmodule
        offshootpoamock = mock.Mock(spec=omniORB.PortableServer.POA)
        offshootpoamock.create_POA.return_value = \
                                     mock.Mock(spec=omniORB.PortableServer.POA)
    ...
    def test_bad_class_type_fault(self):
        def raiser(*args):
            raise TypeError("boom!")
            
        mockmodule = types.ModuleType('MockClass')
        sys.modules['MockClass'] = mockmodule
        mockmodule.__dict__['MockClass'] = type('MockClass',(), {})
        with mock.patch.object(mockmodule.MockClass, "__new__") as instancemock:
            instancemock.side_effect = raiser
            offshootpoamock = mock.Mock(spec=omniORB.PortableServer.POA)
            offshootpoamock.create_POA.return_value = mock.Mock(spec=omniORB.PortableServer.POA)
            self.tc.createPOAForComponent.return_value = offshootpoamock
            self.assertRaises(Acspy.Container.CannotActivateComponentExImpl,
                              self.tc.activate_component,
                              12345, 12, 'Test', 'MockClass',
...
class TestDeactivation(unittest.TestCase):
    ...
    def test_not_component(self):
        ...
        componentdata[Acspy.Container.PYREF] = type('MockClass', (Acspy.Container.ContainerServices,
                                                                          Acspy.Container.ACSComponent,
                                                                          Acspy.Container.ComponentLifecycle), {})()
        componentdata[Acspy.Container.COMPMODULE] = types.ModuleType('MockClass')
...
class TestDestruction(unittest.TestCase):
    ...
    @mock.patch.object(Acspy.Container, 'BaseClient',spec=Acspy.Clients.BaseClient.BaseClient)
    @mock.patch.object(Acspy.Container.Container, 'configCORBA')
    @mock.patch.object(Acspy.Container.Container, 'getMyCorbaRef')
    @mock.patch.object(Acspy.Container.Container, 'getCDBInfo')
    def setUp(self, confcorbamock, refmock, cdbmock, baseclientmock):
    ...
    @mock.patch.object(Acspy.Container.ACSCorba, 'getManager')
    ...
    @mock.patch.object(Acspy.Common.Log, 'stopPeriodicFlush')
    @mock.patch.object(Acspy.Container.ACSCorba, 'getManager')
    @mock.patch.object(Acspy.Clients.BaseClient.BaseClient,'managerLogout')
...
class TestLoggingConfigurableInterface(unittest.TestCase):
    ...
    def getElementMock(self, path, element):
        ret = []
        try:
            print("getElementMockA", path, element, ret)
            loggingConfig = [{ 'Name':'DefinedTestComponent', 'minLogLevel':'8', 'minLogLevelLocal':'9'},
                             { 'Name':'PartiallyDefinedTestComponentA', 'minLogLevelLocal':'9'},
                             { 'Name':'PartiallyDefinedTestComponentB', 'minLogLevel':'8'}]
            if path == "MACI/Containers/" + self.tc.name + "/LoggingConfig" and element == "LoggingConfig/log:e":
                ret = loggingConfig
            if path == "MACI/Containers/" + self.tc.name + "/LoggingConfig/DefinedTestComponent" and element == "log:e":
                ret = [loggingConfig[0]]
            if path == "MACI/Containers/" + self.tc.name + "/LoggingConfig/PartiallyDefinedTestComponentA" and element == "log:e":
                ret = [loggingConfig[1]]
            if path == "MACI/Containers/" + self.tc.name + "/LoggingConfig/PartiallyDefinedTestComponentB" and element == "log:e":
                ret = [loggingConfig[2]]
            print("getElementMock", path, element, ret)
        except Exception as e:
            print("getElementMockC", e)
        return ret
    ...
    def test_configureComponentLogger(self):
        ...
        self.tc.cdbAccess.getElement.side_effect = self.getElementMock
        if 'ACS_LOG_STDOUT' in environ: del environ['ACS_LOG_STDOUT']
        if 'ACS_LOG_CENTRAL' in environ: del environ['ACS_LOG_CENTRAL']
    ...
    def test_configureComponentLogger_partialCentral(self):
        """Partially-defined (central) logger is configured to the CDB values."""
        self.tc.cdbAccess.getElement.side_effect = self.getElementMock
        if 'ACS_LOG_STDOUT' in environ: del environ['ACS_LOG_STDOUT']
        if 'ACS_LOG_CENTRAL' in environ: del environ['ACS_LOG_CENTRAL']
        self.tc.refresh_logging_config()
    ...
    def test_configureComponentLogger_centralEnv(self):
        """Partially-defined (central) logger is configured to the CDB values and not CENTRAL environment value."""
        self.tc.cdbAccess.getElement.side_effect = self.getElementMock
        if 'ACS_LOG_STDOUT' in environ: del environ['ACS_LOG_STDOUT']
    ...
    @mock.patch.object(Acspy.Container.Log, 'getLogger')
