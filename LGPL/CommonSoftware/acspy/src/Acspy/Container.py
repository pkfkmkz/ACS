...
from cdbErrType                             import CDBFieldDoesNotExistEx
...
class AsyncComponentActivator(Thread):
    ...
    def run(self):
        ...
        descOut=CBDescOut(0,self.desc.id_tag)
        try:
...
class Container(BaseClient, maci__POA.Container, Logging__POA.LoggingConfigurable):
    ...
    def __init__ (self, name):
        ...
        # Set to delay calls to activate_component
        self.ready=Event()
        self.ready.clear() 
        ...
        # The map to serialize activation/deactivation of the same component
        #
        # This map is used to reduce the size of the critical sections
        # so that concurrent activations of components with different 
        # names can proceed in parallel while the activation/deactivation of
        # components with the same name is serialized
        self.activationMap=ContainerActivationMap(self.logger)

        #Run everything
        self.logger.logInfo('Container ' + self.name + ' waiting for requests')
        self.logger.logInfo('Container ' + self.name + ' used libraries from ' + environ['PYTHONPATH'])
    ...
    def disconnect(self): # pragma: NO COVER
        ...
        if self.reconnectionThread is not None:
            if self.reconnectionThread.is_alive():
    ...
    def activate_component(self, h, exeid, name, exe, idl_type):
        ...
        '''
        # Delays the activation of components until the container has been initialized 
        self.ready.wait()
        
        # Signal the map that the container is going to start a component
        #
        # If another thread is already activating (or deactivating) a component
        # with this same name then aboutToActivate blocks until it finishes
        self.activationMap.aboutToActivate(name)
        ...
        try:
            ...
            #If we're activating the component, the module should be reloaded. See ICT-19366
            if temp[COMPMODULE] not in self.compModuleCount:
                try:
                    importlib.reload(temp[COMPMODULE])
                except:
                    self.logger.logWarning("Unable to reload:" + str(temp[COMPMODULE]))
                    print_exc()
            interval = time() - start
            ...
            try:
                ...
                except TypeError as e: # When TypeError is thrown, this means that is not a Simulator, let's try like a normal component 
        ...
        try:
            start = time()
            try:
                if temp[PYREF].__init__.__code__.co_argcount == 2: # First try if it's a Simulator
        ...
        if isinstance(temp[PYREF], ComponentLifecycle):
            ...
            except ComponentLifecycleException as e:
                print_exc()
        ...
        #ICT-13587
        try:
            componentsCdbPath = "MACI/Components"
            myDao = None
            try:
                myDao = self.cdbAccess.m_dal.get_DAO_Servant(componentsCdbPath + "/" + name)
            except CDBRecordDoesNotExistEx as e:
                myDao = self.cdbAccess.m_dal.get_DAO_Servant(componentsCdbPath + "/*")
                ctype = myDao.get_string("Type")
                i = 1
                while ctype != idl_type:
                    myDao = self.cdbAccess.m_dal.get_DAO_Servant(componentsCdbPath + "/*" + str(i))
                    ctype = myDao.get_string("Type")
                    i += 1
            keepAlive = myDao.get_string("KeepAliveTime")
            self.logger.logInfo("Component '" + name + "' has KeepAliveTime '" + str(keepAlive) + "'.")
        except CDBFieldDoesNotExistEx as e:
            self.logger.logInfo("Component '" + name + "' doesn't have any KeepAliveTime configuration associated with it.")
        except CDBRecordDoesNotExistEx as e:
            self.logger.logInfo("Component '" + name + "' doesn't have any KeepAliveTime configuration associated with it.")
        
        ret=self.components[name][COMPONENTINFO]
        self.activationMap.activated(name)
        return ret
    ...
    def failedActivation(self, comp_entry): # pragma: NO COVER
        ...
        if comp_entry[NAME] in self.components:
            del self.components[comp_entry[NAME]]
        if  comp_entry[HANDLE] in self.compHandles:
    ...
    def deactivate_component(self, handle):
        ...
        try:
            ...
            compName=comp_entry[NAME]
            self.activationMap.aboutToDeactivate(compName)
        except:
        ...
        #if the number of references to this module falls to zero, we remove from the map
        ...
        self.activationMap.deactivated(compName)
    ...
    def configureComponentLogger(self, name):
        ...
        try:
            ...
            logconfig = self.cdbAccess.getElement("MACI/Containers/"  + self.name + "/LoggingConfig", "LoggingConfig/log:e")
    ...
    def get_component_info(self, handles):
        ...
        if (handles == None) or (handles == []):
            for record in list(self.components.keys()):
                return_seq.append(self.components[record][COMPONENTINFO])
            return return_seq

        for handle in handles:
            if handle in self.compHandles:
    ...
    def getCDBInfo(self):
        ...
        temp_list = []
        try:
            temp_list = self.cdbAccess.getElement("MACI/Containers/" + self.name + "/Autoload", "Autoload/cdb:e")
        except:
        ...
        for temp_package in self.autoLoadPackages:
            try:
                exec(compile(open(temp_package).read(), temp_package, 'exec'))
            except:
    ...
    def handler(self, signum, frame): # pragma: NO COVER
        ...
        if (self.reconnectionThread is not None) and self.reconnectionThread.is_alive():
            self.reconnectionThread.terminateThread()
        ...
        #let us know about!
        for h in list(self.compHandles.keys()):
            self.deactivate_component(h)
