...
def get_integration_logs(channel_name):
    ...
    #resources
    if channel_name in INTEGRATION_LOGS:
...
def get_channel_qofs_props(channel_name):
    ...
    #timeout
    temp_any = any.to_any(int(temp_dict[CosNotification.Timeout]))
    ...
    temp_any = any.to_any(int(temp_dict[CosNotification.MaxEventsPerConsumer]))
...
def get_channel_admin_props(channel_name):
    ...
    #max queue length
    temp_any = any.to_any(int(temp_dict[CosNotification.MaxQueueLength]))
    prop_seq.append(CosNotification.Property(CosNotification.MaxQueueLength,
                                             temp_any))
    
    #max consumers
    temp_any = any.to_any(int(temp_dict[CosNotification.MaxConsumers]))
    prop_seq.append(CosNotification.Property(CosNotification.MaxConsumers,
                                             temp_any))

    #max suppliers
    temp_any = any.to_any(int(temp_dict[CosNotification.MaxSuppliers]))
...
def getEventHandlerTimeoutDict(channel_name):
    ...
    try:
        events = xml_obj.EventChannel.Events.e
    except:
    ...
    if isinstance(events, Sequence)==0:
        events = [ events ]
