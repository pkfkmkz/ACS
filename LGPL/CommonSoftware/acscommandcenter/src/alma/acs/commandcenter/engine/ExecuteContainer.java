...
public class ExecuteContainer {
	...
	String containerType = runModel.getContainerType();
        if (containerType.equals("py")) {
		containerType = containerType + "_unbuffered";
	}
        Tool t = ToolManager.getBuiltinTool("Container_startLocalScript_" + containerType);
        String command = ToolManager.generateCommand(t, runModel);

        Executor.localOutProc(command, t.getEnviron(), true, t.getMaxStartupSeconds() * 1000, t.getExpectedOutput(),
                listener);
    }
