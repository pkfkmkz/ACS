...
public class MaciSupervisor implements IMaciSupervisor {
	...
	public synchronized void refreshNow() throws NoPermissionEx, NotConnectedToManagerException, SystemException, CorbaTransientException, CorbaNotExistException, UnknownErrorException {
		...
		try {
			...
			boolean compsEqual = CollectionUtils.isEqualCollection(maciInfo.components, newComponents, eqComponents);
			boolean contsEqual = CollectionUtils.isEqualCollection(maciInfo.containers, newContainers, eqContainers);
			boolean clientsEqual = CollectionUtils.isEqualCollection(maciInfo.clientApps, newClientApps, eqClients);
			
			// 2013-05 Marcus OSF Mission -----------------------------------------
			if (log.isLoggable(Level.FINER))
				log.finer(write+"diff results: containers="+!contsEqual
						+", components="+!compsEqual
						+", clients="+!clientsEqual);
			// --------------------------------------------------------------------
	...
	private final Equator<ComponentInfo> eqComponents = new Equator<ComponentInfo>() {
		public boolean equate(ComponentInfo ci1, ComponentInfo ci2) {
			return Arrays.deepEquals(new Object[]{ci1.name, ci1.h, ci1.container, ci1.clients}, new Object[]{ci2.name, ci2.h, ci2.container, ci2.clients});
		}
	...
	private final Equator<ContainerInfo> eqContainers = new Equator<ContainerInfo>() {
		public boolean equate(ContainerInfo ci1, ContainerInfo ci2) {
			return Arrays.deepEquals(new Object[]{ci1.name, ci1.h, ci1.components}, new Object[]{ci2.name, ci2.h, ci2.components});
		}
