...
#define MSG_PARSE_ERROR "Parse error at line %lu:%s"
...
void DAOProxy::Start(const char *el, const char **attr)
{
	...
	if ((ACE_OS::strcmp(el, "_") == 0 || ACE_OS::strcmp(el, "e") == 0) && attr[0])
	    {
    ...
    if(!m_inArray && ACE_OS::strcasecmp(el, "cdb:e") == 0)
	{
	m_inArray = true;
...
void DAOProxy::get_field(const char* name, string &value)
{
	...
	if(iter != pNode->m_fieldMap.end())
		...
		ACS_LOG(LM_RUNTIME_CONTEXT, "cdb::DAOProxy::get_field",
			(LM_INFO, "Field %s/%s does not exist.", m_nodeName.c_str(), name));
