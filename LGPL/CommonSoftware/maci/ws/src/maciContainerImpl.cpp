...
CDB::DAL_ptr
ContainerImpl::getContainerCDB()
{
    ACE_Guard<ACE_Recursive_Thread_Mutex> guard(m_cdb_dalMutex);
    ACS_TRACE(__PRETTY_FUNCTION__);
    if (CORBA::is_nil(m_cdb_dal.in())) {
        ACS_LOG(LM_RUNTIME_CONTEXT, __PRETTY_FUNCTION__, (LM_INFO, "Getting CDB reference for first time..."));
        CORBA::Object_var obj;
        try {
            maci::Manager_var mgr = maci::Manager::_duplicate(getManager());
            if (CORBA::is_nil(mgr.in()))
                throw CanNotGetCDBExImpl (__FILE__, __LINE__, __PRETTY_FUNCTION__);
            obj = mgr->get_component(m_handle, "CDB", false);
            ...
            ACSErrTypeCommon::CORBAProblemExImpl corbaProblemEx(__FILE__, __LINE__, __PRETTY_FUNCTION__);
            corbaProblemEx.setMinor(ex.minor());
            corbaProblemEx.setCompletionStatus(ex.completed());
            corbaProblemEx.setInfo(ex._info().c_str());
            throw CanNotGetCDBExImpl (corbaProblemEx, __FILE__, __LINE__, __PRETTY_FUNCTION__);
        } catch (...) {
        ...
        m_cdb_dal = CDB::DAL::_narrow(obj.in());
        if (CORBA::is_nil(m_cdb_dal.in())) {
            ACS_LOG(LM_RUNTIME_CONTEXT, __PRETTY_FUNCTION__, (LM_ERROR, "Failed to obtain reference to CDB."));
        }
    } else {
      ACS_LOG(LM_RUNTIME_CONTEXT, __PRETTY_FUNCTION__, (LM_INFO, "Returning previously gotten CDB..."));
    }
    return CDB::DAL::_duplicate(m_cdb_dal.ptr());
}

CORBA::ORB_var ContainerImpl::getContainerORB()
{
    return orb;
}
...
ContainerImpl::connect()
{
      ...
      if (info.ptr()==0)
	{
      ...
      m_containerServices = instantiateContainerServices(info->h,ctrName,unusedType,poaContainer.in());
...
void ContainerImpl::etherealizeComponent(const char * id, PortableServer::Servant servant) {
    ...
    for (COMPONENT_HASH_MAP_ITER hash_iter(m_activeComponents); hash_iter.next(entry) != 0; hash_iter.advance()) {
        if (ACE_OS::strcmp(id, entry->int_id_.info.name.in())==0) {
            compName = CORBA::string_dup(entry->int_id_.info.name.in());
            compLib = entry->int_id_.lib;
            ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::etherealizeComponent", (LM_INFO, "Component '%s' etherealized.", compName.in()));
    ...
    try{
        ...
        maciErrType::ComponentDeletionExImpl dex(ex, __FILE__, __LINE__, "ContainerImpl::etherealizeComponent");
        dex.setCURL(id);
        dex.setReason("problem in servant's DTOR?. Please check component's DTOR!");
        dex.log(LM_CRITICAL);
        errorStatus = true;
    } catch(CORBA::SystemException &ex) {
        ACSErrTypeCommon::CORBAProblemExImpl corbaProblemEx(__FILE__, __LINE__, "ContainerImpl::etherealizeComponent");
        corbaProblemEx.setMinor(ex.minor());
        corbaProblemEx.setCompletionStatus(ex.completed());
        corbaProblemEx.setInfo(ex._info().c_str());
        maciErrType::ComponentDeletionExImpl dex(corbaProblemEx, __FILE__, __LINE__, "ContainerImpl::etherealizeComponent");
        dex.setCURL(id);
        dex.setReason("problem in servant's DTOR?. Please check component's DTOR!");
        dex.log(LM_CRITICAL);
        errorStatus = true;
    } catch(std::exception &ex) {
        ACSErrTypeCommon::StdExceptionExImpl stdex(__FILE__, __LINE__, "ContainerImpl::etherealizeComponent");
        stdex.setWhat(ex.what());
        maciErrType::ComponentDeletionExImpl dex(stdex, __FILE__, __LINE__, "ContainerImpl::etherealizeComponent");
        dex.setCURL(id);
        dex.setReason("problem in servant's DTOR?. Please check component's DTOR!");
        dex.log(LM_CRITICAL);
        errorStatus = true;
    } catch (...) {
        ACSErrTypeCommon::UnexpectedExceptionExImpl uex(__FILE__, __LINE__, "ContainerImpl::etherealizeComponent");
        maciErrType::ComponentDeletionExImpl dex(uex, __FILE__, __LINE__, "ContainerImpl::etherealizeComponent");
        dex.setCURL(id);
        dex.setReason("problem in servant's DTOR?. Please check component's DTOR!");
        dex.log(LM_CRITICAL);
        errorStatus = true;
    ...
    if (found) {
        if (errorStatus) {
            ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::etherealizeComponent",
              (LM_WARNING, "Although there was problem to etherealized (dtor) component '%s' going to TRY to unload the library what can have an unpredicted result.", compName.in()));
            ContainerImpl::getLoggerProxy()->flush();
        }
        m_dllmgr->unlock(compLib);
    }
    if (errorStatus) {
        ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::etherealizeComponent",
...
// @todo: I do not know if here we have to throw an exception ?
  ...
  try {
    std::string componentsCdbPath = "MACI/Components";
    CDB::DAL_var dal = getContainerCDB();
    CDB::DAO_var myDao;
    try {
      myDao = dal->get_DAO_Servant((componentsCdbPath + std::string("/") + std::string(name)).c_str());
    } catch (cdbErrType::CDBRecordDoesNotExistEx& e) {
      myDao = dal->get_DAO_Servant((componentsCdbPath + std::string("/*")).c_str());
      std::string ctype = myDao->get_string("Type");
      int i = 1;
      while (ctype.compare(std::string(type)) != 0) {
        myDao = dal->get_DAO_Servant((componentsCdbPath + std::string("/*") + std::to_string(i)).c_str());
        ctype = myDao->get_string("Type");
        i++;
      }
    }
    CORBA::String_var keepAlive = myDao->get_string("KeepAliveTime");
    ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::activate_component", (LM_INFO, "Component '%s' has KeepAliveTime '%s'.", name, keepAlive.in()));
  } catch (cdbErrType::CDBFieldDoesNotExistEx& e) {
    ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::activate_component", (LM_INFO, "Component '%s' doesn't have any KeepAliveTime configurated associated with it.", name));
  } catch (cdbErrType::CDBRecordDoesNotExistEx& e) {
    ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::activate_component", (LM_INFO, "Component '%s' doesn't have any KeepAliveTime configurated associated with it.", name));
  }
...
void ContainerImpl::deactivate_component(maci::Handle h) {
    ...
    ContainerComponentInfo info;
    ACE_Guard<ACE_Recursive_Thread_Mutex> guard(m_activeComponents.mutex());
    if (m_activeComponents.find(h, info) != -1 && !CORBA::is_nil(info.info.reference.in())) {
        ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::deactivate_component", (LM_DEBUG, "Deactivating component with handle %d (%s of type %s)", h, info.info.name.in(), info.info.type.in()));
        ...
        if (acscomponent::ACSComponentImpl *tempComp = dynamic_cast<acscomponent::ACSComponentImpl*>(servant)) {
            ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::deactivate_component", (LM_DEBUG, "Component '%s': calling cleanUp Lifecycle .", info.info.name.in()));
        ...
        try{
            ...
            if (servant->_refcount_value() > 1) {
                ACS_LOG(LM_RUNTIME_CONTEXT, "ContainerImpl::deactivate_component(maci::Handle h)",
                  (LM_WARNING, "Current RefCount value for component '%s' should be one, however the current value is %u", info.info.name.in(), servant->_refcount_value()));
            }
            ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::deactivate_component", (LM_DEBUG, "Component '%s' reference set to nil.", info.info.name.in()));

            // Real deactivation.
            // This will also cause the call of the destructor for the component
            if (!deactivateCORBAObject(ref)) {
                ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::deactivate_component", (LM_ERROR, "Failed to deactivate component with handle %d (%s of type %s)", h, info.info.name.in(), info.info.type.in()));
            }//if
            CORBA::release(ref);

            ACS_LOG(LM_RUNTIME_CONTEXT, "maci::ContainerImpl::deactivate_component", (LM_INFO, "Component '%s' deactivated.", info.info.name.in()));
        }catch(...)
        {
            maciErrType::CannotDeactivateComponentExImpl dex(__FILE__, __LINE__, "ContainerImpl::deactivate_component");
            dex.log(LM_ERROR);
        }
 ...
 **/
void
ContainerImpl::shutdown (CORBA::ULong action) {
...
void ContainerImpl::components_available(const maci::ComponentInfoSeq & cobs)
{
...
void ContainerImpl::components_unavailable(const maci::stringSeq & cob_names) {
...
void ContainerImpl::pingCB(maci::PingCallback_ptr cb)
{
    cb->ping(true);
}
