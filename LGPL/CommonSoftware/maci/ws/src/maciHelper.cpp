...
MACIHelper::extractHostnameFromCorbaloc(const ACE_TCHAR *corbaloc)
{
  ...
  // corbaloc reference
  size_t pos = result.find("corbaloc:");
...
std::string MACIHelper::getNameServiceCorbaloc() {
  std::ostringstream corbaloc;
  corbaloc << "corbaloc::" << MACIHelper::getManagerHostname(1, NULL).c_str() << ":" << ACSPorts::getNamingServicePort().c_str() << "/NameService";
  return corbaloc.str();
}
