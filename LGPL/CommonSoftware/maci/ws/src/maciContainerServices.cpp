 ...
 *
 *
 * "@(#) $Id: maciContainerServices.cpp,v 1.48 2012/05/18 06:54:42 bjeram Exp $"
 *
 * who       when      what
 * --------  --------  ----------------------------------------------
...
ACE_Recursive_Thread_Mutex MACIContainerServices::m_offshootMutex;
...
#ifndef MAKE_VXWORKS
    , acsalarm::AlarmSourceThread* alarmSourceThread_p
#endif
    ) :
  ContainerServices(name,poa), m_manager(maci::Manager::_nil()), m_componentHandle(componentHandle)
{
...
//  m_cdb_dal_ref = CDB::DAL::_duplicate(m_containerImpl->getContainerCDBDAL().in());
}
...
MACIContainerServices::MACIContainerServices(
	...
	PortableServer::POA_ptr poa,
	Manager_ptr manager) :
	ContainerServices(name, poa), m_manager(maci::Manager::_nil()),
...
MACIContainerServices::releaseComponent(const char *name)
{
	...
	corbaProblemEx.setCompletionStatus(ex.completed());
	corbaProblemEx.setInfo(ex._info().c_str());
	maciErrType::CannotReleaseComponentExImpl ex2(corbaProblemEx,  __FILE__, __LINE__, __PRETTY_FUNCTION__);
	ex2.setCURL(name);
	throw ex2;
	}
    catch (...)
...
CDB::DAL_ptr MACIContainerServices::getCDB()
{
    CDB::DAL_ptr ret_val = CDB::DAL::_nil();
    ACS_TRACE(__PRETTY_FUNCTION__);
    try
    {
        ret_val = m_containerImpl->getContainerCDB();
        if (CORBA::is_nil(ret_val))
             throw CanNotGetCDBExImpl (__FILE__, __LINE__, __PRETTY_FUNCTION__);
        return ret_val;
    }
    catch (CanNotGetCDBExImpl &ex)
    {
        throw CanNotGetCDBExImpl (ex, __FILE__, __LINE__,	__PRETTY_FUNCTION__);
    }
    catch (...)
    {
        ACSErrTypeCommon::UnexpectedExceptionExImpl uex(__FILE__, __LINE__, __PRETTY_FUNCTION__);
        throw CanNotGetCDBExImpl (uex, __FILE__, __LINE__, __PRETTY_FUNCTION__);
    }//try-catch
}//getCDB
...
{
	ACE_Guard<ACE_Recursive_Thread_Mutex> guard(m_offshootMutex);
