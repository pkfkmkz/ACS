...
public:
  ...
  /// Get CDB DAL reference in this container
  /// ICT-8982: Preserving the reference to CDB from container initilization.
  /// @return The CDB DAL
  CDB::DAL_ptr getContainerCDB();
  ...
  CDB::DAL_var m_cdb_dal; //ICT-8982: Preserving the reference to CDB from container initilization.
  ACE_Recursive_Thread_Mutex m_cdb_dalMutex;
  ...
  std::unique_ptr<acsalarm::AlarmSourceThread> m_alarmSourceThread_ap;

  acsalarm::AlarmSourceThread* getAlarmSourceThread() {
	  if (m_alarmSourceThread_ap.get()==NULL) {
		  m_alarmSourceThread_ap=std::unique_ptr<acsalarm::AlarmSourceThread>(new acsalarm::AlarmSourceThread());
	  }
