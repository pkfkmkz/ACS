...
namespace maci {
    ...
    using ComponentSmartPtr = SmartPtr<T>;
...
public:
  ...
  virtual void pingCB(maci::PingCallback_ptr cb);
...
{
    return ComponentSmartPtr<T>(maci::make_shared_component(this, true, this->getComponent<T>(name, domain, activate)));
}
/*
...
{
    return ComponentSmartPtr<T>(maci::make_shared_component(this, true, this->getDynamicComponent<T>(compSpec,markAsDefault)));
}
...
{
    return ComponentSmartPtr<T>(maci::make_shared_component(this, false, this->getComponentNonSticky<T>(name)));
}
...
template <typename T>
	maci::SmartPtr<T> SimpleClient::getDefaultComponentSmartPtr(
			const char* idlType)
{
	return maci::SmartPtr<T>(maci::make_shared_component(this, true, this->getDefaultComponent<T>(idlType)));
}
...
{
	    return maci::SmartPtr<T>(maci::make_shared_component(this, true, 
