#!/usr/bin/env python
################################################################################################
# @(#) $Id: acsdataClean.py,v 1.11 2012/03/06 12:04:54 tstaig Exp $
#
...
#------------------------------------------------------------------------------
__version__ = "$Id: acsdataClean.py,v 1.11 2012/03/06 12:04:54 tstaig Exp $"
#------------------------------------------------------------------------------
...
def cleanDirectory(dir):
   ...
   try:
       ...
       if not exists(myDir):
           print("I'm trying but '", myDir, "' does not exist!")
           exit(1)
       ...
       for entry in tempList:
           #if OK to remove...
           if (entry not in safeEntries) and (isdir(entry)):
               try:
                   system("rm -rf " + entry)
               except:
                   print("Unable to remove this directory:", entry)
                   
           elif (entry not in safeEntries) and (isfile(entry)):
               try:
                   remove(entry)
               except:
...
if argv.count("-all")!=0 or argv.count("-a")!=0:
   myDir = str(environ['ACSDATA']) + '/tmp/'
   fileList = listdir(myDir)
   for file in fileList:
      if isfile(myDir + file) and not(islink(myDir + file)) and ".lock" in file:
          filepath = myDir + file
          print ("will remove " + filepath)
