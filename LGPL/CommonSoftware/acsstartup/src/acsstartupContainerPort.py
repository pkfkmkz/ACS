...
def portNumberAlreadyUsed(port_number, host, 
    ...
    for cont_name in list(port_dict.keys()):
...
'''
    ...
    if options.manager_reference==None:
        if 'MANAGER_REFERENCE' in environ:
    ...
    if cl_name in container_hosts and container_hosts[cl_name]!=container_host:
    ...
    if newPort!=None and cl_name in container_ports and container_ports[cl_name]!=newPort:
    ...
    elif newPort!=None and (cl_name not in container_ports) and portNumberAlreadyUsed(newPort, container_host,
