...
CASTOR_JARS:=xmljbind.jar jACSUtil.jar castor-1.4.1-acs.jar commons-lang3-3.12.0.jar commons-logging-1.2.jar
CASTOR_CLASSPATH:=$(subst $(SPACE),:,$(patsubst \#error\#%,,$(foreach jar,$(CASTOR_JARS),$(shell searchFile lib/$(jar))/lib/$(jar))))
XJC:= alma.tools.entitybuilder.jaxb.XjcBuilder
XJC_JARS:=xmljbind.jar jACSUtil.jar jaxb-xjc-3.0.2.jar jaxb-impl-3.0.2.jar jakarta.xml.bind-api-3.0.1.jar jaxb-core-3.0.2.jar jakarta.activation-2.0.1.jar istack-commons-runtime-4.1.1.jar txw2-3.0.2.jar
XJC_CLASSPATH:=$(subst $(SPACE),:,$(patsubst \#error\#%,,$(foreach jar,$(XJC_JARS),$(shell searchFile lib/$(jar))/lib/$(jar))))
XALAN:=$(shell searchFile lib/xalan-2.7.3.jar)/lib/xalan-2.7.3.jar
XALAN_SER:=$(shell searchFile lib/serializer-2.7.3.jar)/lib/serializer-2.7.3.jar
OMNI_IDL := omniidl
...
XMLIDL=java --add-opens java.xml/com.sun.org.apache.xml.internal.serialize=ALL-UNNAMED $(CompHelpDoGenerateDef) $(CompHelpOutRootDirDef) -DACS.idl2jbind=$(XML_IDL) -Djavax.xml.parsers.SAXParserFactory=com.sun.org.apache.xerces.internal.jaxp.SAXParserFactoryImpl -Djavax.xml.parsers.DocumentBuilderFactory=com.sun.org.apache.xerces.internal.jaxp.DocumentBuilderFactoryImpl $(AlmaIDLMainClass) -d $$(TMPSRC) $(VERBOSE_DEF)

ACS_CO=ACScomponents
JAVAC ?=javac -J-Xmx1g --add-exports java.management/sun.management=ALL-UNNAMED
...
PIP:=$(shell pyenv which pip)
PIP_ALL:=PYTHONPATH=../$(PYTHON_SITE_PACKAGES) $(PIP) install --no-deps --prefix=..
PIP_CLEAN:=PYTHONPATH=../$(PYTHON_SITE_PACKAGES) $(PIP) uninstall -y
PIP_INSTALL:=PYTHONPATH=$(INSTALL_ROOT)/$(PYTHON_SITE_PACKAGES) $(PIP) install --no-deps --prefix=$(INSTALL_ROOT)
...
$(eval $(call top-level,logts,$(ACSLOGTSDEF),$(ACSLOGTSDEF),true))
...
ifndef MAKE_VXWORKS
ifdef XSDBIND
$(eval $(call top-level,xsdbind,$(XSDBIND),$(XSDBIND),true))
...
ifdef ACSERRDEF
$(eval $(call top-level,xmlerr,$(ACSERRDEF),$(ACSERRDEF),true))
...
$(eval $(call top-level,idl,$(IDL_LIST),$(IDL_FILES),true))
endif
...
get-jar-artifactid=$(if $(and $(ALMASW_INSTDIR),$(wildcard $(ACSDEPS)/acs-pom.xml)),$(shell cat $(ACSDEPS)/acs-pom.xml |sed 's/xmlns=".*"//g' |xmllint --xpath "//dependency[artifactId='$1']/artifactId/text()" - 2> /dev/null))
get-jar-groupid=$(shell cat $(ACSDEPS)/acs-pom.xml |sed 's/xmlns=".*"//g' |xmllint --xpath "//dependency[artifactId='$1']/groupId/text()" -)
get-jar-version=$(shell cat $(ACSDEPS)/acs-pom.xml |sed 's/xmlns=".*"//g' |xmllint --xpath "//dependency[artifactId='$1']/version/text()" -)
get-jar-artifactid-mod=$(if $(and $(if $(and $(ALMASW_INSTDIR),$(wildcard $(ACSDEPS)/acs-pom.xml)),$(shell cat $(ACSDEPS)/acs-pom.xml |sed 's/xmlns=".*"//g' |xmllint --xpath "//modules[module='$1']/module/text()" - 2> /dev/null)),$(wildcard $(ACSDEPS)/$1/pom.xml)),$(shell cat $(ACSDEPS)/$1/pom.xml |sed 's/xmlns=".*"//g' |xmllint --xpath "//project/artifactId/text()" -))
get-jar-mod=$(shell mvn -f $(ACSDEPS)/$1/pom.xml exec:exec -q -Dexec.executable=echo -Dexec.args='$${project.groupId}:$${project.artifactId}:$${project.version}')
get-jar-info=$(if $(call get-jar-artifactid,$1),$(call get-jar-groupid,$1):$1:$(call get-jar-version,$1),$(if $(call get-jar-artifactid-mod,$1),$(call get-jar-mod,$1),$1))
...
$(eval $(call top-level,java,\
	$(JARFILES_LIST) $(C_JARFILES_LIST),$(JARFILES) $(COMPONENTS_JARFILES),true))
endif 
...
$(foreach jar,$(JARFILES_LIST),$(eval $(call acsMakeJavaDependencies,$(jar),$($(jar)_DIRS),$($(jar)_EXTRAS),off,$(USER_JFLAGS),$($(jar)_JFLAGS),$($(jar)_ENDORSED),$(DEBUG),$($(jar)_JLIBS),$($(jar)_EXTRA_SRCS))))

endif

ifneq ($(strip $(C_JARFILES_LIST)),)
$(foreach jar,$(C_JARFILES_LIST),$(eval $(call JARPrereq,$(jar))))
$(foreach jar,$(C_JARFILES_LIST),$(eval $(call acsMakeJavaDependencies,$(jar),$($(jar)_DIRS),$($(jar)_EXTRAS),on,$(USER_JFLAGS),$($(jar)_JFLAGS),$($(jar)_ENDORSED),$(DEBUG),$($(jar)_JLIBS),$($(jar)_EXTRA_SRCS))))
endif
...
javadoc: 
	...
	$(AT)for member in $(foreach jarfile, $(JARFILES_LIST), $($(jarfile)_DIRS)) ;\
		do \
		javadoc $(JavadocOptions) -classpath `acsMakeJavaClasspath -endorsed` -d ../doc/api/html `find $${member} -type f -name \*.java` > /dev/null 2>&1 ;\
		done
...
$(foreach dds,$(DDS_FILES),\
   $(eval $(call acsMakeLibraryDependencies,,$(dds)TypeSupportStubs,$(dds)TypeSupportS $(dds)TypeSupportImpl $(dds)TypeSupportC,,,$(dds)Stubs) ) \
)
...
get-pkg-artifactid=$(if $(and $(ALMASW_INSTDIR),$(wildcard $(shell python-config --prefix)/acs-py.req)),$(shell cat $(shell python-config --prefix)/acs-py.req |sed 's/\#.*//g' |sed 's/=.*//g' |sed 's/ *//g' |grep -x $1 2> /dev/null))
get-pkg-version=$(shell cat $(shell python-config --prefix)/acs-py.req |sed 's/\#.*//g' |sed 's/ *//g' |grep $1= |sed 's/.*=//g')
get-pkg-info=$(if $(call get-pkg-artifactid,$1),$1==$(call get-pkg-version,$1),$1)

.INTERMEDIATE: $(CURDIR)/../$(PYTHON_SITE_PACKAGES)/README.rst
$(CURDIR)/../$(PYTHON_SITE_PACKAGES)/README.rst:
	touch $@
...
$(eval $(call top-level,python_package,$(PY_PACK_LIST),$(PY_PACKAGES),true))
$(foreach pp,$(PY_PACK_LIST),$(eval $(call acsMakePythonPackageDependencies,$(pp))))
endif
#
ifneq ($(strip $(PY_MOD_LIST)),)
$(eval $(call top-level,python_module,$(PY_MOD_LIST),$(PY_MODULES),true))
...
.PHONY:
install_files: files_begin $(foreach ifile,$(INSTALL_FILES),install_file_$(ifile)) install_sources
