...
def searchingImport(name, globals=None, locals=None, fromlist=None):
    ...
    try:
        ...
        if inspect.ismodule(m) and not m.__name__ in sys.builtin_module_names and not hasattr(m,'__file__'):
            importlib.reload(m)
    except ImportError:
...
def _importModule(partname, fqname, parent):
    ...
    m = None
    try:
        spec = importlib.util.find_spec(partname, parent and parent.__path__)
    except ImportError:
        try:
...
builtins.__import__ = searchingImport
