...
public abstract class TestCase extends junit.framework.TestCase {
	...
	private void checkSkip( Method runMethod ) {
		...
		SessionFactoryImpl sfi = (SessionFactoryImpl) getSessions();

		if (dialectList.isEmpty() || dialectList.contains(sfi.getJdbcServices().getDialect().getClass())) {
			skip = false;
		} else {
			log.warn("Skipping test {}, because test does not apply for dialect {}", runMethod.getName(), sfi.getJdbcServices().getDialect().getClass());
			skip = true;
		}
	}
	...
	protected void runSchemaGeneration() {
		...
		for(Entry<Object, Object> prop: cfg.getProperties().entrySet()) {
