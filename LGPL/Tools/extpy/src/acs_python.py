...
def getMissingImports(packInitName, importList):
    ...
    #sanity checks
    if ("PYTHONPATH" in os.environ)==0:
        print("ERROR - $PYTHONPATH undefined")
        return retVal

    #list of all directories we should check for stubs in
    checkDirs = [x for x in os.environ["PYTHONPATH"].split(os.path.pathsep) if x != '']
...
def updateInit(path, modules, pymodules):
    os.chdir(path)
